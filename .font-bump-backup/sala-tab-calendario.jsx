...
const SALA_RES_DATA = (() => {
  const data = [
    ...
    {id:'r18', time:'20:30', dur:90,  name:'Fontana',         posti:4, table:2,  status:'confermata', phone:'+39 333 ...',       note:null, notes:'Seggiolone bambino', source:'tel'},
    ...
    {id:'r8',  time:'20:30', dur:90,  name:'Tavolata Neri',   posti:8, table:1,  status:'inattesa',   phone:'+39 320 99 88 777', note:{type:'compleanno', text:'Candelina al dolce'}, notes:null, source:'tel'},
    // Slot 21:30 (5 tavoli, dur 90 → terminano alle 23:00)
    {id:'r6',  time:'21:30', dur:90,  name:'Famiglia Rossi',  posti:4, table:7,  status:'confermata', phone:'+39 348 22 33 444', note:{type:'allergia', text:'Allergia glutine'}, notes:null, source:'tel'},
    ...
    {id:'r8b', time:'21:30', dur:90,  name:'Tavolata Neri',   posti:8, table:8,  status:'inattesa',   phone:'+39 320 99 88 777', note:{type:'compleanno', text:'Compleanno · candelina'}, notes:null, source:'tel'},
    ...
    {id:'r26', time:'12:00', dur:75,  name:'Mele',           posti:2, table:4,    status:'inattesa',   phone:'+39 347 ...', note:null, notes:null, source:'tel'},
...
function ToolbarCalendar({ today, selected, onPick }) {
  ...
  const navBtn = (disabled) => ({
    ...
    color: disabled ? '#D1D5DB' : '#0F1115', fontSize: 14,
  ...
  return (
    <div onClick={e=>e.stopPropagation()} style={{
      ...
      <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom: 10}}>
        ...
        <span style={{fontSize: 13, fontWeight: 700, color:'#0F1115', textTransform:'capitalize'}}>{monthLabel}</span>
        <button onClick={goNext} style={navBtn(false)}>›</button>
      </div>
      <div style={{display:'grid', gridTemplateColumns:'repeat(7, 1fr)', gap: 4, marginBottom: 4}}>
        {CAL_DOW.map((d,i) => (
          <span key={i} style={{fontSize: 10, fontWeight: 700, color:'#9CA3AF', textAlign:'center', padding: 2}}>{d}</span>
        ))}
      </div>
        ...
        {resizeDrag && resizeDrag.turno?.id === turno.id && (() => {
          ...
          return (
            <div key="rg" style={{position:'absolute', left:`${gLeft}%`, width:`${gWidth}%`,
              ...
              padding:'0 7px', fontSize:10.5, fontWeight:700, color:'#059669',
                  ...
                  <span style={{overflow:'hidden', textOverflow:'ellipsis', flex:1, minWidth:0, paddingLeft:6, display:'flex', alignItems:'center', gap:4}}>
                    ...
                    {r.source === 'byup' && (
                      <span style={{
                        ...
                        <span style={{fontSize:12, fontWeight:900, color:'#fff', lineHeight:1, letterSpacing:-0.5,
                          fontFamily:'-apple-system,system-ui,sans-serif', display:'inline-block', transform:'translateY(-0.5px)',
                        }}>b</span>
                      </span>
                    )}
                  </span>
                  <span title={`${r.posti} ${r.posti===1?'coperto':'coperti'}`} style={{opacity:0.55, flexShrink:0, paddingRight:6, fontSize:14, fontWeight:800, display:'inline-flex', alignItems:'center', gap:3}}>
        ...
        <div style={{minWidth: innerW + 'px', width: innerW + 'px'}}>
          ...
          <div style={{display:'grid', gridTemplateColumns: dynGrid, gap:8,
              ...
              {(() => {
                ...
                for (let m = Math.ceil(turno.start / gridInterval) * gridInterval; m <= turno.end; m += gridInterval) {
                  ...
                  ticks.push(<span key={m} style={{position:'absolute', left,
                    top: isHour ? 2 : 8, fontSize: isHour ? 12 : 9.5,
            ...
            <div style={{display:'flex', alignItems:'center', justifyContent:'center', height:'100%'}}>
              <span style={{fontSize:9, fontWeight:600, color:'#AEB4BE', letterSpacing:1.2,
        ...
        <div onClick={() => setConflictWarning(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{display:'flex', alignItems:'center', gap:10}}>
              ...
              <div style={{fontSize:15, fontWeight:800, color:'#0F1115', letterSpacing:-0.3}}>
  ...
  return (
    ...
    >
      ...
      <div style={{flex:1, minWidth:0, display:'flex', alignItems:'center', gap:6, overflow:'hidden'}}>
        <span style={{
          fontSize:15, fontWeight: isImminente ? 700 : 500, color:'#0F1115',
        ...
        {r.source === 'byup' && (
          <span title="Prenotazione byup" style={{
            ...
            <span style={{fontSize:9, fontWeight:900, color:'#fff', lineHeight:1, letterSpacing:-0.5,
              fontFamily:'-apple-system,system-ui,sans-serif', display:'inline-block', transform:'translateY(-0.5px)',
            }}>b</span>
          </span>
        )}
        <span style={{color:'#D1D5DB', flexShrink:0, fontSize:13}}>·</span>
        <span style={{fontSize:14, fontWeight:600, color:'#6B7280', whiteSpace:'nowrap', flexShrink:0}}>Tav. {r.table}</span>
        <span style={{color:'#D1D5DB', flexShrink:0, fontSize:13}}>·</span>
        <span title={`${r.posti} ${r.posti === 1 ? 'coperto' : 'coperti'}`} style={{
          display:'inline-flex', alignItems:'center', gap:3,
          fontSize:14, fontWeight:600, color:'#6B7280', whiteSpace:'nowrap', flexShrink:0,
          cursor:'default',
        }}>
        ...
        {r.note && <span style={{color:'#D1D5DB', flexShrink:0, fontSize:13}}>·</span>}
        {r.note && <NotePill note={r.note}/>}
        {r.notes && <span style={{color:'#D1D5DB', flexShrink:0, fontSize:13}}>·</span>}
        {r.notes && <span style={{
          fontSize:13, color:'#9CA3AF', fontWeight:400,
          whiteSpace:'nowrap', flexShrink:1,
          overflow:'hidden', textOverflow:'ellipsis',
        }}>{r.notes}</span>}
      </div>

      {/* Cancella — compare su hover, azione distruttiva */}
      {hov && !dim && (
        <button
...
const NOTE_EVENT_META = {
  anniversario: { label:'Anniversario', icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg> },
  compleanno:   { label:'Compleanno',   icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-8a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8"/><path d="M4 16s.5-1 2-1 2.5 2 4 2 2.5-2 4-2 2.5 2 4 2 2-1 2-1"/><line x1="2" y1="21" x2="22" y2="21"/><line x1="7" y1="8" x2="7" y2="3"/><line x1="12" y1="8" x2="12" y2="3"/><line x1="17" y1="8" x2="17" y2="3"/></svg> },
  aziendale:    { label:'Aziendale',    icon: <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg> },
};

const ALLERGEN_COLOR = { color:'#92400E', bg:'#FEF3C7', border:'#F59E0B' };

function getAllergenDef(text) {
  const t = (text || '').toLowerCase();
  if (t.includes('glutine') || t.includes('grano') || t.includes('frumento'))
    return { label:'Glutine', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 22V9"/><path d="M12 9c0-3 3-5 3-5s1 3-3 5"/><path d="M12 9c0-3-3-5-3-5s-1 3 3 5"/>
      <path d="M12 14c0-3 3-4 3-4s1 3-3 4"/><path d="M12 14c0-3-3-4-3-4s-1 3 3 4"/>
      <path d="M12 19c0-3 3-4 3-4s1 3-3 4"/><path d="M12 19c0-3-3-4-3-4s-1 3 3 4"/>
    </svg> };
  if (t.includes('lattosio') || t.includes('latte') || t.includes('latticini') || t.includes('caseina'))
    return { label:'Lattosio', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (t.includes('noci') || t.includes('noce') || t.includes('mandorl') || t.includes('arachid') || t.includes('nocciola') || t.includes('frutta secca') || t.includes('pistacchio'))
    return { label:'Frutta secca', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 2C7 2 4 6 4 11c0 6 4 11 8 11s8-5 8-11c0-5-3-9-8-9z"/>
      <path d="M12 2c0 4-2 7-2 9"/><path d="M12 2c0 4 2 7 2 9"/>
    </svg> };
  if (t.includes('uov'))
    return { label:'Uova', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 2C8 2 5 7 5 12.5C5 18 8 22 12 22s7-4 7-9.5C19 7 16 2 12 2z"/>
    </svg> };
  if (t.includes('pesce') || t.includes('crostace') || t.includes('mollusc') || t.includes('gamber'))
    return { label:'Pesce', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M6.5 12c.94-3.46 4.94-6 10.5-6-3 3-4 7-2 9-5 0-9-1-9-1"/><path d="M18 12l-4 1"/><path d="M6.5 12L3 14"/>
      <circle cx="20" cy="10" r="2"/><path d="M6.5 12L2 10"/>
    </svg> };
  return { label:'Allergia', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
function NotePill({ note }) {
  if (note.type === 'allergia') {
    const def = getAllergenDef(note.text);
    return (
  ...
  const meta = NOTE_EVENT_META[note.type];
  if (!meta) return null;
  return (
    <span title={note.text} style={{
      display:'inline-flex', alignItems:'center', gap:4,
