                ...
                {menus.map(m => {
                  ...
                  return (
                    <div key={m.id} onClick={() => setActiveMenuId(m.id)} style={{
                      ...
                      <div style={{display:'flex', alignItems:'center', gap: 6, marginBottom: 4}}>
                        {isRenaming ? (
                          <input autoFocus value={renameVal} onChange={e => setRenameVal(e.target.value)}
                            ...
                            style={{flex:1, fontSize:13.5, fontWeight:700, border:'none', outline:`2px solid ${PN.PINK}`, borderRadius:5, padding:'2px 6px', fontFamily:'inherit', background:'transparent', color: PN.PINK_DARK}}
                          />
                        ) : (
                          <span style={{fontSize:13.5, fontWeight:700, flex:1, color: isActive ? PN.PINK_DARK : PN.TEXT}}>{m.name}</span>
                        )}
                      ...
                      <div style={{display:'flex', alignItems:'center', gap:8, fontSize:11.5, color:PN.MUTED}}>
                          ...
                          {menus.length > 1 && <>
                            ...
                            {isConfirmDel ? (
                              <div style={{padding:'6px 8px', display:'flex', gap:5}}>
                                <button onClick={() => setConfirmDelId(null)} style={{flex:1, padding:'5px 0', fontSize:11.5, fontWeight:600, background:'#F4F5F7', border:'none', borderRadius:6, cursor:'pointer', fontFamily:'inherit', color:PN.TEXT}}>Annulla</button>
                                <button onClick={() => { deleteMenu(m.id); setOpenMenuDot(null); setConfirmDelId(null); }} style={{flex:1, padding:'5px 0', fontSize:11.5, fontWeight:700, background:'#DC2626', border:'none', borderRadius:6, cursor:'pointer', fontFamily:'inherit', color:'#fff'}}>Elimina</button>
                              </div>
                            ) : (
...
function MenuDotItem({ icon, children, danger, onClick }) {
  return (
    ...
    >
      <span style={{width:16, fontSize:13}}>{icon}</span>{children}
    </button>
  );
}
          ...
          <div style={{display:'flex', background: PN.WHITE, padding:3, borderRadius:8, gap:2, border:`1px solid ${PN.BORDER}`}}>
            {[
              ...
              <button key={s.id} onClick={() => setStateFilter(s.id)} style={{
                ...
                {s.label}
                <span style={{fontSize: 10.5, padding:'1px 6px', borderRadius: 999, background: stateFilter===s.id ? 'rgba(255,255,255,0.2)' : PN.BORDER_SOFT}}>{s.count}</span>
              </button>
            ))}
          </div>
        </div>
                ...
                <div>
                  ...
                  <div style={{padding: '12px 18px', borderTop: `1px dashed ${PN.BORDER_SOFT}`, background: '#FCFCFD'}}>
                    <button onClick={() => setPicker(cat.name)} style={{
                      ...
                      color: PN.MUTED_SOFT, fontSize: 12.5, fontWeight: 600, cursor:'pointer', fontFamily:'inherit',
  ...
  return (
    <div>
      <div style={{
        ...
        <div style={{flex:1, fontSize:13, color: PN.GREEN, fontWeight:600, lineHeight:1.4}}>
      ...
      <div style={{fontSize:12.5, fontWeight:700, color:PN.MUTED, letterSpacing:0.3, textTransform:'uppercase', marginBottom:8}}>Per porzione</div>
      <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:8}}>
        {[{l:'Kcal', k:'kcal'},{l:'Carb. (g)', k:'carb'},{l:'Proteine (g)', k:'prot'},{l:'Grassi (g)', k:'fat'}].map(f => (
          <div key={f.k}>
            ...
            <div style={{fontSize:12, color:PN.MUTED, textAlign:'center', marginTop:4, fontWeight:600}}>{f.l}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{
      ...
      <button onClick={onToggle} style={{
        ...
        <div style={{flex:1, minWidth:0}}>
          <div style={{fontSize:14.5, fontWeight:700, color: PN.TEXT}}>{title}</div>
          <div style={{fontSize:13, color: PN.MUTED, marginTop:1}}>{subtitle}</div>
        </div>
      ...
      {ingredients.length > 0 && (
        <div style={{display:'flex', flexDirection:'column', gap:4, marginBottom:12}}>
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{display:'flex', alignItems:'center', gap:8, padding:'8px 10px'}}>
                  <span style={{flex:1, fontSize:13.5, fontWeight:600, color:PN.TEXT, minWidth:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                    {ing.name}
                  </span>
                  ...
                  {isEditingQty ? (
                    <div style={{display:'flex', alignItems:'center', gap:3}}>
                      <input autoFocus value={ing.qty || ''}
                        ...
                        placeholder="0"
                        style={{width:44, padding:'3px 6px', border:`1px solid ${PN.PINK}`, borderRadius:5, fontSize:13, fontFamily:'inherit', textAlign:'right', outline:'none'}}
                      />
                      <select value={ing.unit || 'g'}
                        onChange={e => setIngredients(arr => arr.map((x, idx) => idx===i ? {...x, unit: e.target.value} : x))}
                        style={{padding:'3px 4px', border:`1px solid ${PN.BORDER}`, borderRadius:5, fontSize:12.5, fontFamily:'inherit', outline:'none', background:'#fff', cursor:'pointer'}}
                      >
                  ...
                  <button onClick={() => setExpandedIdx(isExpanded ? null : i)} title="Gestisci allergeni" style={{
                    ...
                    color: PN.MUTED, fontSize:12.5, cursor:'pointer', fontFamily:'inherit',
                  }}>
                    {ingAllergens.length > 0 ? (
                      <>
                        {ingAllergens.slice(0,3).map(aid => <window.AllergenIcon key={aid} id={aid} size={13}/>)}
                        {ingAllergens.length > 3 && <span style={{fontSize:11, color:PN.MUTED, fontWeight:700}}>+{ingAllergens.length-3}</span>}
                      </>
                    ) : (
                ...
                {isExpanded && (
                  <div style={{padding:'10px 12px 12px', borderTop:`1px solid ${PN.BORDER_SOFT}`, background:'#F7F8FA'}}>
                    <div style={{fontSize:11.5, fontWeight:700, color:PN.MUTED, letterSpacing:0.4, textTransform:'uppercase', marginBottom:8}}>
                      Allergeni · {ing.name}
                    </div>
                    <div style={{display:'flex', flexWrap:'wrap', gap:5}}>
          ...
          {ingredients.some(i => i.removable) && (
            <div style={{display:'flex', alignItems:'center', gap:5, fontSize:11.5, color:PN.MUTED, paddingLeft:2}}>
      ...
      <div style={{position:'relative'}}>
        <div style={{display:'flex', alignItems:'center', gap:8, padding:'9px 12px', border:`1px solid ${PN.BORDER}`, borderRadius:8, background:'#fff'}}>
          ...
          <input value={query}
            ...
            style={{flex:1, border:'none', outline:'none', fontSize:13.5, fontFamily:'inherit', background:'transparent', color:PN.TEXT}}
          />
          {query.trim() && !exactMatch && (
            <button onMouseDown={() => addNew(query)} style={{
              background: PN.PINK, color:'#fff', border:'none',
              padding:'4px 10px', borderRadius:5, fontSize:12.5, fontWeight:700,
        ...
        {showSuggest && dbMatches.length > 0 && (
          <div style={{position:'absolute', top:'calc(100% + 4px)', left:0, right:0, background:'#fff', border:`1px solid ${PN.BORDER}`, borderRadius:8, boxShadow:'0 8px 24px rgba(0,0,0,0.08)', zIndex:10, maxHeight:240, overflowY:'auto'}}>
            <div style={{padding:'7px 12px', fontSize:11.5, fontWeight:700, color:PN.MUTED, letterSpacing:0.3, textTransform:'uppercase', borderBottom:`1px solid ${PN.BORDER_SOFT}`}}>Dal tuo archivio</div>
            {dbMatches.map(s => (
              ...
              >
                ...
                <span style={{fontSize:13.5, color:PN.TEXT, fontWeight:600, flex:1}}>{s.name}</span>
  ...
  return (
    <div>
        ...
        <div style={{display:'flex', flexDirection:'column', gap:6, marginBottom:12}}>
          {extras.map((ex, i) => (
            <div key={i} style={{display:'grid', gridTemplateColumns:'1fr auto auto', gap:10, alignItems:'center', padding:'8px 10px 8px 12px', background:'#FAFBFC', borderRadius:8}}>
              <span style={{fontSize:14, color: PN.TEXT, fontWeight:600}}>{ex.name}</span>
              <span style={{fontSize:14, fontWeight:700, color: PN.PINK_DARK}}>+ € {ex.price.toFixed(2)}</span>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 100px auto', gap:8}}>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="es. Tartufo nero" style={{padding:'8px 12px', border:`1px solid ${PN.BORDER}`, borderRadius:8, fontSize:14, fontFamily:'inherit', outline:'none'}}/>
        <div style={{position:'relative'}}>
          <span style={{position:'absolute', left:10, top:'50%', transform:'translateY(-50%)', fontSize:14, color: PN.MUTED, fontWeight:600}}>+€</span>
          <input value={price} onChange={e => setPrice(e.target.value)} placeholder="4,00" style={{width:'100%', padding:'8px 12px 8px 30px', border:`1px solid ${PN.BORDER}`, borderRadius:8, fontSize:14, fontFamily:'inherit', outline:'none'}}/>
        </div>
        <button onClick={add} style={{background: name.trim() ? PN.PINK : '#F4F5F7', color: name.trim() ? '#fff' : PN.MUTED, border:'none', padding:'0 14px', borderRadius:8, fontSize:14, fontWeight:700, cursor: name.trim()?'pointer':'default', fontFamily:'inherit'}}>Aggiungi</button>
      </div>
    </div>
  );
}
...
function VariantsList({ variants, setVariants }) {
  ...
  return (
    <div>
        ...
        {variants.map((v, i) => (
          <div key={i} style={{border:`1px solid ${PN.BORDER}`, borderRadius:8, padding:12}}>
            <div style={{display:'grid', gridTemplateColumns:'1fr auto', gap:10, alignItems:'center', marginBottom:10}}>
              <input value={v.name} onChange={e => updateGroup(i, {name:e.target.value})} placeholder="Nome del gruppo (es. Cottura, Formato, Pane)" style={{padding:'7px 10px', border:`1px solid ${PN.BORDER}`, borderRadius:6, fontSize:14, fontFamily:'inherit', outline:'none', fontWeight:600}}/>
            ...
            <div style={{display:'flex', flexDirection:'column', gap:5, marginBottom:8}}>
              {v.options.map((opt, oi) => (
                <div key={oi} style={{display:'flex', gap:6}}>
                  <input value={opt} onChange={e => updateGroup(i, {options: v.options.map((x, idx) => idx===oi ? e.target.value : x)})} placeholder={`Opzione ${oi+1} (es. Al sangue)`} style={{flex:1, padding:'6px 10px', border:`1px solid ${PN.BORDER}`, borderRadius:6, fontSize:13.5, fontFamily:'inherit', outline:'none'}}/>
                  {v.options.length > 1 && (
              ...
              <button onClick={() => updateGroup(i, {options: [...v.options, '']})} style={{background:'transparent', border:'none', color: PN.PINK_DARK, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:5, padding:'4px 0', alignSelf:'flex-start'}}>+ Aggiungi opzione</button>
            </div>
            <label style={{display:'flex', alignItems:'center', gap:7, fontSize:13, color: PN.MUTED, fontWeight:600, cursor:'pointer', paddingTop:8, borderTop:`1px solid ${PN.BORDER_SOFT}`}}>
      ...
      <button onClick={addGroup} style={{marginTop: variants.length ? 10 : 0, width:'100%', padding:'10px', background:'transparent', border:`1.5px dashed ${PN.BORDER}`, borderRadius:8, color: PN.MUTED, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:6, justifyContent:'center'}}>+ Aggiungi gruppo di varianti</button>
    </div>
  );
}
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{padding:'16px 22px', borderBottom:`1px solid ${PN.BORDER_SOFT}`, display:'flex', alignItems:'center', gap:12}}>
          <div style={{flex:1, minWidth:0}}>
            <div style={{fontSize:12, color:PN.MUTED, textTransform:'uppercase', letterSpacing:0.5, fontWeight:700, marginBottom:2}}>
              {fromLibrary ? 'Libreria piatti' : (catName || cat)}
            </div>
            <div style={{fontSize:18, fontWeight:800, color:PN.TEXT, lineHeight:1.2}}>
        ...
        <div style={{padding:'0', display:'flex', flexDirection:'column', overflowY:'auto', flex:1}}>
          ...
          <div style={{padding:'20px 22px 0', display:'flex', flexDirection:'column', gap:12}}>
            <div style={{display:'flex', flexDirection:'column', gap:10}}>
                ...
                <div style={{flex:1}}>
                  <ImpField label="Nome">
                    <input autoFocus value={name} onChange={e=>setName(e.target.value)} placeholder="es. Spaghetti alle vongole" style={{
                      width:'100%', padding:'10px 12px', border:`1px solid ${PN.BORDER}`, borderRadius:8, fontSize:14.5, fontFamily:'inherit', outline:'none',
                    }}/>
                  </ImpField>
                </div>
                <div style={{marginBottom:16}}>
                  <div aria-hidden="true" style={{fontSize:13, fontWeight:600, marginBottom:6, lineHeight:1.4, color:'transparent', userSelect:'none', pointerEvents:'none'}}>·</div>
                  <label onClick={() => setNoPrep(v => !v)} style={{
                    ...
                    <div style={{
                      ...
                      {noPrep && <span style={{color:'#fff', fontSize:10, lineHeight:1}}>✓</span>}
                    </div>
                    <span style={{fontSize:12.5, color: noPrep ? '#1E293B' : '#64748B', fontWeight:500, whiteSpace:'nowrap'}}>
                      Prodotto finito
                    </span>
                    ...
                    >
                      <span style={{width:15, height:15, borderRadius:'50%', background:'#E2E8F0', color:'#64748B', fontSize:10, fontWeight:700, display:'inline-grid', placeItems:'center', cursor:'help'}}>i</span>
                      <span className="np-tip" style={{
                        display:'none', position:'fixed',
                        transform:'translateX(-50%)', width:220,
                        background:'#1E293B', color:'#F8FAFC', fontSize:11.5, lineHeight:1.5,
              ...
              <ImpField label="Descrizione breve">
                <textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={2} placeholder="Ingredienti principali, breve descrizione…" style={{
                  width:'100%', padding:'10px 12px', border:`1px solid ${PN.BORDER}`, borderRadius:8, fontSize:14, fontFamily:'inherit', outline:'none', resize:'none', lineHeight:1.5,
                }}/>
              </ImpField>
            </div>
            ...
            <div>
              <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:7}}>
                <span style={{fontSize:12, fontWeight:700, color:PN.MUTED, letterSpacing:0.4, textTransform:'uppercase'}}>Foto</span>
                <span style={{fontSize:12, color:PN.MUTED}}>{photos.length}/3</span>
              </div>
              <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:10}}>
                ...
                {photos.length < 3 && (
                  ...
                  >
                    ...
                    <span style={{fontSize:11.5, color:PN.MUTED, fontWeight:600}}>JPG / PNG</span>
                  </button>
                )}
              </div>
            </div>
              ...
              {!fromLibrary && (
                <ImpField label="Categoria" style={{flex:'0 0 160px'}}>
                  <select value={cat} onChange={e=>setCat(e.target.value)} style={{
                    width:'100%', padding:'9px 12px', border:`1px solid ${PN.BORDER}`, borderRadius:8, fontSize:14, fontFamily:'inherit', outline:'none', background:PN.WHITE,
                  }}>
              ...
              {!fromLibrary && catName && (
                <ImpField label={`Prezzo in "${catName}" (€, IVA escl.)`} style={{flex:'0 0 140px'}}>
                  <input value={initialPrice} onChange={e=>setInitialPrice(e.target.value)} placeholder="0,00" style={{
                    width:'100%', padding:'9px 12px', border:`1px solid ${PN.BORDER}`, borderRadius:8, fontSize:14, fontFamily:'inherit', outline:'none',
                  }}/>
                </ImpField>
              )}
            </div>
            ...
            {fromLibrary && (
              <div style={{padding:'9px 12px', borderRadius:8, background:'#F8FAFC', border:`1px solid ${PN.BORDER_SOFT}`, fontSize:13.5, color:PN.MUTED, lineHeight:1.5}}>
          ...
          <div style={{padding:'16px 22px 0', display:'flex', flexDirection:'column', gap:14}}>
            <div style={{fontSize:12, fontWeight:800, color:PN.MUTED, letterSpacing:0.8, textTransform:'uppercase'}}>Per il cliente</div>

            {/* Allergeni */}
            <div>
              <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8}}>
                <span style={{fontSize:13.5, fontWeight:700, color:PN.TEXT}}>Allergeni</span>
                {ingredientAllergens.size > 0 && (
                  <span style={{fontSize:12, color:PN.MUTED, fontStyle:'italic'}}>
                ...
                {ALLERGENS.map(a => {
                  ...
                  return (
                    <button key={a.id} onClick={() => toggleAllergen(a.id)} disabled={fromIng && !fromManual} title={fromIng && !fromManual ? 'Derivato dagli ingredienti' : ''} style={{
                      ...
                      color: on ? PN.PINK_DARK : PN.MUTED,
                      fontSize:13, fontWeight:600, cursor:(fromIng && !fromManual) ? 'not-allowed' : 'pointer', fontFamily:'inherit',
                    }}>
                      <AllergenIcon id={a.id} size={14}/>
                      {a.name}
                      {fromIng && <span style={{color:PN.PINK_DARK, fontSize:12}}>•</span>}
                    </button>
                  );
                })}
              </div>
            ...
            <div>
              <div style={{fontSize:13.5, fontWeight:700, color:PN.TEXT, marginBottom:8}}>Surgelati</div>
              <label onClick={() => setHasFrozen(v => !v)} style={{
                ...
                <div style={{
                  ...
                  {hasFrozen && <span style={{color:'#fff', fontSize:10, lineHeight:1}}>✓</span>}
                </div>
                <span style={{fontSize:13, color: hasFrozen ? '#1E3A8A' : PN.TEXT, fontWeight:600}}>
            ...
            <div>
              <div style={{fontSize:13.5, fontWeight:700, color:PN.TEXT, marginBottom:8}}>Disponibile anche in versione</div>
              ...
              {dietaryTags.length > 0 && (
                <div style={{background:'#F8FAFC', borderRadius:10, padding:'10px 12px', border:`1px solid ${PN.BORDER_SOFT}`}}>
                  <div style={{fontSize:10.5, fontWeight:700, color:PN.MUTED, letterSpacing:0.3, textTransform:'uppercase', marginBottom:8}}>Sovrapprezzo per versione (opzionale)</div>
                  <div style={{display:'flex', flexDirection:'column', gap:5}}>
                    {dietaryTags.map(t => (
                      <div key={t.name} style={{display:'grid', gridTemplateColumns:'1fr auto', gap:10, alignItems:'center', padding:'5px 10px', background:'#fff', border:`1px solid ${PN.BORDER_SOFT}`, borderRadius:7}}>
                        <span style={{fontSize:12.5, color:PN.TEXT, fontWeight:600}}>{t.name}</span>
                        <div style={{display:'flex', alignItems:'center', gap:5}}>
                          ...
                          <input value={t.surcharge} onChange={e => setTagSurcharge(t.name, e.target.value.replace(/[^0-9,.]/g,''))} placeholder="0,00"
                            style={{width:60, padding:'4px 8px', border:`1px solid ${PN.BORDER}`, borderRadius:6, fontSize:13.5, fontFamily:'inherit', textAlign:'right', outline:'none'}}/>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div style={{padding:'16px 22px 0', display:'flex', flexDirection:'column', gap:8}}>
            <div style={{fontSize:12, fontWeight:800, color:PN.MUTED, letterSpacing:0.8, textTransform:'uppercase', marginBottom:4}}>Personalizzazioni cliente</div>
            <CollapseSection
              title="Ingredienti"
            ...
            >
              <div style={{fontSize:13, color:PN.MUTED, marginBottom:10, lineHeight:1.45}}>
          ...
          <div style={{padding:'16px 22px 20px', display:'flex', flexDirection:'column', gap:16}}>
            <div style={{fontSize:12, fontWeight:800, color:PN.MUTED, letterSpacing:0.8, textTransform:'uppercase'}}>Cucina &amp; gestione interna</div>
            ...
            <div style={{
              ...
              <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:8}}>
                <div style={{fontSize:13.5, fontWeight:700, color:PN.TEXT}}>Ricetta — procedimento</div>
                {noPrep && (
                  <span style={{fontSize:11.5, color:PN.MUTED, fontStyle:'italic', fontWeight:500}}>
                ...
                {recipeSteps.map((step, i) => (
                  <div key={i} style={{display:'flex', alignItems:'flex-start', gap:8}}>
                    <span style={{
                      ...
                      background:'#F1F5F9', color:'#64748B', fontSize:12.5, fontWeight:700,
                    ...
                    <textarea value={step} disabled={noPrep}
                      ...
                      style={{flex:1, padding:'7px 11px', border:`1px solid ${PN.BORDER}`, borderRadius:8, fontSize:14, fontFamily:'inherit', outline:'none', resize:'none', lineHeight:1.5, overflow:'hidden', background: step ? PN.WHITE : '#FAFBFC'}}
                      onInput={e => { e.target.style.height='auto'; e.target.style.height=e.target.scrollHeight+'px'; }}
                    />
                    {recipeSteps.length > 1 && (
                      <button onClick={() => setRecipeSteps(s => s.filter((_,idx)=>idx!==i))} disabled={noPrep}
                        style={{flexShrink:0, width:22, height:22, marginTop:8, background:'transparent', border:'none', cursor: noPrep ? 'default' : 'pointer', color:PN.MUTED, fontSize:14, display:'grid', placeItems:'center', borderRadius:4}}
              ...
              <button onClick={() => setRecipeSteps(s => [...s,''])} disabled={noPrep} style={{
                ...
                background:'transparent', color:PN.MUTED, fontSize:13, fontWeight:600, cursor: noPrep ? 'default' : 'pointer', fontFamily:'inherit',
              }}
            ...
            <ImpField label="Food cost (opzionale)" style={{maxWidth:200}}>
              ...
              <div style={{fontSize:12, color:PN.MUTED, marginTop:3}}>Per le analytics di marginalità</div>
            </ImpField>

            {/* Valori nutrizionali */}
            <div>
              <div style={{fontSize:13.5, fontWeight:700, color:PN.TEXT, marginBottom:8}}>Valori nutrizionali</div>
              <NutritionFields/>
              <div style={{fontSize:12.5, color:PN.MUTED, marginTop:7}}>I valori nutrizionali verranno mostrati sul menù.</div>
            </div>
          ...
          <div>
            {isEdit && onDelete && (
              <button onClick={() => { if (confirm('Eliminare questo piatto dalla libreria? Sarà rimosso anche da tutti i menù.')) onDelete(); }} style={{
                background:'transparent', border:'none', color:PN.RED, fontSize:12.5, fontWeight:600, cursor:'pointer', fontFamily:'inherit',
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            display:'flex', alignItems:'center', gap: 8,
            background:'transparent', border:'none', padding: 0, cursor:'pointer',
            color: PN.TEXT, fontFamily:'inherit', fontSize: 13, fontWeight: 600,
          }}
        >
          <span style={{
        ...
        {advOpen && (
          <div style={{
            ...
            <div style={{fontSize: 11.5, color: PN.MUTED, lineHeight: 1.5}}>
            ...
            {BUCKETS.map(b => {
              ...
              return (
                <div key={b.key} style={{
                  ...
                  <div style={{minWidth: 100, fontSize: 12.5, fontWeight: 700, color: PN.TEXT, display:'flex', alignItems:'center', gap: 6}}>
                    {b.label}
      ...
      >
        {!modules.sala ? (
          <div style={{
            ...
            <div style={{fontSize: 13.5, fontWeight: 700, marginBottom: 4}}>Sala disattivata</div>
        ...
        <div>
          <div style={{fontSize: 13, fontWeight: 700, marginBottom: 4, display:'inline-flex', alignItems:'center', gap: 7}}>
      ...
      >
        {!modules.prenotazioni ? (
          <div style={{
            ...
            <div style={{fontSize: 13.5, fontWeight: 700, marginBottom: 4}}>Prenotazioni disattivate</div>
            <div style={{fontSize: 12, color: PN.MUTED, marginBottom: 14, maxWidth: 360, margin:'0 auto 14px'}}>
