...
const NP_FS = { xs:11, sm:12, md:13, base:14, lg:16, xl:22 };
        ...
        {visibleDays.map(d => {
          ...
          return (
            <button key={iso} onClick={()=>onChange(iso)} style={{
              ...
              <span style={{fontSize: 16, fontWeight: 800, lineHeight: 1.1}}>{d.getDate()}</span>
              {isToday && (
                <span style={{
    ...
    <div onClick={e=>e.stopPropagation()} style={{
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(7, 1fr)', gap: 4, marginBottom: 4}}>
        {['L','M','M','G','V','S','D'].map((d,i)=>(
          <span key={i} style={{
            fontSize: 10, fontWeight: 700, color: NP_T.textSubtle,
          ...
          {chartData.map((f) => {
            ...
            return (
              <button key={f.label}
                ...
                <span style={{fontSize: 18, fontWeight: 800, color: c.num, lineHeight: 1}}>
                  {isFull ? '—' : free}
                </span>
                <span style={{fontSize: 8, fontWeight: 700, color: c.num, opacity: 0.85, lineHeight: 1,
                  textTransform:'uppercase', letterSpacing: 0.4}}>
                  {isFull ? 'pieno' : 'liberi'}
                </span>
                <span style={{fontSize: 11, fontWeight: isSel ? 800 : 600,
