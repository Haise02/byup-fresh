    ...
    <div style={{
      ...
      <div onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{padding:'18px 22px', borderBottom:`1px solid ${ADM.BORDER}`, display:'flex', alignItems:'center', gap:12}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:17.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.01em'}}>Invia messaggio</div>
          ...
          <div style={{padding:'22px 26px', borderRight:`1px solid ${ADM.BORDER}`}}>
            {step === 1 && (
              <>
                ...
                {audience.presetIds.length > 0 && (
                  <div style={{padding:'12px 14px', background:ADM.INFO_SOFT, border:`1px solid #BFDBFE`, borderRadius:8, marginBottom:18, fontSize:14.5, color:'#1E40AF', display:'flex', alignItems:'center', gap:8}}>
                    <BuIcons.filter size={13}/>
                    ...
                    <button onClick={()=>setAudience({...audience, presetIds:[]})} style={{marginLeft:'auto', background:'none', border:'none', color:ADM.INFO, fontSize:14, fontWeight:600, cursor:'pointer', textDecoration:'underline'}}>Rimuovi</button>
                  </div>
                )}
                ...
                <div style={{padding:'16px 18px', background:`linear-gradient(135deg, ${ADM.PINK_BG_SOFT}, #fff)`, border:`1px solid ${ADM.BORDER}`, borderRadius:10}}>
                  <div style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:4}}>Destinatari stimati</div>
                  <div style={{fontSize:34, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.02em', display:'flex', alignItems:'baseline', gap:8}}>
                    {fmtNum(total)}
                    <span style={{fontSize:15, fontWeight:500, color:ADM.MUTED}}>{audience.type === 'utenti' ? 'utenti' : 'locali'}</span>
                  </div>
                  <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:4}}>
            ...
            {step === 3 && (
              <>
                ...
                {channels.sms && (
                  <div style={{marginTop:14, padding:'12px 14px', background:ADM.WARN_SOFT, border:`1px solid #FCD34D`, borderRadius:8, fontSize:14.5, color:'#92400E', display:'flex', gap:10, alignItems:'flex-start'}}>
                    <BuIcons.alertTriangle size={14} color={ADM.WARN}/>
        ...
        <div style={{padding:'14px 22px', borderTop:`1px solid ${ADM.BORDER}`, display:'flex', alignItems:'center', gap:10, background:'#fff'}}>
          <div style={{fontSize:14, color:ADM.MUTED}}>Bozza salvata automaticamente</div>
          <div style={{flex:1}}/>
...
function AudienceCard({ icon, label, desc, selected, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div>
        <div style={{fontSize:15.5, fontWeight:600, color:ADM.TEXT}}>{label}</div>
        <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2, lineHeight:1.4}}>{desc}</div>
      </div>
    </button>
  );
}
...
function ChannelToggle({ icon, label, desc, active, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{flex:1}}>
        <div style={{fontSize:14.5, fontWeight:600, color:ADM.TEXT}}>{label}</div>
      ...
      }}>{active && <BuIcons.check size={11}/>}</div>
    </button>
  );
}
...
function ReviewRow({ label, value, last }) {
  return (
    <div style={{display:'flex', padding:'10px 0', borderBottom: last ? 'none' : `1px solid ${ADM.BORDER_SOFT}`}}>
      <div style={{width:140, fontSize:14, color:ADM.MUTED, fontWeight:500}}>{label}</div>
      <div style={{flex:1, fontSize:14.5, color:ADM.TEXT, fontWeight:500}}>{value}</div>
    </div>
  );
}
    ...
    <>
        ...
        <PreviewCard kind="push">
          <div style={{
            ...
            <div style={{flex:1, minWidth:0}}>
              ...
              <div style={{fontSize:13.5, fontWeight:700, color:'#000', lineHeight:1.3}}>{renderedTitle}</div>
              <div style={{fontSize:12.5, color:'#444', marginTop:2, lineHeight:1.3, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden'}}>{renderedBody}</div>
            </div>
          </div>
        </PreviewCard>
      )}
      {channels.email && (
        <PreviewCard kind="email">
          <div style={{background:'#fff', border:`1px solid ${ADM.BORDER}`, borderRadius:8, overflow:'hidden'}}>
            <div style={{padding:'10px 14px', borderBottom:`1px solid ${ADM.BORDER_SOFT}`, fontSize:12, color:ADM.MUTED}}>
              <div><strong>Da:</strong> byup &lt;hello@byup.it&gt;</div>
              <div style={{marginTop:2}}><strong>Oggetto:</strong> {renderedTitle}</div>
            </div>
            <div style={{padding:'16px', fontSize:13.5, color:ADM.TEXT, lineHeight:1.5}}>
              <div style={{fontSize:15, fontWeight:700, marginBottom:8}}>Ciao {sampleName},</div>
      ...
      {channels.in_app && (
        <PreviewCard kind="in_app">
          <div style={{
            ...
            <div style={{fontSize:11, fontWeight:700, opacity:0.8, textTransform:'uppercase', letterSpacing:'0.06em'}}>Novità</div>
            <div style={{fontSize:15, fontWeight:700, marginTop:4, lineHeight:1.3}}>{renderedTitle}</div>
      ...
      {channels.sms && (
        <PreviewCard kind="sms">
          <div style={{background:'#E5F0FF', padding:'10px 12px', borderRadius:12, borderTopLeftRadius:4, fontSize:13.5, color:'#000', lineHeight:1.4}}>
      ...
      {!channels.push && !channels.email && !channels.in_app && !channels.sms && (
        <div style={{padding:'40px 20px', textAlign:'center', color:ADM.MUTED_SOFT, fontSize:14}}>
...
function PreviewCard({ kind, children }) {
  ...
  return (
    <div>
      <div style={{fontSize:12, color:ADM.MUTED_SOFT, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:6}}>{labels[kind]}</div>
      {children}
    </div>
  );
}
