        ...
        <div style={{padding:'20px 24px', borderBottom:`1px solid ${ADM.BORDER}`, display:'flex', alignItems:'center', gap:14}}>
          ...
          <div style={{flex:1, minWidth:0}}>
            <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:3}}>
              <div style={{fontSize:20, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.01em'}}>{l.nome}</div>
              <AdmPlanBadge piano={l.piano}/>
              <AdmStatoBadge stato={l.stato}/>
            </div>
            <div style={{fontSize:14, color:ADM.MUTED, display:'flex', gap:10}}>
    ...
    <div style={{padding:'20px 24px', display:'flex', flexDirection:'column', gap:16}}>
      ...
      {l.stato === 'onboarding' && stoppedStep && (
        <div style={{padding:'14px 16px', background: ADM.WARN_SOFT, border:`1px solid #FCD34D`, borderRadius:10, display:'flex', gap:12, alignItems:'flex-start'}}>
          <div style={{color: ADM.WARN, marginTop:1}}><BuIcons.pause size={18}/></div>
          <div style={{flex:1}}>
            <div style={{fontSize:15, fontWeight:700, color:ADM.TEXT}}>Fermo da {fmtRelative(l.lastLogin)} su "{stoppedStep.label}"</div>
            <div style={{fontSize:14, color:'#92400E', marginTop:3}}>Considera di inviare un reminder o assistenza dedicata per sbloccarlo.</div>
          </div>
      ...
      {l.stato === 'skipped' && (
        <div style={{padding:'14px 16px', background: ADM.INFO_SOFT, border:`1px solid #BFDBFE`, borderRadius:10, display:'flex', gap:12, alignItems:'flex-start'}}>
          <div style={{color: ADM.INFO, marginTop:1}}><BuIcons.info size={18}/></div>
          <div style={{flex:1}}>
            <div style={{fontSize:15, fontWeight:700, color:ADM.TEXT}}>Onboarding saltato</div>
            <div style={{fontSize:14, color:'#1E40AF', marginTop:3}}>Il locale è andato direttamente in Panoramica senza completare il setup guidato.</div>
          </div>
        </div>
      )}
      ...
      <AdmCard padding={18}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:12}}>Andamento ordini · 14 giorni</div>
...
function MiniStat({ label, value, sub }) {
  return (
    <AdmCard padding={14}>
      <div style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.04em'}}>{label}</div>
      <div style={{fontSize:22, fontWeight:700, color:ADM.TEXT, marginTop:4, letterSpacing:'-0.01em'}}>{value}</div>
...
function DrwAdozioneDigitale({ locale: l }) {
  ...
  if (qr == null) {
    return (
      <AdmCard padding={18}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:6}}>Adozione digitale</div>
        <div style={{fontSize:14, color:ADM.MUTED, lineHeight:1.5}}>
  ...
  return (
    <AdmCard padding={18}>
      <div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:12, gap:12}}>
        <div>
          <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT}}>Adozione digitale</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>% di ordini/coperti che passano dal QR byup · ultimi 30 giorni</div>
        </div>
        <span style={{
      ...
      <div style={{display:'flex', alignItems:'baseline', gap:10, marginBottom:12}}>
        <div style={{fontSize:36, fontWeight:800, color: band.color, letterSpacing:'-0.02em', lineHeight:1}}>
          {qr === 0 ? '0%' : `${qr.toFixed(1)}%`}
        </div>
        <div style={{fontSize:14, color:ADM.MUTED, lineHeight:1.4}}>fascia <strong style={{color:ADM.TEXT}}>{band.range}</strong></div>
      </div>
      ...
      <div style={{display:'flex', justifyContent:'space-between', fontSize:12, color:ADM.MUTED_SOFT, fontFamily:'ui-monospace, monospace'}}>
      ...
      <div style={{
        ...
        fontSize:14, color:ADM.TEXT, lineHeight:1.5,
      }}>
...
function DrwScanOrdini({ locale: l }) {
  ...
  if (l.qrAdoption == null) {
    return (
      <AdmCard padding={18}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:6}}>Ordini · Scan QR</div>
        <div style={{fontSize:14, color:ADM.MUTED, lineHeight:1.5}}>
  ...
  return (
    <AdmCard padding={18}>
      <div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:12, gap:12}}>
        <div style={{minWidth:0}}>
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT}}>Ordini · Scan QR</div>
            <span
              ...
              {hoverInfo && (
                <span style={{
                  ...
                  <div style={{display:'flex', justifyContent:'space-between', fontSize:13.5, color:ADM.TEXT, marginBottom:3}}>
                    <span style={{color:ADM.MUTED}}>Ordini</span>
                    <span style={{fontWeight:700}}>{fmtNum(opp.ord)}</span>
                  </div>
                  <div style={{display:'flex', justifyContent:'space-between', fontSize:13.5, color:ADM.TEXT, marginBottom:6}}>
                    <span style={{color:ADM.MUTED}}>Scan QR</span>
                    <span style={{fontWeight:700}}>{fmtNum(opp.scan)}</span>
                  </div>
                  <div style={{borderTop:`1px solid ${ADM.BORDER_SOFT}`, paddingTop:6, display:'flex', justifyContent:'space-between', fontSize:14.5, color:ADM.TEXT, fontWeight:700}}>
          ...
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>
      ...
      <div style={{
        ...
        <div style={{fontSize:14, color: aboveAvg ? '#065F46' : '#7F1D1D', lineHeight:1.4, minWidth:0}}>
...
function DrwMetricBox({ label, value, tone }) {
  return (
    <div style={{padding:'10px 12px', background:ADM.PANEL_SOFT, border:`1px solid ${ADM.BORDER_SOFT}`, borderRadius:8}}>
      ...
      <div style={{fontSize:20, fontWeight:800, color: tone || ADM.TEXT, letterSpacing:'-0.02em', lineHeight:1.1, fontFamily:'ui-monospace, monospace'}}>{value}</div>
    </div>
  );
}
  ...
  return (
    <AdmCard padding={18}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14}}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT}}>Funnel onboarding</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>{l.completedSteps.length} di {ONB_STEPS.length} step completati</div>
      </div>
          ...
          <React.Fragment key={s.id}>
            <div style={{display:'flex', flexDirection:'column', alignItems:'center', gap:6, minWidth:0, flex: variant==='compact' ? 1 : 'none'}}>
              <div style={{
                ...
                {done ? <BuIcons.check size={14}/> : stuck ? <BuIcons.clock size={14}/> : i+1}
              </div>
              <div style={{textAlign:'center', minWidth:0}}>
                ...
                {time && variant !== 'compact' && (
                  <div style={{fontSize:12, color:ADM.MUTED_SOFT, marginTop:1}}>{fmtDate(time)}</div>
                )}
                {stuck && variant !== 'compact' && (
                  <div style={{fontSize:12, color:ADM.WARN, marginTop:1, fontWeight:600}}>Fermo qui</div>
                )}
              </div>
            </div>
    ...
    <div style={{padding:'20px 24px', display:'flex', flexDirection:'column', gap:16}}>
      <AdmCard padding={24}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24}}>
          <div>
            <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT}}>Stato del funnel di onboarding</div>
            <div style={{fontSize:14, color:ADM.MUTED, marginTop:2}}>Timeline da iscrizione a Go-live</div>
          </div>
      ...
      {/* Timeline verticale */}
      <AdmCard padding={20}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Cronologia step</div>
          ...
          {ONB_STEPS.map((s, i) => {
            ...
            return (
              <div key={s.id} style={{display:'flex', gap:14, paddingBottom: i === ONB_STEPS.length-1 ? 0 : 18, position:'relative'}}>
                ...
                }}>{done ? <BuIcons.check size={12}/> : i+1}</div>
                <div style={{flex:1, paddingTop:1}}>
                  <div style={{fontSize:15.5, fontWeight:600, color:ADM.TEXT}}>{s.label}</div>
                  <div style={{fontSize:13.5, color: stuck ? ADM.WARN : ADM.MUTED, marginTop:2, fontWeight: stuck ? 600 : 400}}>
...
function DrwAnagrafica({ locale: l }) {
  return (
    <div style={{padding:'20px 24px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:14}}>
      <AdmCard padding={20}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Anagrafica locale</div>
      ...
      <AdmCard padding={20}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Dati fiscali</div>
      ...
      <AdmCard padding={20} style={{gridColumn:'span 2'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14}}>
          <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT}}>Certificazioni</div>
        ...
        <div style={{display:'flex', flexDirection:'column', gap:8}}>
          {[
            ...
            <div key={i} style={{display:'flex', alignItems:'center', gap:12, padding:'10px 12px', background:ADM.PANEL_SOFT, borderRadius:8, border:`1px solid ${ADM.BORDER_SOFT}`}}>
              <BuIcons.filePdf size={16} color={ADM.MUTED}/>
              <div style={{flex:1, fontSize:14.5, color:ADM.TEXT, fontWeight:500}}>{c.tipo}</div>
              <div style={{fontSize:13.5, color:ADM.MUTED}}>{c.data}</div>
...
function DataRow({ label, value, mono, last }) {
  return (
    <div style={{display:'flex', padding:'10px 0', borderBottom: last ? 'none' : `1px solid ${ADM.BORDER_SOFT}`}}>
      <div style={{width:140, fontSize:13.5, color:ADM.MUTED, fontWeight:500}}>{label}</div>
      <div style={{flex:1, fontSize:14.5, color:ADM.TEXT, fontWeight:500, fontFamily: mono ? 'ui-monospace, monospace' : 'inherit'}}>{value}</div>
    </div>
  );
}
...
function DrwAttivita({ locale: l }) {
  ...
  return (
    <div style={{padding:'20px 24px'}}>
      <AdmCard padding={20}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Log attività</div>
        <div style={{display:'flex', flexDirection:'column'}}>
          {events.map((e, i) => {
            ...
            return (
              <div key={i} style={{display:'flex', gap:12, alignItems:'center', padding:'10px 0', borderBottom: i === events.length-1 ? 'none' : `1px solid ${ADM.BORDER_SOFT}`}}>
                ...
                <div style={{flex:1, fontSize:14.5, color:ADM.TEXT}}>{e.text}</div>
...
function DrwFatturazione({ locale: l }) {
  ...
  return (
    <div style={{padding:'20px 24px', display:'flex', flexDirection:'column', gap:14}}>
      <AdmCard padding={20}>
        <div style={{display:'flex', alignItems:'center', gap:16}}>
          <div style={{
            ...
            display:'grid', placeItems:'center',
          }}><BuIcons.crown size={22}/></div>
          <div style={{flex:1}}>
            <div style={{fontSize:13, color:ADM.MUTED, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em'}}>Piano attivo</div>
            <div style={{fontSize:22, fontWeight:700, color:ADM.TEXT, marginTop:2}}>{piano.label} · {fmtEur(piano.price)}/mese</div>
            {l.extras > 0 && <div style={{fontSize:14, color:ADM.PURPLE, marginTop:3, fontWeight:600}}>+ {fmtEur(l.extras)}/mese in extra</div>}
          </div>
      ...
      <AdmCard padding={20}>
        <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Ultime fatture</div>
        <div>
          {fatture.map((f, i) => (
            <div key={i} style={{display:'flex', alignItems:'center', gap:12, padding:'12px 0', borderBottom: i === fatture.length-1 ? 'none' : `1px solid ${ADM.BORDER_SOFT}`}}>
              <BuIcons.receipt size={15} color={ADM.MUTED}/>
              <div style={{flex:1}}>
                <div style={{fontSize:14.5, fontWeight:600, color:ADM.TEXT}}>Fattura #{f.num}</div>
                <div style={{fontSize:13.5, color:ADM.MUTED}}>{fmtDate(f.data)}</div>
              </div>
              <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT}}>{fmtEur(f.importo)}</div>
