...
//
// L'SLA non è uguale per tutti: un locale fermo (non incassa) ha 1 ora, un
// problema che blocca il servizio 2, il resto la giornata lavorativa. La
// priorità è quindi un attributo della categoria, non un campo che l'operatore
// mette a mano — così non è negoziabile sotto pressione.
const SRV_CATEGORIE = {
  blocco:      { label: 'Locale fermo',        slaMin: 60,  color: 'DANGER' },
  servizio:    { label: 'Servizio degradato',  slaMin: 120, color: 'WARN'   },
  contabilita: { label: 'Contabilità e fisco', slaMin: 480, color: 'INFO'   },
  configurazione: { label: 'Configurazione',   slaMin: 480, color: 'TEAL'   },
  commerciale: { label: 'Commerciale',         slaMin: 480, color: 'PURPLE' },
};
...
const RICHIAMATE = (() => {
  const grezze = [
    ...
    { id:'RC-236', localeId:'L1034', cat:'blocco',      da:2.8*SRV_ORA,
    ...
    { id:'RC-228', localeId:'L1023', cat:'servizio',    da:1.5*SRV_GIORNO, fattaDopo:3*SRV_ORA,  op:'support3', durata:7,  voto:2,
    ...
    { id:'RC-225', localeId:'L1003', cat:'contabilita', da:2.6*SRV_GIORNO, fattaDopo:6.8*SRV_ORA, op:'support3', durata:12, voto:3,
    ...
    { id:'RC-218', localeId:'L1042', cat:'blocco',      da:5.5*SRV_GIORNO, fattaDopo:1.8*SRV_ORA, op:'support3', durata:15, voto:1,
  ...
  return grezze.map(g => {
    const locale = LOCALI.find(l => l.id === g.localeId);
    const catDef = SRV_CATEGORIE[g.cat];
    const prenotataIl = new Date(Date.now() - g.da);
    const entro = new Date(prenotataIl.getTime() + catDef.slaMin * SRV_MIN);
