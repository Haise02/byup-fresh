...
function AdmStatistichePage() {
  ...
  return (
    <div style={{padding:28, display:'flex', flexDirection:'column', gap:18}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'2fr 1fr', gap:14}}>
        <AdmCard padding={22}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:18}}>
            <div>
              <div style={{fontSize:21, fontWeight:600, color:ADM.TEXT}}>Ordini & Prenotazioni · ultimi 30 giorni</div>
              <div style={{fontSize:19, color:ADM.MUTED, marginTop:2}}>Aggregato di tutti i locali</div>
            </div>
        ...
        <AdmCard padding={22}>
          <div style={{fontSize:21, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Distribuzione oraria</div>
          <div style={{fontSize:18.5, color:ADM.MUTED, marginBottom:12}}>Ordini per fascia oraria</div>
          <div style={{display:'flex', flexDirection:'column', gap:7}}>
            {[
              ...
              <div key={s.h} style={{display:'flex', alignItems:'center', gap:10}}>
                <div style={{width:48, fontSize:18, color:ADM.MUTED, fontFamily:'ui-monospace,monospace'}}>{s.h}</div>
                ...
                <div style={{width:40, fontSize:18, color:ADM.TEXT, fontWeight:600, textAlign:'right'}}>{s.v}%</div>
              </div>
            ))}
          </div>
        </AdmCard>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14}}>
        <AdmCard padding={22}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14}}>
            <div style={{fontSize:21, fontWeight:600, color:ADM.TEXT}}>Piatti più ordinati</div>
            <span style={{fontSize:18.5, color:ADM.MUTED}}>Top 8 di {fmtNum(2840)}</span>
          </div>
          <div style={{display:'flex', flexDirection:'column', gap:9}}>
            {TOP_PIATTI.map((p, i) => (
              <div key={i} style={{display:'flex', alignItems:'center', gap:12, padding:'4px 0'}}>
                <div style={{width:18, textAlign:'center', fontSize:19, color:ADM.MUTED_SOFT, fontWeight:700}}>{i+1}</div>
                <div style={{flex:1, minWidth:0}}>
                  <div style={{fontSize:19.5, color:ADM.TEXT, fontWeight:500, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{p.nome}</div>
                  <div style={{fontSize:18, color:ADM.MUTED}}>{p.categoria} · {p.locali} locali</div>
                </div>
                <div style={{fontSize:20, fontWeight:600, color:ADM.TEXT, minWidth:60, textAlign:'right'}}>{fmtNum(p.ordini)}</div>
                <div style={{minWidth:46, textAlign:'right'}}>
                  <span style={{fontSize:18, fontWeight:600, color: p.trend>=0 ? ADM.OK : ADM.DANGER, display:'inline-flex', alignItems:'center', gap:2}}>
        ...
        <AdmCard padding={22}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14}}>
            <div style={{fontSize:21, fontWeight:600, color:ADM.TEXT}}>Locali con più ordini</div>
            <span style={{fontSize:18.5, color:ADM.MUTED}}>Mese corrente</span>
          </div>
          <div style={{display:'flex', flexDirection:'column', gap:9}}>
            {[...LOCALI].filter(l=>l.stato==='active').sort((a,b)=>b.ordiniMese-a.ordiniMese).slice(0,8).map((l, i) => (
              <div key={l.id} style={{display:'flex', alignItems:'center', gap:12, padding:'4px 0'}}>
                ...
                <div style={{flex:1, minWidth:0}}>
                  <div style={{fontSize:19.5, color:ADM.TEXT, fontWeight:500, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{l.nome}</div>
                  <div style={{fontSize:18, color:ADM.MUTED}}>{l.citta}</div>
                </div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:20, fontWeight:600, color:ADM.TEXT}}>{fmtNum(l.ordiniMese)}</div>
                  <div style={{fontSize:17.5, color:ADM.MUTED}}>{fmtEur(l.ordiniMese * l.ticketMedio)}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14}}>
        <AdmCard padding={22}>
          <div style={{fontSize:21, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Schermate più utilizzate · gestionale</div>
          <div style={{display:'flex', flexDirection:'column', gap:11}}>
            {SCREENS_USAGE.map((s, i) => (
              <div key={i}>
                <div style={{display:'flex', justifyContent:'space-between', marginBottom:5}}>
                  <span style={{fontSize:19.5, color:ADM.TEXT, fontWeight:500}}>{s.nome}</span>
        ...
        <AdmCard padding={22}>
          <div style={{fontSize:21, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Distribuzione per regione</div>
            ...
            {REGIONI.map((r, i) => {
              ...
              return (
                <div key={r} style={{display:'flex', alignItems:'center', gap:10}}>
                  <div style={{width:120, fontSize:19, color:ADM.TEXT, fontWeight:500}}>{r}</div>
                  ...
                  <div style={{width:50, fontSize:18.5, color:ADM.TEXT, fontWeight:600, textAlign:'right'}}>{n} loc.</div>
                  <div style={{width:42, fontSize:17.5, color:ADM.MUTED, textAlign:'right'}}>{Math.round(pct)}%</div>
                </div>
              );
            })}
          </div>
...
function Legend({ color, label }) {
  return (
    <div style={{display:'inline-flex', alignItems:'center', gap:6, fontSize:18.5, color:ADM.MUTED, fontWeight:500}}>
