...
function TrendBadge({ delta, label='vs 7gg', size='sm', tone, hideLabel=false }) {
  ...
  const fs = size === 'lg' ? 12.5 : size === 'sm' ? 11 : 11.5;
  return (
    <span style={{
  ...
  return (
    <div style={{padding:'24px 28px', display:'flex', flexDirection:'column', gap:14}}>
      ...
      <div style={{padding:'11px 14px', background:ADM.WARN_SOFT, border:`1px solid ${ADM.WARN}33`, borderRadius:9, display:'flex', gap:10, alignItems:'flex-start'}}>
        <BuIcons.info size={14} color={ADM.WARN}/>
        <div style={{flex:1, fontSize:13.5, color:'#7A4A0C', lineHeight:1.5}}>
      ...
      <AdmCard padding={0}>
        <div style={{padding:'14px 22px 12px', borderBottom:`1px solid ${ADM.BORDER}`}}>
          <div style={{fontSize:15.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.01em'}}>Food cost & marginalità per categoria</div>
          <div style={{fontSize:13, color:ADM.MUTED, marginTop:2}}>Margine lordo stimato · proxy del valore di un marketplace fornitori</div>
        </div>
        <div style={{display:'grid', gridTemplateColumns:'1.4fr 0.9fr 0.9fr 1.3fr 1fr', padding:'12px 22px', background:ADM.PANEL_SOFT, fontSize:13, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', borderBottom:`1px solid ${ADM.BORDER}`}}>
          <div>Categoria</div>
          ...
          return (
            <div key={c.cat} style={{
              ...
              <div style={{display:'flex', alignItems:'center', gap:10}}>
                ...
                <div>
                  <div style={{fontSize:15, fontWeight:700, color:ADM.TEXT}}>{c.cat}</div>
                  <div style={{fontSize:12.5, color:ADM.MUTED}}>Ricavi/mese {fmtEur(Math.round(c.ricavi))}</div>
                </div>
              </div>
              <div style={{fontSize:15, fontWeight:700, color:ADM.TEXT, textAlign:'right', fontFamily:'ui-monospace, monospace'}}>{fmtEur(c.prezzo)}</div>
              <div style={{fontSize:15, fontWeight:700, color:ADM.DANGER, textAlign:'right', fontFamily:'ui-monospace, monospace'}}>{c.foodCost}%</div>
              <div style={{display:'flex', alignItems:'center', gap:10}}>
                ...
                <span style={{fontSize:15, fontWeight:800, color:marginTone, minWidth:42, textAlign:'right', fontFamily:'ui-monospace, monospace'}}>{c.margine}%</span>
              </div>
              <div style={{fontSize:15, color:ADM.TEXT, fontWeight:600, textAlign:'right', fontFamily:'ui-monospace, monospace'}}>{fmtNum(c.ord)}</div>
            </div>
          );
        })}
        <div style={{padding:'14px 22px', borderTop:`1px solid ${ADM.BORDER}`, background:ADM.PANEL_SOFT, fontSize:13.5, color:ADM.MUTED, lineHeight:1.5}}>
...
function HoverStat({ label, value, sub, accent='PINK', icon='trendUp', onClick, tooltip, big, size='md', trend, trendLabel, spark, alert }) {
  ...
  const isXL = size === 'xl';
  const valueSize = isXL ? 40 : big ? 30 : 26;
        ...
        <div style={{display:'flex', alignItems:'center', gap:14}}>
          ...
          <div style={{flex:1, minWidth:0}}>
            <div style={{display:'flex', alignItems:'center', gap:7}}>
              <div style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em'}}>{label}</div>
              {tooltip && (
                <span style={{
            ...
            {(sub || spark || alert) && (
              <div style={{display:'flex', alignItems:'center', gap:10, marginTop:6, flexWrap:'wrap'}}>
                {sub && <div style={{fontSize:14, color:alert ? ADM.WARN : ADM.MUTED, fontWeight:alert ? 600 : 500, flex:1, minWidth:0}}>
  ...
  return (
    <div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:18, marginBottom:14}}>
        <div>
          <div style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8}}>Ultimo mese</div>
          ...
          <div style={{borderTop:`1px solid ${ADM.BORDER_SOFT}`, marginTop:7, paddingTop:7, display:'flex', justifyContent:'space-between'}}>
            <span style={{fontSize:13.5, fontWeight:700, color:ADM.TEXT}}>Totale</span>
            <span style={{fontSize:15, fontWeight:800, color:ADM.TEXT}}>{fmtEur(sub + extra)}</span>
          </div>
          {Math.abs(delta) > 0.5 && (
        ...
        <div>
          <div style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8}}>Ultimo anno (12m)</div>
          ...
          <div style={{borderTop:`1px solid ${ADM.BORDER_SOFT}`, marginTop:7, paddingTop:7, display:'flex', justifyContent:'space-between'}}>
            <span style={{fontSize:13.5, fontWeight:700, color:ADM.TEXT}}>Totale</span>
            <span style={{fontSize:15, fontWeight:800, color:ADM.TEXT}}>{fmtEur(ricaviAnno)}</span>
          </div>
      ...
      <div style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:6}}>Andamento 12 mesi</div>
      <MiniRevBar/>
    </div>
  );
}
      ...
      {last12.map((m, i) => {
        ...
        return (
          <div key={i} style={{flex:1, height:'100%', display:'flex', flexDirection:'column', justifyContent:'flex-end'}}>
            ...
            <div style={{fontSize:10, color:ADM.MUTED_SOFT, marginTop:3, textAlign:'center', fontWeight:600}}>{m.mese.slice(0,3)}</div>
          </div>
        );
      })}
    </div>
...
function RevRow({ color, label, val }) {
  return (
    <div style={{display:'flex', alignItems:'center', gap:8, padding:'4px 0'}}>
      ...
      <span style={{fontSize:14, color:ADM.TEXT, flex:1}}>{label}</span>
      <span style={{fontSize:14.5, fontWeight:700, color:ADM.TEXT}}>{val}</span>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{paddingTop:14, borderTop:`1px solid ${ADM.BORDER_SOFT}`, marginBottom:14}}>
        <div style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8}}>
          Suddivisione piano free
        </div>
        <div style={{display:'flex', alignItems:'center', gap:10, marginBottom:6}}>
          <span style={{width:9, height:9, borderRadius:2, background:ADM.OK}}/>
          <span style={{fontSize:14, color:ADM.TEXT, flex:1, fontWeight:500}}>Free attivi</span>
          ...
          <span style={{fontSize:14, color:ADM.TEXT, fontWeight:700, width:32, textAlign:'right'}}>{freeActive}</span>
        ...
        <div style={{display:'flex', alignItems:'center', gap:10}}>
          ...
          <span style={{fontSize:14, color:ADM.TEXT, flex:1, fontWeight:500}}>Free inattivi</span>
          <div style={{width:60, height:5, background:'#F4F5F7', borderRadius:99, overflow:'hidden'}}>
            <div style={{width:`${freeInactPct}%`, height:'100%', background:ADM.MUTED_LIGHT}}/>
          </div>
          <span style={{fontSize:14, color:ADM.TEXT, fontWeight:700, width:32, textAlign:'right'}}>{freeInactive}</span>
        ...
        {paidPlans.map(p => {
          ...
          return (
            <div key={p.id} style={{display:'flex', alignItems:'center', gap:10, marginBottom:6}}>
              ...
              <span style={{fontSize:14, color:ADM.TEXT, flex:1, fontWeight:500}}>{p.label}</span>
              <div style={{width:60, height:5, background:'#F4F5F7', borderRadius:99, overflow:'hidden'}}>
                <div style={{width:`${pct}%`, height:'100%', background:ADM[p.color]}}/>
              </div>
              <span style={{fontSize:14, color:ADM.TEXT, fontWeight:700, width:32, textAlign:'right'}}>{c}</span>
...
function SplitRow({ color, label, val, pct }) {
  return (
    <div style={{display:'flex', alignItems:'center', gap:8}}>
      ...
      <div style={{flex:1, minWidth:0}}>
        <div style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>{label}</div>
        <div style={{fontSize:17, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.01em'}}>{val} <span style={{fontSize:13, color:ADM.MUTED_SOFT, fontWeight:500}}>· {pct}%</span></div>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      {stuckOver7 > 0 && (
        <div style={{
          ...
          <span style={{
            ...
            <BuIcons.clock size={15}/>
          </span>
          <div style={{flex:1, minWidth:0}}>
            <div style={{fontSize:14.5, fontWeight:700, color:ADM.TEXT}}>
      ...
      {stuck.length > 0 && (
        <div style={{marginBottom:14}}>
          <div style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:7}}>Top più vecchi</div>
            ...
            {stuck.map(({l, age}, i) => {
              ...
              return (
                ...
                onMouseLeave={e=>e.currentTarget.style.background = '#fff'}>
                  ...
                  <div style={{flex:1, minWidth:0}}>
                    <div style={{fontSize:14, fontWeight:600, color:ADM.TEXT, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{l.nome}</div>
                    <div style={{fontSize:12.5, color:ADM.MUTED, marginTop:1}}>{l.citta} · {l.stato === 'pending' ? 'Setup iniziale' : 'Onboarding'}</div>
                  </div>
                  <span style={{fontSize:14, fontWeight:800, color:tone, fontFamily:'ui-monospace, monospace', whiteSpace:'nowrap'}}>{age}gg</span>
                  <span style={{color:ADM.MUTED}}><BuIcons.chevronRight size={12}/></span>
                </button>
              );
            })}
          </div>
...
function OnbRow({ count, label, desc, color }) {
  return (
    <div style={{display:'flex', gap:14, alignItems:'flex-start'}}>
      ...
      <div>
        <div style={{fontSize:14.5, fontWeight:700, color:ADM.TEXT}}>{label}</div>
        <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:3, lineHeight:1.45}}>{desc}</div>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14}}>
        <div style={{padding:'12px 14px', background:`${ADM.OK}10`, border:`1px solid ${ADM.OK}40`, borderRadius:9}}>
          <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em'}}>Conversion rate</div>
          <div style={{fontSize:24, fontWeight:800, color:ADM.OK, marginTop:5, letterSpacing:'-0.02em', lineHeight:1}}>{convRate}%</div>
        ...
        <div style={{padding:'12px 14px', background:ADM.PANEL_SOFT, border:`1px solid ${ADM.BORDER_SOFT}`, borderRadius:9}}>
          <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em'}}>Iscritti totali</div>
          <div style={{fontSize:24, fontWeight:800, color:ADM.TEXT, marginTop:5, letterSpacing:'-0.02em', lineHeight:1}}>{fmtNum(tot)}</div>
          <div style={{fontSize:13, color:ADM.MUTED, marginTop:4}}>{pending} in attesa di iniziare</div>
        </div>
      </div>

      {/* Stacked bar */}
      <div style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8}}>Stato dell'onboarding</div>
        ...
        {rows.map((r, i) => {
          ...
          return (
            <div key={i} style={{display:'flex', alignItems:'center', gap:10}}>
              ...
              <div style={{flex:1, minWidth:0}}>
                <div style={{fontSize:14, color:ADM.TEXT, fontWeight:600}}>{r.label}</div>
                <div style={{fontSize:12.5, color:ADM.MUTED, marginTop:1}}>{r.desc}</div>
              </div>
              <span style={{fontSize:15, color:ADM.TEXT, fontWeight:800, width:32, textAlign:'right'}}>{r.count}</span>
              <span style={{fontSize:13, color:ADM.MUTED, fontWeight:600, width:36, textAlign:'right'}}>{pct}%</span>
            </div>
          );
        })}
    ...
    <div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14}}>
        <div style={{padding:'12px 14px', background:`${ADM.PURPLE}10`, border:`1px solid ${ADM.PURPLE}40`, borderRadius:9}}>
          <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em'}}>Media</div>
          <div style={{fontSize:24, fontWeight:800, color:ADM.PURPLE, marginTop:5, letterSpacing:'-0.02em', lineHeight:1}}>{fmtMin(media)}</div>
          <div style={{fontSize:13, color:ADM.MUTED, marginTop:4}}>su {campione} locali</div>
        </div>
        <div style={{padding:'12px 14px', background:ADM.PANEL_SOFT, border:`1px solid ${ADM.BORDER_SOFT}`, borderRadius:9}}>
          <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em'}}>Mediana</div>
          <div style={{fontSize:24, fontWeight:800, color:ADM.TEXT, marginTop:5, letterSpacing:'-0.02em', lineHeight:1}}>{fmtMin(mediana)}</div>
          <div style={{fontSize:13, color:ADM.MUTED, marginTop:4}}>min {fmtMin(minV)} · max {fmtMin(maxV)}</div>
        </div>
      </div>

      {/* Distribuzione */}
      <div style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8}}>Distribuzione</div>
        ...
        {buckets.map((b, i) => {
          ...
          return (
            <div key={i} style={{display:'flex', alignItems:'center', gap:10}}>
              ...
              <span style={{fontSize:14, color:ADM.TEXT, fontWeight:500, width:88, flexShrink:0}}>{b.range}</span>
              ...
              <span style={{fontSize:14, color:ADM.TEXT, fontWeight:700, width:28, textAlign:'right'}}>{b.count}</span>
              <span style={{fontSize:13, color:ADM.MUTED, fontWeight:600, width:36, textAlign:'right'}}>{pct}%</span>
            </div>
          );
        })}
      ...
      <div style={{paddingTop:12, borderTop:`1px solid ${ADM.BORDER_SOFT}`}}>
        <div style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:8}}>Tempo medio per step</div>
        <div style={{display:'flex', flexDirection:'column', gap:5}}>
          {steps.map((s, i) => {
            ...
            return (
              <div key={i} style={{display:'flex', alignItems:'center', gap:10}}>
                <span style={{fontSize:13.5, color:ADM.TEXT, fontWeight:500, width:138, flexShrink:0}}>{s.label}</span>
                ...
                <span style={{fontSize:13.5, color: isSlow ? ADM.WARN : ADM.TEXT, fontWeight:700, width:54, textAlign:'right'}}>{fmtMin(s.minutes)}</span>
              </div>
            );
          })}
        </div>
...
function UtentiTooltip({ tot, attivi24, attivi7g, attivi30g }) {
  return (
    <div>
      ...
      <div style={{marginTop:12, paddingTop:12, borderTop:`1px solid ${ADM.BORDER_SOFT}`, fontSize:13.5, color:ADM.MUTED, lineHeight:1.5}}>
...
function ActiveCell({ label, val, tot, color }) {
  ...
  return (
    <div style={{padding:'10px 12px', background:ADM[color+'_SOFT'], borderRadius:8}}>
      <div style={{fontSize:12, fontWeight:700, color:ADM[color], textTransform:'uppercase', letterSpacing:'0.05em'}}>{label}</div>
      <div style={{fontSize:20, fontWeight:800, color:ADM.TEXT, marginTop:4, letterSpacing:'-0.02em'}}>{fmtNum(val)}</div>
    ...
    <div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:10}}>
        <div style={{padding:'12px 14px', background:ADM.INFO_SOFT, borderRadius:8}}>
          <div style={{fontSize:12, fontWeight:700, color:ADM.INFO, textTransform:'uppercase', letterSpacing:'0.05em'}}>Accessi guest</div>
          <div style={{fontSize:24, fontWeight:800, color:ADM.TEXT, marginTop:4, letterSpacing:'-0.02em'}}>{fmtNum(accessi)}</div>
        ...
        <div style={{padding:'12px 14px', background:ADM.OK_SOFT, borderRadius:8}}>
          <div style={{fontSize:12, fontWeight:700, color:ADM.OK, textTransform:'uppercase', letterSpacing:'0.05em'}}>Ordini effettuati</div>
          <div style={{fontSize:24, fontWeight:800, color:ADM.TEXT, marginTop:4, letterSpacing:'-0.02em'}}>{fmtNum(ordini)}</div>
    ...
    <div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14}}>
        <div style={{padding:'12px 14px', background:`${c}10`, border:`1px solid ${c}40`, borderRadius:9}}>
          <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em'}}>{meseLabel || 'Ultimi 30 giorni'}</div>
          <div style={{fontSize:24, fontWeight:800, color:ADM.TEXT, marginTop:5, letterSpacing:'-0.02em', lineHeight:1}}>{fmtNum(mese)}</div>
        </div>
        <div style={{padding:'12px 14px', background:ADM.PANEL_SOFT, border:`1px solid ${ADM.BORDER_SOFT}`, borderRadius:9}}>
          <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em'}}>{annoLabel || 'Ultimi 12 mesi'}</div>
          <div style={{fontSize:24, fontWeight:800, color:ADM.TEXT, marginTop:5, letterSpacing:'-0.02em', lineHeight:1}}>{fmtNum(anno)}</div>
        </div>
      </div>
    </div>
  );
...
function ScanOrdiniBox({ label, scan, ord, ratio, primary }) {
  ...
  return (
    <div style={{
      ...
      <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:7}}>{label}</div>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', gap:12, marginBottom:4}}>
        <span style={{fontSize:13, color:ADM.MUTED, fontWeight:600, flexShrink:0}}>Ordini</span>
        <span style={{fontSize:16, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.01em'}}>{fmtNum(ord)}</span>
      </div>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', gap:12, marginBottom:7}}>
        ...
        <span style={{fontSize:16, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.01em'}}>{fmtNum(scan)}</span>
      </div>
      <div style={{borderTop:`1px solid ${primary ? ADM.PINK_SOFT : ADM.BORDER_SOFT}`, paddingTop:7, display:'flex', justifyContent:'space-between', alignItems:'baseline', gap:12}}>
        ...
        <span style={{fontSize:18, fontWeight:800, color: c, letterSpacing:'-0.01em'}}>
...
function SectionTitle({ title, desc }) {
  return (
    <div style={{display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginTop:6, marginBottom:-4}}>
      <div>
        <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.005em'}}>{title}</div>
        <div style={{fontSize:14, color:ADM.MUTED, marginTop:2}}>{desc}</div>
      </div>
    </div>
  );
}
      ...
      <div style={{display:'flex', alignItems:'flex-start', gap:12, marginBottom: spark ? 10 : 0}}>
        ...
        <div style={{flex:1, minWidth:0}}>
          ...
          <div style={{display:'flex', alignItems:'baseline', gap:8, marginTop:4, flexWrap:'wrap'}}>
            <div style={{fontSize:26, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.025em', lineHeight:1}}>{value}</div>
            {hasTrend && <TrendBadge delta={trend} label={trendLabel} hideLabel/>}
          </div>
          {sub && <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:5, lineHeight:1.4}}>{sub}</div>}
        </div>
      </div>
      {spark && (
  ...
  return (
    <AdmCard padding={big ? 22 : 18} onClick={onClick} style={onClick ? {cursor:'pointer'} : null}>
      <div style={{display:'flex', alignItems:'flex-start', gap:14}}>
        ...
        <div style={{flex:1, minWidth:0}}>
          <div style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600, textTransform:'uppercase', letterSpacing:'0.04em'}}>{label}</div>
          <div style={{fontSize: big ? 28 : 22, fontWeight:800, color:ADM.TEXT, marginTop:4, letterSpacing:'-0.02em'}}>{value}</div>
          {sub && <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:3}}>{sub}</div>}
        </div>
      </div>
    </AdmCard>
  );
  ...
  return (
    <AdmCard padding={16}>
      <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:6}}>
        ...
        <div style={{fontSize:14, fontWeight:600, color:ADM.TEXT}}>Piano {piano.label}</div>
        <div style={{flex:1}}/>
        <div style={{fontSize:12.5, color:ADM.MUTED_SOFT, fontWeight:600}}>{pct}%</div>
      </div>
      <div style={{fontSize:26, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.02em', marginBottom:6}}>{fmtNum(count)}</div>
    ...
    <div style={{padding:'24px 28px', display:'flex', flexDirection:'column', gap:18}}>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, gap:14, flexWrap:'wrap'}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Funnel di onboarding</div>
            <div style={{fontSize:14, color:ADM.MUTED, marginTop:2}}>Conversione da iscrizione a Go-live</div>
          </div>
        ...
        {bottleneckStep && (
          <div style={{
            ...
            <span style={{width:30, height:30, borderRadius:8, background:ADM.DANGER, color:'#fff', display:'grid', placeItems:'center', flexShrink:0}}>
              <BuIcons.alertTriangle size={14}/>
            </span>
            <div style={{flex:1, minWidth:0}}>
              <div style={{fontSize:14, color:ADM.TEXT, fontWeight:600, lineHeight:1.45}}>
          ...
          {ONB_STEPS.map((step, i) => {
            ...
            return (
              <div key={step.id} style={{
                ...
                <div style={{width:170, fontSize:14.5, color:ADM.TEXT, fontWeight: isAvvio || isBottleneck ? 700 : 500, display:'flex', alignItems:'center', gap:6, flexWrap:'wrap'}}>
                  {step.label}
                ...
                <div style={{width:96, textAlign:'right', fontSize:13.5, color: isBottleneck ? ADM.DANGER : (dropoff > 0 && !isOptional ? ADM.DANGER : ADM.MUTED), fontWeight: isBottleneck ? 800 : 600}}>
                  {i > 0 && dropoff > 0 ? (
      ...
      <AdmCard padding={20}>
        <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Top città</div>
        <div style={{display:'flex', flexDirection:'column', gap:9}}>
          {TOP_CITTA.map((c, i) => (
            <div key={i} style={{display:'flex', alignItems:'center', gap:12, padding:'4px 0'}}>
              <div style={{fontSize:15, color:ADM.MUTED_SOFT, fontWeight:700, width:18}}>{i+1}</div>
              <div style={{fontSize:15, color:ADM.TEXT, fontWeight:500, flex:1}}>{c.citta}</div>
              <div style={{fontSize:13.5, color:ADM.MUTED}}>{c.locali} locali</div>
              <div style={{fontSize:14, color:ADM.TEXT, fontWeight:600, width:80, textAlign:'right'}}>{fmtNum(c.ordini)} ord</div>
              <div style={{fontSize:13.5, color:ADM.OK, fontWeight:600, width:70, textAlign:'right'}}>{fmtEur(c.mrr)}</div>
            </div>
          ))}
        </div>
      </AdmCard>
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>Abbandono locali</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Quanti se ne vanno, quando, perché · metrica critica per investor reporting</div>
      </div>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:18}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Tasso di abbandono per piano</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Annualizzato · vs media di settore food-service · tempo medio di permanenza prima dell'abbandono</div>
          </div>
            ...
            return (
              <div key={p.id} style={{padding:'14px 16px', background:'#fff', border:`1px solid ${ADM.BORDER}`, borderRadius:12}}>
                ...
                <div style={{display:'flex', alignItems:'baseline', gap:8, marginBottom:8}}>
                  <div style={{fontSize:32, fontWeight:800, color: p.rate > 15 ? ADM.DANGER : p.rate > 8 ? ADM.WARN : ADM.OK, letterSpacing:'-0.03em', lineHeight:1}}>{p.rate.toFixed(1)}%</div>
                  <span style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>/ anno</span>
                </div>
        ...
        <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:16, lineHeight:1.5}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1.4fr 1fr', gap:14}}>
        <AdmCard padding={20}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
            <div>
              <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Locali persi al mese · ultimi 12 mesi</div>
              <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Numero locali che hanno disdetto · picco a fine anno fiscale</div>
            </div>
            <div style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>
            ...
            {churnMonthly.map((m,i) => {
              ...
              return (
                <div key={i} style={{flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:6}}>
                  <div style={{fontSize:12.5, fontWeight:700, color: peak ? ADM.DANGER : ADM.MUTED}}>{m.n}</div>
                  <div style={{
                    width:'100%',
                  ...
                  <div style={{fontSize:11.5, fontWeight:600, color:ADM.MUTED_SOFT, letterSpacing:'0.02em'}}>{m.m}</div>
                </div>
              );
            })}
          </div>
        ...
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, marginBottom:14}}>Motivi di abbandono</div>
              ...
              return (
                <div key={i}>
                  <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:4}}>
                    ...
                    <span style={{fontSize:14, color:ADM.TEXT, fontWeight:600, flex:1, lineHeight:1.3}}>{r.n}</span>
                    <span style={{fontSize:14, color:ADM.TEXT, fontWeight:800}}>{r.pct}%</span>
                  </div>
                  <div style={{height:5, background:'#F4F5F7', borderRadius:99, overflow:'hidden'}}>
          ...
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:14, lineHeight:1.5}}>
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>Tempo medio di servizio</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Dall'ordine confermato alla chiusura conto · media di settore</div>
      </div>

      <div style={{display:'grid', gridTemplateColumns:'1fr 1.6fr', gap:14}}>
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, marginBottom:4}}>Distribuzione locali</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginBottom:14}}>Per fascia di tempo medio</div>
          <div style={{display:'flex', alignItems:'baseline', gap:8, marginBottom:18}}>
            <div style={{fontSize:40, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.03em', lineHeight:1}}>{serviceOverall}</div>
            <div style={{fontSize:15, color:ADM.MUTED, fontWeight:600}}>min media piattaforma</div>
          </div>
          <div style={{display:'flex', flexDirection:'column', gap:9}}>
            {serviceDist.map((d,i) => {
              ...
              return (
                <div key={i}>
                  <div style={{display:'flex', justifyContent:'space-between', marginBottom:4}}>
                    <span style={{fontSize:14, color:ADM.TEXT, fontWeight:600}}>{d.range} <span style={{color:ADM.MUTED, fontWeight:500}}>· {d.label}</span></span>
                    <span style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>{d.pct}%</span>
                  </div>
                  <div style={{height:6, background:'#F4F5F7', borderRadius:99, overflow:'hidden'}}>
        ...
        <AdmCard padding={20}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
            <div>
              <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Per tipo locale</div>
              <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Mediana · P75 · campione di locali</div>
            </div>
              ...
              return (
                <div key={i}>
                  <div style={{display:'flex', justifyContent:'space-between', marginBottom:6}}>
                    <span style={{fontSize:14.5, color:ADM.TEXT, fontWeight:700}}>{t.tipo}</span>
                    <span style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>n={t.n}</span>
                  </div>
                  ...
                  <div style={{position:'relative', height:24, background:'#F4F5F7', borderRadius:8, overflow:'hidden'}}>
                    ...
                    <span style={{position:'absolute', right:8, top:'50%', transform:'translateY(-50%)', fontSize:12.5, fontWeight:700, color:ADM.MUTED}}>P75 {t.p75}</span>
                  </div>
                </div>
              );
            })}
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>Mix canali d'ordine · 12 mesi</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Come si spostano gli ordini tra cameriere, QR-tavolo, app cliente</div>
      </div>
        ...
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
          <div style={{display:'flex', gap:16}}>
            {[
              ...
              <span key={i} style={{display:'inline-flex', alignItems:'center', gap:6, fontSize:14, fontWeight:600, color:ADM.TEXT}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14}}>
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, marginBottom:4}}>Top in transizione digitale</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginBottom:14}}>Locali con la spinta digitale più rapida (% ordini app+QR · 6 mesi fa → oggi)</div>
          <div style={{display:'flex', flexDirection:'column', gap:11}}>
            {channelMovers.map((m,i) => (
              <div key={m.id} style={{display:'flex', alignItems:'center', gap:12}}>
                <span style={{fontSize:14, fontWeight:800, color:ADM.OK, width:18}}>{i+1}</span>
                <div style={{flex:1, minWidth:0}}>
                  <div style={{fontSize:14.5, fontWeight:600, color:ADM.TEXT, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{m.nome}</div>
                  <div style={{fontSize:12.5, color:ADM.MUTED}}>{m.citta}</div>
                </div>
                <div style={{display:'flex', alignItems:'center', gap:6, fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>
                  <span style={{fontFamily:'ui-monospace,monospace'}}>{m.from}%</span>
                  <BuIcons.chevronRight size={11}/>
        ...
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, marginBottom:4}}>Fermi a zero digitale</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginBottom:14}}>Locali attivi da 5+ mesi ma con &lt; 5% ordini digitali · candidati a intervento formazione</div>
          <div style={{display:'flex', flexDirection:'column', gap:11}}>
            {channelStuck.map((m,i) => (
              <div key={m.id} style={{display:'flex', alignItems:'center', gap:12}}>
                <span style={{fontSize:14, fontWeight:800, color:ADM.DANGER, width:18}}>{i+1}</span>
                <div style={{flex:1, minWidth:0}}>
                  <div style={{fontSize:14.5, fontWeight:600, color:ADM.TEXT, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{m.nome}</div>
                  <div style={{fontSize:12.5, color:ADM.MUTED}}>{m.citta} · attivo da {m.daysActive}g</div>
                </div>
      ...
      {/* ═════ LTV / CAC ═════ */}
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>LTV / CAC locale</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Economia per locale · la prima metrica che ti chiederà un investitore pre-seed</div>
      </div>

      <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:14}}>
        <AdmCard padding={18}>
          <div style={{fontSize:13, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em'}}>LTV medio</div>
          <div style={{fontSize:32, fontWeight:800, color:ADM.TEXT, marginTop:6, letterSpacing:'-0.03em', lineHeight:1}}>{fmtEur(avgLTV)}</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:7, lineHeight:1.4}}>Media paganti · MRR × tenure</div>
        </AdmCard>
        <AdmCard padding={18}>
          <div style={{fontSize:13, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em'}}>CAC medio</div>
          <div style={{fontSize:32, fontWeight:800, color:ADM.TEXT, marginTop:6, letterSpacing:'-0.03em', lineHeight:1}}>{fmtEur(avgCAC)}</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:7, lineHeight:1.4}}>Marketing + sales / locali acquisiti</div>
        </AdmCard>
        <AdmCard padding={18}>
          ...
          <div style={{display:'flex', alignItems:'baseline', gap:8, marginTop:6}}>
            <div style={{fontSize:32, fontWeight:800, color:ADM.OK, letterSpacing:'-0.03em', lineHeight:1}}>{ratioLTVCAC.toFixed(1)}×</div>
            <span style={{fontSize:13, color:ADM.OK, fontWeight:700, padding:'2px 7px', background:ADM.OK_SOFT, borderRadius:5}}>health</span>
          </div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:7, lineHeight:1.4}}>Soglia investitori: ≥ 3× sano · ≥ 5× eccellente</div>
        </AdmCard>
        <AdmCard padding={18}>
          ...
          <div style={{fontSize:32, fontWeight:800, color:ADM.TEXT, marginTop:6, letterSpacing:'-0.03em', lineHeight:1}}>{avgPayback.toFixed(1)} <span style={{fontSize:16, fontWeight:600, color:ADM.MUTED}}>mesi</span></div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:7, lineHeight:1.4}}>Tempo per recuperare il CAC</div>
        </AdmCard>
      </div>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>LTV cumulativo per piano</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Ricavo cumulativo netto dal mese di attivazione · LTV - CAC</div>
          </div>
          <div style={{display:'flex', gap:14, fontSize:13.5, color:ADM.TEXT, fontWeight:600}}>
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>No show prenotazioni</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Prima causa di sofferenza del ristoratore</div>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1.6fr', gap:14}}>
        <AdmCard padding={20}>
          <div style={{display:'flex', alignItems:'center', gap:12, marginBottom:14}}>
            ...
            <div>
              ...
              <div style={{display:'flex', alignItems:'baseline', gap:6, marginTop:3}}>
                <div style={{fontSize:34, fontWeight:800, color:ADM.DANGER, letterSpacing:'-0.025em', lineHeight:1}}>{noShowRate}%</div>
                <span style={{fontSize:14, color:ADM.MUTED, fontWeight:600}}>≈ {fmtNum(noShowCount30g)} prenotazioni</span>
              </div>
            </div>
          </div>
          <div style={{padding:'12px 14px', background:ADM.PANEL_SOFT, borderRadius:9, fontSize:13.5, color:ADM.MUTED, lineHeight:1.5, marginBottom:16}}>
          ...
          <div>
            ...
            <div style={{display:'flex', justifyContent:'space-between', fontSize:12.5, color:ADM.MUTED, marginTop:6, fontWeight:600}}>
        ...
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, marginBottom:4}}>Dove succede di più</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginBottom:14}}>% di no show per <strong style={{color:ADM.TEXT}}>giorno della settimana</strong> × <strong style={{color:ADM.TEXT}}>fascia oraria del giorno</strong></div>
          <div style={{display:'grid', gridTemplateColumns:`66px repeat(${noShowSlots.length}, 1fr)`, gap:4}}>
            <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED_SOFT, textTransform:'uppercase', letterSpacing:'0.06em', display:'flex', alignItems:'flex-end', paddingBottom:6}}>Giorno</div>
            {noShowSlots.map(s => (
              <div key={s.label} style={{display:'flex', flexDirection:'column', alignItems:'center', gap:2, padding:'4px 0'}}>
                <span style={{fontSize:11.5, fontWeight:700, color:ADM.MUTED_SOFT, textTransform:'uppercase', letterSpacing:'0.05em'}}>{s.label}</span>
                <span style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, letterSpacing:'0.02em', fontVariantNumeric:'tabular-nums'}}>{s.range}</span>
              </div>
            ))}
            {noShowHeat.map(row => (
              <React.Fragment key={row.g}>
                <div style={{fontSize:13.5, fontWeight:700, color:ADM.TEXT, display:'flex', alignItems:'center'}}>{row.g}</div>
                {row.vals.map((v,j) => {
          ...
          <div style={{display:'flex', justifyContent:'flex-end', alignItems:'center', gap:8, marginTop:10}}>
            <span style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.05em'}}>Intensità no show</span>
            <span style={{fontSize:12, color:ADM.MUTED, fontWeight:600}}>basso</span>
            ...
            <span style={{fontSize:12, color:ADM.MUTED, fontWeight:600}}>alto</span>
          </div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:14, lineHeight:1.5}}>
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>Posizionamento prezzi per città</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Listino medio dei locali partner · benchmark territoriale</div>
      </div>

      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, gap:14, flexWrap:'wrap'}}>
          <div style={{minWidth:0, flex:'1 1 300px'}}>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Prezzo medio per città · andamento 12 mesi</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Prezzo medio del piatto sui menù dei locali partner. Confronta fino a {priceMaxPick} piatti e osserva il trend nel tempo.</div>
          </div>
          {/* Region selector */}
          <div style={{display:'inline-flex', alignItems:'center', gap:8}}>
            <span style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em'}}>Area</span>
            <select value={priceRegion} onChange={(e)=>setPriceRegion(e.target.value)} style={{
              ...
              fontSize:14, fontWeight:600, color:ADM.TEXT, background:'#fff', fontFamily:'inherit', cursor:'pointer',
        ...
        <div style={{marginBottom:18}}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:8}}>
            <span style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em'}}>Piatti da confrontare</span>
          ...
          <div style={{display:'flex', flexWrap:'wrap', gap:6, padding:'8px 8px', border:`1px solid ${ADM.BORDER}`, borderRadius:9, background:'#fff', minHeight:42, alignItems:'center', cursor:'text', position:'relative'}} onClick={()=>setPricePickerOpen(true)}>
            {priceSel.map((name, i) => {
              ...
              return (
                <span key={name} style={{display:'inline-flex', alignItems:'center', gap:6, padding:'4px 8px 4px 10px', background:`${c}14`, border:`1px solid ${c}40`, borderRadius:99, fontSize:13.5, fontWeight:600, color:ADM.TEXT}}>
                  <span style={{width:8, height:8, borderRadius:'50%', background:c}}/>
                  {name}
                  <button onClick={(e)=>{e.stopPropagation(); if (priceSel.length > 1) setPriceSel(priceSel.filter(x=>x!==name));}} style={{background:'transparent', border:'none', padding:0, margin:'0 0 0 2px', cursor:'pointer', color:ADM.MUTED, fontSize:16, lineHeight:1, fontFamily:'inherit', width:16, height:16, display:'inline-grid', placeItems:'center'}} aria-label={`Rimuovi ${name}`}>×</button>
                </span>
              );
            })}
            <input
              ...
              style={{flex:'1 1 160px', minWidth:140, border:'none', outline:'none', fontSize:14, color:ADM.TEXT, fontFamily:'inherit', padding:'4px 6px', background:'transparent'}}
            />
            {pricePickerOpen && (
              <div style={{position:'absolute', top:'100%', left:0, right:0, marginTop:6, background:'#fff', border:`1px solid ${ADM.BORDER}`, borderRadius:9, boxShadow:'0 8px 24px rgba(15,17,21,0.10)', maxHeight:280, overflowY:'auto', zIndex:30}}>
                {priceFiltered.length === 0 ? (
                  <div style={{padding:'12px 14px', fontSize:14, color:ADM.MUTED}}>Nessun piatto corrisponde a "{priceQuery}"</div>
                ) : (() => {
                  const byCat = priceFiltered.reduce((acc, d) => { (acc[d.cat] = acc[d.cat] || []).push(d); return acc; }, {});
                  return Object.entries(byCat).map(([cat, items]) => (
                    <div key={cat}>
                      <div style={{padding:'8px 14px 4px', fontSize:12.5, fontWeight:700, color:ADM.MUTED_SOFT, textTransform:'uppercase', letterSpacing:'0.06em', background:ADM.PANEL_SOFT, position:'sticky', top:0}}>{cat}</div>
                      {items.map(d => {
                        ...
                        return (
                          <button
                            ...
                            style={{
                              ...
                              opacity: disabled ? 0.4 : 1, color: ADM.TEXT, fontSize:14, fontWeight: selected ? 700 : 500,
                            }}>
                            <span style={{width:14, height:14, borderRadius:4, border:`1.5px solid ${selected ? ADM.PINK_DARK : ADM.BORDER}`, background: selected ? ADM.PINK_DARK : '#fff', display:'inline-grid', placeItems:'center', color:'#fff', fontSize:12, fontWeight:800}}>{selected?'✓':''}</span>
                            <span style={{flex:1}}>{d.n}</span>
                            <span style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600, fontVariantNumeric:'tabular-nums'}}>{fmtEur(dishBasePrice(d.n).toFixed(2))}</span>
                          </button>
                        );
                      })}
                    </div>
            ...
            <div style={{overflow:'hidden', marginBottom:18}}>
              ...
              <div style={{display:'flex', flexWrap:'wrap', gap:'6px 16px', marginTop:10, justifyContent:'center'}}>
                {priceSeriesByDish.map((d, di) => {
                  ...
                  return (
                    <span key={d.name} style={{display:'inline-flex', alignItems:'center', gap:6, fontSize:13.5, fontWeight:600, color:ADM.TEXT}}>
                      <span style={{width:16, height:3, borderRadius:2, background:c}}/>
                      {d.name}
                      <span style={{fontSize:12.5, color: deltaPct >= 1 ? ADM.DANGER : deltaPct <= -1 ? ADM.OK : ADM.MUTED, fontWeight:700, fontVariantNumeric:'tabular-nums'}}>
              ...
              <div key={dish} style={{padding:'14px 16px', background:'#fff', border:`1px solid ${ADM.BORDER_SOFT}`, borderLeft:`3px solid ${c}`, borderRadius:9}}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:10, marginBottom:12}}>
                  <div style={{display:'flex', alignItems:'center', gap:8}}>
                    ...
                    <span style={{fontSize:15, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.005em'}}>{dish}</span>
                  </div>
                  <div style={{display:'flex', gap:14, alignItems:'baseline', fontSize:13.5, color:ADM.MUTED}}>
                  ...
                  {snap.map((r, i) => {
                    ...
                    return (
                      <div key={r.city} style={{display:'flex', alignItems:'center', gap:10}}>
                        <span style={{fontSize:12, fontWeight:800, color:ADM.MUTED_SOFT, width:14, textAlign:'right'}}>{i+1}</span>
                        <span style={{fontSize:13.5, fontWeight:600, color:ADM.TEXT, width:78}}>{r.city}</span>
                        ...
                        <span style={{fontSize:14, fontWeight:800, color:ADM.TEXT, width:54, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>{fmtEur(r.price)}</span>
                        <span style={{fontSize:12.5, fontWeight:700, color: delta > 5 ? ADM.DANGER : delta < -5 ? ADM.OK : ADM.MUTED, width:46, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>{delta >= 0 ? '+' : ''}{delta.toFixed(0)}%</span>
                      </div>
                    );
                  })}
                </div>
  ...
  return (
    <AdmCard padding={20}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16, gap:14}}>
        <div>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Adozione digitale</div>
          <div style={{fontSize:14, color:ADM.MUTED, marginTop:3, maxWidth:560, lineHeight:1.5}}>
        ...
        <div style={{textAlign:'right', flexShrink:0}}>
          ...
          <div style={{fontSize:24, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.02em', marginTop:2}}>{fmtNum(total)}</div>
        </div>
      </div>
      ...
      {isCriticalShare && (
        ...
        onMouseLeave={e=>{ e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.boxShadow='0 8px 24px -10px rgba(220,38,38,0.55)'; }}>
          <span style={{width:40, height:40, borderRadius:10, background:'rgba(255,255,255,0.18)', display:'grid', placeItems:'center', flexShrink:0, boxShadow:'inset 0 1px 0 rgba(255,255,255,0.25)'}}>
            <BuIcons.alertTriangle size={18} color="#fff"/>
          </span>
          <div style={{flex:1, minWidth:0, textAlign:'left'}}>
            ...
            <div style={{fontSize:13.5, color:'rgba(255,255,255,0.88)', marginTop:3, lineHeight:1.45}}>
          ...
          <span style={{
            ...
            Vai alla lista <BuIcons.chevronRight size={13} color="#fff"/>
          </span>
        </button>
      )}
        ...
        {buckets.map(b => {
          ...
          return (
            <div key={b.id}
              ...
              {showText && (
                <>
                  <span style={{fontSize:17, fontWeight:800, letterSpacing:'-0.01em'}}>{b.locali.length}</span>
                  <span style={{fontSize:13.5, fontWeight:600, opacity:0.92}}>{pct.toFixed(0)}%</span>
                </>
              )}
            </div>
          );
      ...
      <div style={{
        display:'flex', marginTop:8, fontSize:12.5, color:ADM.MUTED_SOFT, fontWeight:600,
        ...
        {ADOPTION_BANDS.map(b => (
          <div key={b.id} style={{
            ...
            <div style={{display:'flex', alignItems:'center', gap:7, marginBottom:5}}>
              ...
              <span style={{fontSize:14, fontWeight:700, color:ADM.TEXT}}>{b.label}</span>
              <span style={{fontSize:12.5, color:ADM.MUTED, fontFamily:'ui-monospace, monospace', marginLeft:'auto'}}>{b.range}</span>
            </div>
        ...
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10}}>
          <div style={{display:'flex', alignItems:'center', gap:8}}>
            <span style={{
              ...
              <BuIcons.alert size={13}/>
            </span>
            <div>
              <div style={{fontSize:15, fontWeight:700, color:'#7C2D12'}}>Da attivare</div>
          ...
          <span style={{
            ...
            borderRadius:99, fontSize:13.5, fontWeight:800,
          }}>{daAttivare.length}</span>
        </div>

        {daAttivare.length === 0 ? (
          <div style={{padding:'14px 8px', fontSize:14.5, color:'#9A3412', fontStyle:'italic', textAlign:'center'}}>
          ...
          <div style={{
            ...
            {daAttivare.slice(0, 12).map((l, i, arr) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{flex:1, minWidth:0}}>
                    <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>
                  ...
                  <span style={{color:ADM.MUTED, flexShrink:0}}><BuIcons.chevronRight size={15}/></span>
                </button>
              );
            })}
            {daAttivare.length > 12 && (
              <div style={{
                ...
                fontSize:13.5, color:ADM.MUTED, textAlign:'center', fontStyle:'italic',
                background:'#FFFBF6',
              }}>
...
function SottoMediaScanCard({ onNav }) {
  ...
  return (
    <AdmCard padding={20}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, gap:14, flexWrap:'wrap'}}>
        <div>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Locali sotto la media · Conversione QR</div>
          <div style={{fontSize:14, color:ADM.MUTED, marginTop:3, maxWidth:540, lineHeight:1.5}}>
      ...
      {/* Lista */}
      {sottoMedia.length === 0 ? (
        <div style={{padding:'24px 16px', textAlign:'center', fontSize:15, color:ADM.MUTED, fontStyle:'italic'}}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            fontSize:12, fontWeight:700, color:ADM.MUTED,
              ...
              onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                <div style={{display:'flex', alignItems:'center', gap:9, minWidth:0}}>
                  ...
                  <div style={{minWidth:0}}>
                    ...
                    <div style={{fontSize:12.5, color:ADM.MUTED, marginTop:1, fontFamily:'ui-monospace, monospace'}}>{l.id} · {l.citta}</div>
                  </div>
                </div>
                <div style={{textAlign:'right', fontSize:14.5, color:ADM.TEXT, fontWeight:600, fontFamily:'ui-monospace, monospace'}}>{fmtNum(l._ord)}</div>
                <div style={{textAlign:'right', fontSize:14.5, color:ADM.TEXT, fontWeight:600, fontFamily:'ui-monospace, monospace'}}>{fmtNum(l._scan)}</div>
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:15, fontWeight:800, color:ADM.DANGER, letterSpacing:'-0.01em'}}>{fmtPct(l._ratio)}</div>
                  <div style={{fontSize:12.5, color:ADM.MUTED, marginTop:1, fontWeight:600}}>
                    {gap.toFixed(0)}% vs media
                  </div>
                </div>
                <span style={{color:ADM.MUTED, textAlign:'right'}}><BuIcons.chevronRight size={15}/></span>
              </button>
            );
          })}
...
function SottoMediaKpi({ label, value, sub, highlight }) {
  return (
    <div style={{
      ...
      <div style={{fontSize:22, fontWeight:800, color: highlight ? ADM.PINK_DARK : ADM.TEXT, marginTop:5, letterSpacing:'-0.02em', lineHeight:1}}>{value}</div>
...
function DashUtentiApp() {
  ...
  return (
    <div style={{padding:'24px 28px', display:'flex', flexDirection:'column', gap:18}}>
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>Ritorno degli utenti nel tempo</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Quanti continuano a usare l'app dopo il primo ordine</div>
      </div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1.4fr 1fr', gap:14}}>
        <AdmCard padding={22}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, gap:14, flexWrap:'wrap'}}>
            <div style={{minWidth:0, flex:'1 1 240px'}}>
              <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Quanti tornano dopo il primo ordine</div>
              <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2, lineHeight:1.45}}>
          ...
          <div style={{fontSize:14, color:ADM.MUTED, marginTop:14, lineHeight:1.55, padding:'12px 14px', background:ADM.PANEL_SOFT, borderRadius:9}}>
        ...
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, marginBottom:4}}>Per mese di iscrizione</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginBottom:14, lineHeight:1.45}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'66px 32px repeat(5, 1fr)', gap:4, alignItems:'center'}}>
            <div></div>
            <div style={{fontSize:11.5, fontWeight:700, color:ADM.MUTED, textAlign:'right', paddingRight:3, letterSpacing:'0.02em', textTransform:'uppercase'}}>utenti</div>
            {cohortPointLabels.map(d => (
              <div key={d} style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textAlign:'center', lineHeight:1.2}}>{d}</div>
            ))}
            {retentionCohorts.map(row => (
              <React.Fragment key={row.coh}>
                <div style={{fontSize:13.5, fontWeight:700, color:ADM.TEXT}}>{row.coh}</div>
                <div style={{fontSize:12.5, color:ADM.MUTED_SOFT, textAlign:'right', paddingRight:3, fontFamily:'ui-monospace, monospace'}}>{row.size}</div>
                {row.points.map((v,j) => {
                  if (v === null) return <div key={j} style={{padding:'9px 2px', textAlign:'center', fontSize:12, color:ADM.MUTED_SOFT}}>—</div>;
          ...
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:16, lineHeight:1.55, padding:'12px 14px', background:ADM.PANEL_SOFT, borderRadius:9}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14}}>
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Distribuzione per età</div>
          <div style={{display:'flex', flexDirection:'column', gap:8}}>
            {fasceEta.map((f, i) => (
              <div key={i}>
                <div style={{display:'flex', justifyContent:'space-between', marginBottom:4}}>
                  <span style={{fontSize:14.5, color:ADM.TEXT}}>{f.label}</span>
                  <span style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>{f.pct}%</span>
                </div>
                <div style={{height:6, background:'#F4F5F7', borderRadius:99, overflow:'hidden'}}>
        ...
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Genere</div>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:10, marginTop:18}}>
            {[
              ...
              <div key={i} style={{display:'flex', alignItems:'center', gap:10}}>
                ...
                <span style={{fontSize:14.5, color:ADM.TEXT, flex:1}}>{g.label}</span>
                <span style={{fontSize:14.5, color:ADM.MUTED, fontWeight:600}}>{fmtNum(Math.round(totUtenti*(g.val/100)))} · {g.val}%</span>
              </div>
            ))}
          </div>
        </AdmCard>
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>Comportamento d'ordine per cluster demografico</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Cluster = combinazione sesso × fascia d'età · chi spende quanto · quando · cosa</div>
      </div>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Scontrino medio per sesso × età</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Scontrino medio per gruppo · ultimi 30 giorni</div>
          </div>
          <div style={{display:'flex', alignItems:'center', gap:6, fontSize:13, color:ADM.MUTED, fontWeight:600}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'80px repeat(5, 1fr)', gap:6}}>
          ...
          {['F','M'].map(sex => (
            <React.Fragment key={sex}>
              <div style={{display:'flex', alignItems:'center', gap:8, fontSize:14, fontWeight:700, color:sex==='F'?ADM.PINK:ADM.INFO}}>
              ...
              {ages.map(a => {
                ...
                return (
                  <div key={a} style={{
                    ...
                    <div style={{fontSize:18, fontWeight:800, color:intensity > 0.55 ? '#fff' : ADM.TEXT, letterSpacing:'-0.015em'}}>{fmtEur(v)}</div>
                  </div>
                );
              })}
            </React.Fragment>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1.2fr 1fr', gap:14}}>
        <AdmCard padding={20}>
          <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, marginBottom:14}}>Distribuzione scontrino</div>
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
            {ticketBuckets.map((b,i) => (
              <div key={i}>
                <div style={{display:'flex', justifyContent:'space-between', marginBottom:5}}>
                  <span style={{fontSize:14.5, color:ADM.TEXT, fontWeight:600}}>{b.range}</span>
                  <span style={{fontSize:14, color:ADM.MUTED, fontWeight:600}}>{b.pct}% utenti</span>
                </div>
        ...
        <AdmCard padding={20}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, gap:10}}>
            <div style={{minWidth:0}}>
              <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Frequenza ordini</div>
              <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Ordini al mese per utente · per fascia d'età</div>
            </div>
          </div>
          {(() => {
              ...
              <React.Fragment>
                ...
                <div style={{padding:'14px 16px', background:`linear-gradient(135deg, ${ADM.PINK_BG_SOFT}, #fff)`, border:`1px solid ${ADM.PINK_SOFT}`, borderRadius:10, display:'flex', alignItems:'center', gap:14, marginBottom:14}}>
                  <div style={{display:'flex', flexDirection:'column', alignItems:'flex-start', minWidth:84}}>
                    <div style={{fontSize:38, fontWeight:800, color:ADM.PINK_DARK, letterSpacing:'-0.03em', lineHeight:1}}>{peak.orders.toFixed(1)}</div>
                    <div style={{fontSize:12.5, color:ADM.MUTED, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.06em', marginTop:4}}>ord / mese</div>
                  </div>
                  <div style={{flex:1, minWidth:0, borderLeft:`1px solid ${ADM.PINK_SOFT}`, paddingLeft:14}}>
                    <div style={{fontSize:12.5, fontWeight:700, color:ADM.PINK_DARK, textTransform:'uppercase', letterSpacing:'0.06em'}}>Segmento più attivo</div>
                    <div style={{fontSize:17, fontWeight:700, color:ADM.TEXT, marginTop:3, letterSpacing:'-0.01em'}}>{peak.age} anni</div>
                    <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:3, lineHeight:1.4}}>Ordinano ogni <strong style={{color:ADM.TEXT}}>~{Math.round(30/peak.orders)} giorni</strong> · +{Math.round((peak.orders/freqAvg-1)*100)}% rispetto alla media app</div>
                  </div>
                </div>
                  ...
                  {sortedFreq.map((f,i) => {
                    ...
                    return (
                      <div key={f.age} style={{display:'grid', gridTemplateColumns:'52px 1fr auto', alignItems:'center', columnGap:12}}>
                        <span style={{fontSize:14, fontWeight:700, color: isPeak ? ADM.TEXT : ADM.MUTED, letterSpacing:'-0.005em'}}>{f.age}</span>
                        <div style={{display:'flex', alignItems:'center', gap:10, minWidth:0}}>
                          ...
                          <span style={{fontSize:17, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.02em', minWidth:34, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>{f.orders.toFixed(1)}</span>
                        </div>
                        <div style={{display:'flex', alignItems:'center', gap:6, justifyContent:'flex-end'}}>
                          <span style={{fontSize:12.5, fontWeight:700, color: delta > 0 ? ADM.OK : ADM.DANGER, minWidth:36, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>{delta>=0?'+':''}{delta.toFixed(0)}%</span>
                          <span style={{fontSize:12, fontWeight:700, padding:'3px 8px', background:cls.bg, color:cls.tone, borderRadius:99, letterSpacing:'0.02em', minWidth:72, textAlign:'center'}}>{cls.label}</span>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:10, lineHeight:1.5}}>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Top categorie ordinate · per cluster demografico</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Ogni colonna è un segmento di clientela <strong style={{color:ADM.TEXT}}>sesso × fascia d'età</strong>. Top 3 categorie più ordinate dal segmento, % sugli ordini del segmento.</div>
          </div>
        </div>
              ...
              <div key={i} style={{padding:'14px 14px 12px', background:ADM.PANEL_SOFT, borderRadius:9, border:`1px solid ${ADM.BORDER_SOFT}`}}>
                <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:12}}>
                  ...
                  <div style={{display:'flex', flexDirection:'column', gap:1, minWidth:0}}>
                    <span style={{fontSize:14, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.01em', lineHeight:1.1}}>{sexLabel}</span>
                    <span style={{fontSize:12.5, fontWeight:600, color:ADM.MUTED, letterSpacing:'0.02em'}}>{ageLabel} anni</span>
                  </div>
                </div>
                {row.cats.map((c,j) => (
                  <div key={j} style={{marginBottom: j < row.cats.length-1 ? 8 : 0}}>
                    <div style={{display:'flex', justifyContent:'space-between', marginBottom:3}}>
                      <span style={{fontSize:13.5, color:ADM.TEXT, fontWeight:600}}>{c.n}</span>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Fasce orarie di ordine per età</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Intensità relativa per fascia oraria · trigger per workflow push/email</div>
          </div>
        </div>
        <div style={{display:'grid', gridTemplateColumns:'80px repeat(6, 1fr)', gap:5}}>
          <div></div>
          {hourBands.map(b => (
            <div key={b} style={{fontSize:12, fontWeight:700, color:ADM.MUTED, textAlign:'center', padding:'2px 0', letterSpacing:'0.02em', lineHeight:1.3}}>{b}</div>
          ))}
          {hourCohorts.map(row => (
            <React.Fragment key={row.c}>
              <div style={{fontSize:14, fontWeight:700, color:ADM.TEXT, display:'flex', alignItems:'center'}}>{row.c}</div>
              {row.vals.map((v,j) => {
                ...
                return (
                  <div key={j} style={{
                    ...
                    <span style={{fontSize:13.5, fontWeight:700, color: intensity > 0.5 ? '#fff' : ADM.TEXT}}>{v}</span>
                  </div>
                );
              })}
            </React.Fragment>
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>Geografia del consumo</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Dove si ordina cosa</div>
      </div>
      ...
      <AdmCard padding={0}>
        <div style={{padding:'16px 20px', borderBottom:`1px solid ${ADM.BORDER}`, display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Top città · volume ordini & scontrino</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Ordini mensili aggregati e scontrino medio</div>
          </div>
        </div>
        ...
        {cityRows.map((c, i) => (
          <div key={c.citta} style={{
            ...
            <div style={{display:'flex', alignItems:'center', gap:8}}>
              <span style={{width:22, height:22, borderRadius:6, background:`hsl(${(c.citta.charCodeAt(0)*7)%360}, 45%, 55%)`, color:'#fff', fontSize:12, fontWeight:800, display:'grid', placeItems:'center'}}>{i+1}</span>
              <span style={{fontSize:15, fontWeight:600, color:ADM.TEXT}}>{c.citta}</span>
            </div>
            <div style={{fontSize:14.5, color:ADM.MUTED, textAlign:'right', fontWeight:600}}>{c.locali}</div>
            <div style={{display:'flex', alignItems:'center', gap:10}}>
              ...
              <span style={{fontSize:14.5, color:ADM.TEXT, fontWeight:700, minWidth:60, textAlign:'right'}}>{fmtNum(c.ordiniMese)}</span>
            </div>
            <div style={{fontSize:15, fontWeight:700, color:ADM.TEXT, textAlign:'right'}}>{fmtEur(c.aov)}</div>
          </div>
        ))}
      </AdmCard>
      ...
      <AdmCard padding={20}>
        <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT, marginBottom:14}}>Top piatti per macro-regione</div>
            ...
            <div key={i} style={{padding:'12px 14px', background:ADM.PANEL_SOFT, borderRadius:9, border:`1px solid ${ADM.BORDER_SOFT}`}}>
              ...
              {r.top.map((t,j) => (
                <div key={j} style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom: j < r.top.length-1 ? 6 : 0}}>
                  <span style={{fontSize:13.5, color:ADM.TEXT, fontWeight:j===0?700:500, lineHeight:1.3}}>{t.n}</span>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16, gap:14, flexWrap:'wrap'}}>
          <div style={{minWidth:0, flex:'1 1 320px'}}>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Stagionalità · confronto piatti su 12 mesi</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Indice di ordini · <strong style={{color:ADM.TEXT}}>100 = media annuale del piatto</strong>. Confronta fino a {seasonalMaxPick} piatti dello stesso o di diversi catering pattern. Dato monetizzabile verso fornitori horeca.</div>
          </div>
          {/* Region selector */}
          <div style={{display:'inline-flex', alignItems:'center', gap:8}}>
            <span style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em'}}>Area</span>
            ...
            {seasonalPickerOpen && (
              <div style={{position:'absolute', top:'100%', left:0, right:0, marginTop:6, background:'#fff', border:`1px solid ${ADM.BORDER}`, borderRadius:9, boxShadow:'0 8px 24px rgba(15,17,21,0.10)', maxHeight:280, overflowY:'auto', zIndex:30}}>
                {seasonalFiltered.length === 0 ? (
                  <div style={{padding:'12px 14px', fontSize:14, color:ADM.MUTED}}>Nessun piatto corrisponde a "{seasonalQuery}"</div>
                ) : (() => {
        ...
        {(() => {
          if (seasonalSeriesByDish.length === 0) {
            return <div style={{padding:'48px 20px', textAlign:'center', color:ADM.MUTED, fontSize:14.5}}>Seleziona almeno un piatto per visualizzare la curva di stagionalità</div>;
          }
                ...
                {seasonalSeriesByDish.map((d, di) => {
                  ...
                  return (
                    <span key={d.name} style={{display:'inline-flex', alignItems:'center', gap:6, fontSize:13.5, fontWeight:600, color:ADM.TEXT}}>
                      <span style={{width:16, height:3, borderRadius:2, background:c}}/>
                      {d.name}
                      <span style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600}}>· picco {monthsLabel[peakIdx]}</span>
                    </span>
                  );
                })}
              </div>
        ...
        <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:18, lineHeight:1.5}}>
      ...
      <div style={{display:'flex', alignItems:'center', flexWrap:'wrap', gap:'4px 12px', marginTop:18, paddingLeft:12, borderLeft:`3px solid ${ADM.PINK_DARK}`}}>
        <div style={{fontSize:18.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.015em'}}>Cosa si mangia</div>
        <div style={{fontSize:13.5, color:ADM.MUTED}}>Piatti, ingredienti, macro-nutrienti</div>
      </div>
      ...
      <div style={{padding:'11px 14px', background:ADM.WARN_SOFT, border:`1px solid #FCD34D`, borderRadius:9, display:'flex', gap:10, alignItems:'flex-start'}}>
        <BuIcons.info size={14} color={ADM.WARN}/>
        <div style={{flex:1, fontSize:13.5, color:'#92400E', lineHeight:1.5}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:14}}>
        <AdmCard padding={20}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:14, gap:10}}>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Top piatti più ordinati in Italia</div>
            <div style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600}}>Sparkline = ultimi 12 mesi · Δ% vs 12m fa</div>
          </div>
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
                ...
                <div key={i}>
                  <div style={{display:'flex', justifyContent:'space-between', marginBottom:4, alignItems:'center', gap:10}}>
                    <div style={{display:'flex', alignItems:'center', gap:8, minWidth:0, flex:1}}>
                      <span style={{fontSize:12.5, fontWeight:800, color:ADM.MUTED_SOFT, minWidth:14}}>{i+1}</span>
                      <span style={{fontSize:14.5, fontWeight:600, color:ADM.TEXT, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{p.nome}</span>
                      <span style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600}}>· {p.categoria}</span>
                    </div>
                    <MicroSpark data={p.spark} color={trendUp ? ADM.OK : ADM.DANGER} width={62} height={20}/>
                    <span style={{fontSize:12.5, fontWeight:700, color: trendUp ? ADM.OK : ADM.DANGER, minWidth:38, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>{trendUp?'+':''}{p.trend}%</span>
                    <span style={{fontSize:14, color:ADM.TEXT, fontWeight:700, minWidth:50, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>{fmtNum(p.ordini)}</span>
                  </div>
                  <div style={{height:5, background:'#F4F5F7', borderRadius:99, overflow:'hidden'}}>
        ...
        <AdmCard padding={20}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:14, gap:10}}>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Top ingredienti più ordinati</div>
            <div style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600}}>Sparkline = ultimi 12 mesi · Δ% vs 12m fa</div>
          </div>
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
              ...
              return (
                <div key={i}>
                  <div style={{display:'flex', justifyContent:'space-between', marginBottom:4, alignItems:'center', gap:10}}>
                    <div style={{display:'flex', alignItems:'center', gap:8, minWidth:0, flex:1}}>
                      <span style={{fontSize:12.5, fontWeight:800, color:ADM.MUTED_SOFT, minWidth:14}}>{i+1}</span>
                      <span style={{fontSize:14.5, fontWeight:600, color:ADM.TEXT, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{ing.n}</span>
                      <span style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600}}>· {ing.cat}</span>
                    </div>
                    <MicroSpark data={ing.spark} color={trendUp ? ADM.OK : ADM.DANGER} width={62} height={20}/>
                    <span style={{fontSize:12.5, fontWeight:700, color: trendUp ? ADM.OK : ADM.DANGER, minWidth:38, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>{trendUp?'+':''}{ing.trend}%</span>
                    <span style={{fontSize:14, color:ADM.TEXT, fontWeight:700, minWidth:50, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>{fmtNum(ing.ord)}</span>
                  </div>
                  <div style={{height:5, background:'#F4F5F7', borderRadius:99, overflow:'hidden'}}>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, gap:14, flexWrap:'wrap'}}>
          <div style={{minWidth:0, flex:'1 1 300px'}}>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Distribuzione macro-nutrienti · andamento 12 mesi</div>
            <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>% calorie medie per ordine · come la dieta si manifesta nelle diverse aree geografiche</div>
          </div>
          <div style={{display:'inline-flex', alignItems:'center', gap:8}}>
            <span style={{fontSize:12.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em'}}>Area</span>
              ...
              <div key={i} style={{padding:'14px 14px', background:ADM.PANEL_SOFT, borderRadius:9, border:`1px solid ${ADM.BORDER_SOFT}`}}>
                <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8}}>
                  <div style={{display:'flex', alignItems:'center', gap:7}}>
                    ...
                    <span style={{fontSize:13.5, color:ADM.TEXT, fontWeight:700}}>{m.n}</span>
                  </div>
                  {m.trend !== 0 && (
                    <span style={{fontSize:12.5, fontWeight:700, color:toneColor, fontVariantNumeric:'tabular-nums'}}>
                      {trendUp?'+':''}{m.trend}pp
                    </span>
                  )}
                </div>
                <div style={{display:'flex', alignItems:'baseline', gap:8}}>
                  <div style={{fontSize:28, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.025em', lineHeight:1}}>{m.pct}%</div>
                  <div style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600}}>vs {macrosTime[i].series[0]}% un anno fa</div>
                </div>
    ...
    <div style={{padding:'24px 28px', display:'flex', flexDirection:'column', gap:18}}>
        ...
        <AdmCard padding={20}>
          <div style={{display:'flex', alignItems:'flex-start', gap:14, marginBottom:14}}>
            <div style={{width:42, height:42, borderRadius:10, background:ADM[benchTone+'_SOFT'], color:ADM[benchTone], display:'grid', placeItems:'center', flexShrink:0}}>
              <BuIcons.shield size={18}/>
            </div>
            <div style={{flex:1, minWidth:0}}>
              <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Tasso di attivazione staff · giornaliero</div>
              <div style={{fontSize:14, color:ADM.MUTED, marginTop:3, lineHeight:1.5}}>
                {benchText}
              </div>
            </div>
          </div>
          ...
          <div style={{display:'flex', justifyContent:'space-between', fontSize:12.5, color:ADM.MUTED_SOFT, fontWeight:600, fontFamily:'ui-monospace, monospace', marginTop:18}}>
        ...
        <AdmCard padding={20}>
          <div style={{display:'flex', alignItems:'flex-start', gap:13}}>
            ...
            <div style={{flex:1, minWidth:0}}>
              ...
              <div style={{fontSize:30, fontWeight:800, color: staffAbbandono.length > 0 ? ADM.DANGER : ADM.OK, marginTop:4, letterSpacing:'-0.025em', lineHeight:1}}>{staffAbbandono.length}</div>
              <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:5, lineHeight:1.45}}>
          ...
          {staffAbbandono.length > 0 && (
            <div style={{marginTop:12, paddingTop:12, borderTop:`1px solid ${ADM.BORDER_SOFT}`, display:'flex', flexDirection:'column', gap:6}}>
              {staffAbbandono.slice(0, 3).map((l, i) => {
                ...
                return (
                  <div key={l.id} style={{display:'flex', alignItems:'center', gap:8, fontSize:13.5}}>
              ...
              {staffAbbandono.length > 3 && (
                <div style={{fontSize:12.5, color:ADM.MUTED, fontStyle:'italic', marginTop:2}}>e altri {staffAbbandono.length - 3}…</div>
              )}
            </div>
          )}
        </AdmCard>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, flexWrap:'wrap', gap:10}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Distribuzione attività staff</div>
            <div style={{fontSize:14, color:ADM.MUTED, marginTop:2}}>Azioni eseguite per giorno × fascia oraria · ultime 4 settimane</div>
          </div>
        ...
        <div style={{display:'grid', gridTemplateColumns:'48px repeat(7, 1fr)', gap:6, alignItems:'center'}}>
          ...
          {heatmap[0].fasce.map((_, fi) => (
            <React.Fragment key={fi}>
              <div style={{fontSize:12.5, color:ADM.MUTED, fontWeight:600, textAlign:'right', fontFamily:'ui-monospace, monospace'}}>{heatmap[0].fasce[fi].f}</div>
              {heatmap.map((d, di) => {
                ...
                return (
                  <div key={di} title={`${d.giorno} ${d.fasce[fi].f}: ${v}% intensità`}
                    ...
                    <span style={{fontSize:12.5, color: opacity > 0.55 ? '#fff' : ADM.TEXT, fontWeight:700, letterSpacing:'-0.005em'}}>{v}</span>
                  </div>
                );
              })}
            </React.Fragment>
        ...
        <div style={{marginTop:14, paddingTop:12, borderTop:`1px solid ${ADM.BORDER_SOFT}`, display:'flex', gap:18, fontSize:13.5, color:ADM.MUTED, flexWrap:'wrap'}}>
      ...
      <AdmCard padding={20}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14, flexWrap:'wrap', gap:10}}>
          <div>
            <div style={{fontSize:16, fontWeight:700, color:ADM.TEXT}}>Azioni più frequenti</div>
            <div style={{fontSize:14, color:ADM.MUTED, marginTop:2}}>Ordinate per volume · ultimi 30 giorni + trend WoW</div>
          </div>
          <div style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>{fmtNum(topActions.reduce((s,a)=>s+a.usi, 0))} azioni totali</div>
        </div>
        <div style={{display:'flex', flexDirection:'column'}}>
          {topActions.map((a,i,arr)=>{
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{minWidth:0}}>
                  <div style={{fontSize:15, color:ADM.TEXT, fontWeight:600, marginBottom:5, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{a.nome}</div>
                ...
                <div style={{textAlign:'right', fontSize:15, color:ADM.TEXT, fontWeight:700, fontFamily:'ui-monospace, monospace'}}>{fmtNum(a.usi)}</div>
  ...
  return (
    <AdmCard padding={22}>
      <div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:18, marginBottom:18, flexWrap:'wrap'}}>
        <div>
          <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT}}>Ricavi nel periodo</div>
          <div style={{fontSize:14, color:ADM.MUTED, marginTop:2}}>
        ...
        <div style={{display:'flex', gap:18, alignItems:'center'}}>
          ...
          <div style={{paddingLeft:14, borderLeft:`1px solid ${ADM.BORDER}`}}>
            ...
            <div style={{fontSize:26, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.02em', marginTop:2}}>{fmtEur(histSub+histExtra)}</div>
          </div>
        </div>
        ...
        {data.map((m, i) => {
          ...
          return (
            <div key={i} data-tip={`${m.mese}\nAbbonamenti: ${fmtEur(m.sub)}\nExtra: ${fmtEur(m.extra)}\nTotale: ${fmtEur(m.sub + m.extra)}`} style={{flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:6, height:'100%', cursor:'help'}}>
              ...
              <div style={{fontSize:12.5, color:ADM.MUTED, fontWeight:500, whiteSpace:'nowrap'}}>{m.mese}</div>
            </div>
          );
        })}
      </div>
...
function Legend({ color, label, val }) {
  return (
    <div style={{display:'flex', flexDirection:'column'}}>
      ...
      <div style={{fontSize:20, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.02em', marginTop:2, display:'flex', alignItems:'center', gap:7}}>
...
function ScreensCard() {
  ...
  return (
    <AdmCard padding={20}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
        <div>
          <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT}}>Schermate più usate</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Clicca per vedere le tab interne</div>
        </div>
        <span style={{fontSize:12.5, color:ADM.MUTED_SOFT}}>% locali che la visita</span>
      </div>
      <div style={{display:'flex', flexDirection:'column', gap:8}}>
            ...
            <div key={i}>
              <button onClick={()=> hasTabs && setExpanded(isOpen ? null : i)} style={{
                ...
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:5}}>
                  <span style={{display:'flex', alignItems:'center', gap:6, fontSize:14.5, color:ADM.TEXT, fontWeight:500}}>
                    {hasTabs ? <span style={{display:'inline-flex', color:ADM.MUTED, transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)', transition:'transform 0.15s'}}><BuIcons.chevronRight size={11}/></span> : <span style={{width:11}}/>}
                    {s.nome}
                  </span>
                  <span style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600}}>{fmtNum(s.visite)} · {s.pct}%</span>
                </div>
                <div style={{height:5, background:'#F4F5F7', borderRadius:99, overflow:'hidden'}}>
                  ...
                  {s.tabs.map((t, ti) => (
                    <div key={ti} style={{paddingLeft:10}}>
                      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:3}}>
                        <span style={{fontSize:13.5, color:ADM.TEXT}}>{t.nome}</span>
...
function FeaturesCard() {
  return (
    <AdmCard padding={20}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14}}>
        <div>
          <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT}}>Funzionalità più usate</div>
          <div style={{fontSize:13.5, color:ADM.MUTED, marginTop:2}}>Azioni dei gestori</div>
        </div>
        <span style={{fontSize:12.5, color:ADM.MUTED_SOFT}}>Ultimi 30 giorni</span>
      </div>
      <div style={{display:'flex', flexDirection:'column'}}>
        {FEATURES_USAGE.slice(0, 9).map((f, i) => (
          <div key={i} style={{display:'flex', alignItems:'center', gap:10, padding:'7.5px 0', borderBottom: i === 8 ? 'none' : `1px solid ${ADM.BORDER_SOFT}`}}>
            ...
            <div style={{flex:1, minWidth:0}}>
              <div style={{fontSize:14.5, color:ADM.TEXT, fontWeight:500, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{f.nome}</div>
              <div style={{fontSize:12.5, color:ADM.MUTED, marginTop:1}}>{f.modulo}</div>
            </div>
            <div style={{fontSize:14, color:ADM.TEXT, fontWeight:600, width:74, textAlign:'right'}}>{fmtNum(f.usi)}</div>
            <div style={{width:36, textAlign:'right'}}>
              <span style={{fontSize:12.5, fontWeight:600, color: f.trend >= 0 ? ADM.OK : ADM.DANGER}}>{f.trend >= 0 ? '↑' : '↓'} {Math.abs(f.trend)}%</span>
            </div>
          </div>
        ))}
      </div>
