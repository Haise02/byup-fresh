      ...
      <div style={{flex:1, display:'flex', overflow:'hidden'}}>
        ...
        <div style={{width:360, flexShrink:0, background:'#fff', borderRight:`1px solid ${ADM.BORDER}`, display:'flex', flexDirection:'column'}}>
          <div style={{padding:'12px 16px', borderBottom:`1px solid ${ADM.BORDER}`, display:'flex', alignItems:'center', gap:8}}>
            ...
            <div style={{flex:1}}/>
            <span style={{fontSize:18, color:ADM.MUTED, fontWeight:600}}>{filtered.length}</span>
          </div>
...
function CertKpi({ icon, color, label, value }) {
  ...
  return (
    <div style={{display:'inline-flex', alignItems:'center', gap:10}}>
      ...
      <div>
        <div style={{fontSize:17.5, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em', lineHeight:1}}>{label}</div>
        <div style={{fontSize:24, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.02em', lineHeight:1.2, marginTop:3}}>{value}</div>
      </div>
    </div>
  );
}
      ...
      <div style={{display:'flex', alignItems:'center', gap:11}}>
        ...
        <div style={{flex:1, minWidth:0}}>
          <div style={{display:'flex', alignItems:'center', gap:6, marginBottom:3}}>
            <span style={{fontSize:20, fontWeight:700, color:ADM.TEXT, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{tipo.label}</span>
            {aging && <span style={{fontSize:16, fontWeight:700, color:ADM.WARN, padding:'1px 5px', background:ADM.WARN_SOFT, borderRadius:3, textTransform:'uppercase', letterSpacing:'0.04em'}}>SLA</span>}
          </div>
          <div style={{fontSize:18.5, color:ADM.MUTED, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{loc?.nome} · {loc?.citta}</div>
          <div style={{display:'flex', alignItems:'center', gap:7, marginTop:6}}>
            {c.stato === 'pending' && <span style={{fontSize:17, fontWeight:700, color:ADM.WARN, padding:'2px 7px', borderRadius:4, background:ADM.WARN_SOFT}}>In revisione</span>}
            {c.stato === 'approvata' && <span style={{fontSize:17, fontWeight:700, color:ADM.OK, padding:'2px 7px', borderRadius:4, background:ADM.OK_SOFT}}>Approvata</span>}
            {c.stato === 'rifiutata' && <span style={{fontSize:17, fontWeight:700, color:ADM.DANGER, padding:'2px 7px', borderRadius:4, background:ADM.DANGER_SOFT}}>Rifiutata</span>}
            <span style={{fontSize:17.5, color:ADM.MUTED_SOFT}}>· {fmtRelative(c.dataInvio)}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{flex:1, display:'flex', overflow:'hidden'}}>
      ...
      <div style={{flex:1, background:'#EFEFF1', display:'flex', flexDirection:'column', overflow:'hidden'}}>
        <div style={{padding:'12px 18px', background:'#fff', borderBottom:`1px solid ${ADM.BORDER}`, display:'flex', alignItems:'center', gap:10}}>
          <div style={{width:24, height:24, borderRadius:5, background:'#DC2626', color:'#fff', display:'grid', placeItems:'center', fontSize:16, fontWeight:800, letterSpacing:'0.04em'}}>PDF</div>
          <div style={{fontSize:19.5, fontWeight:700, color:ADM.TEXT}}>{c.file}</div>
          <span style={{fontSize:18, color:ADM.MUTED}}>· {c.size}</span>
          <div style={{flex:1}}/>
        ...
        <div style={{flex:1, overflow:'auto', padding:'28px', display:'flex', justifyContent:'center'}}>
          <div style={{
            ...
            <div style={{position:'absolute', top:24, right:24, padding:'4px 9px', background:'#F3F4F6', color:'#6B7280', fontSize:16, fontWeight:700, letterSpacing:'0.05em', borderRadius:4, textTransform:'uppercase', fontFamily:'system-ui'}}>Anteprima</div>

            <div style={{textAlign:'center', borderBottom:'2px solid #999', paddingBottom:18, marginBottom:26}}>
              <div style={{fontSize:18, color:'#666', letterSpacing:'0.18em', textTransform:'uppercase'}}>{tipo.ente}</div>
              <div style={{fontSize:27, fontWeight:700, marginTop:10, letterSpacing:'-0.01em'}}>{tipo.label}</div>
              <div style={{fontSize:18, color:'#666', marginTop:6, fontStyle:'italic'}}>{tipo.desc}</div>
              <div style={{fontSize:17, color:'#888', marginTop:8, fontFamily:'monospace'}}>Documento n° {c.id} / 2025</div>
            </div>
            <div style={{fontSize:19, lineHeight:1.8}}>
              ...
              <div style={{marginTop:56, display:'flex', justifyContent:'space-between'}}>
                <div>
                  <div style={{borderTop:'1px solid #666', paddingTop:6, width:180, fontSize:17, color:'#666'}}>Data di emissione</div>
                  <div style={{marginTop:5, fontSize:18}}>{fmtDate(c.dataInvio)}</div>
                </div>
                <div>
                  <div style={{borderTop:'1px solid #666', paddingTop:6, width:180, fontSize:17, color:'#666'}}>Firma ente certificatore</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{width:380, background:'#fff', borderLeft:`1px solid ${ADM.BORDER}`, overflow:'auto', display:'flex', flexDirection:'column'}}>
        ...
        <div style={{padding:'22px 22px 18px', borderBottom:`1px solid ${ADM.BORDER}`, display:'flex', alignItems:'flex-start', gap:13}}>
          ...
          <div style={{flex:1, minWidth:0}}>
            <div style={{fontSize:17.5, color:ADM.MUTED, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.06em'}}>{tipo.ente}</div>
            <div style={{fontSize:23, fontWeight:800, color:ADM.TEXT, letterSpacing:'-0.01em', marginTop:2, lineHeight:1.25}}>{tipo.label}</div>
          </div>
        </div>

        {/* Locale info */}
        <div style={{padding:'16px 22px', borderBottom:`1px solid ${ADM.BORDER_SOFT}`, background:ADM.PANEL_SOFT}}>
          <div style={{fontSize:17.5, color:ADM.MUTED, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:5}}>Locale richiedente</div>
          <div style={{fontSize:21, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.005em'}}>{loc?.nome}</div>
          <div style={{fontSize:18.5, color:ADM.MUTED, marginTop:2}}>{loc?.tipo} · {loc?.citta} ({loc?.regione})</div>
          <div style={{fontSize:18, color:ADM.MUTED_SOFT, marginTop:5, fontFamily:'ui-monospace,monospace'}}>P.IVA {loc?.piva}</div>
        </div>
        ...
        <div style={{padding:'16px 22px', flex:1}}>
          ...
          {c.stato === 'rifiutata' && (
            <div style={{marginTop:16, padding:'12px 14px', background:ADM.DANGER_SOFT, border:`1px solid #FECACA`, borderRadius:8}}>
              <div style={{fontSize:18, fontWeight:700, color:ADM.DANGER, textTransform:'uppercase', letterSpacing:'0.04em', marginBottom:5, display:'flex', alignItems:'center', gap:5}}>
                <BuIcons.x size={16}/> Motivo del rifiuto
              </div>
              <div style={{fontSize:19.5, color:'#7F1D1D', lineHeight:1.5}}>{c.motivo}</div>
            </div>
          )}
        </div>
        ...
        {c.stato === 'pending' && (
          <div style={{padding:'18px 22px', background:ADM.PANEL_SOFT, borderTop:`1px solid ${ADM.BORDER_SOFT}`}}>
            {!decision && (
              <>
                <div style={{fontSize:18, fontWeight:700, color:ADM.MUTED, textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:12}}>Revisione documento</div>
                <div style={{display:'flex', flexDirection:'column', gap:8}}>
                  <button onClick={()=>setDecision('approva')} style={{
                    ...
                    <BuIcons.check size={20}/> Approva
                  </button>
                  <button onClick={()=>setDecision('rifiuta')} style={{
                    padding:'12px 14px', background:'#fff', color:ADM.DANGER,
                    border:`1.5px solid ${ADM.DANGER}`, borderRadius:9, fontSize:20, fontWeight:700,
                    cursor:'pointer', fontFamily:'inherit',
                    display:'flex', alignItems:'center', justifyContent:'center', gap:8,
                    letterSpacing:'-0.005em',
                  }}>
                    <BuIcons.x size={20}/> Rifiuta
                  </button>
                  <button style={{
                    padding:'10px 14px', background:'transparent', color:ADM.MUTED,
                    border:'none', borderRadius:8, fontSize:19, fontWeight:600,
                    cursor:'pointer', fontFamily:'inherit',
                  }}>
            ...
            {decision === 'approva' && (
              <>
                <div style={{fontSize:20, fontWeight:700, color:ADM.OK, marginBottom:6, display:'flex', alignItems:'center', gap:7}}>
                  <BuIcons.check size={20}/> Confermare approvazione?
                </div>
                <div style={{fontSize:18.5, color:ADM.MUTED, marginBottom:12, lineHeight:1.5}}>Il documento verrà marcato come valido e il locale riceverà una notifica.</div>
                <textarea value={motivo} onChange={e=>setMotivo(e.target.value)} placeholder="Nota interna (opzionale)" style={{
                  ...
                  fontSize:19.5, fontFamily:'inherit', resize:'vertical', outline:'none',
                  marginBottom:10, boxSizing:'border-box',
                }}/>
                <div style={{display:'flex', gap:8}}>
                  <button onClick={()=>setDecision(null)} style={{flex:1, padding:'9px 12px', background:'#fff', color:ADM.MUTED, border:`1px solid ${ADM.BORDER}`, borderRadius:8, fontSize:19, fontWeight:600, cursor:'pointer', fontFamily:'inherit'}}>Annulla</button>
                  <button style={{flex:1, padding:'9px 12px', background:`linear-gradient(135deg, ${ADM.OK}, #15803D)`, color:'#fff', border:'none', borderRadius:8, fontSize:19, fontWeight:700, cursor:'pointer', fontFamily:'inherit', display:'inline-flex', alignItems:'center', justifyContent:'center', gap:6, boxShadow:`0 4px 12px -4px ${ADM.OK}80`}}>
                    <BuIcons.check size={18}/> Conferma
                  </button>
                </div>
              </>
            )}
            ...
            {decision === 'rifiuta' && (
              <>
                <div style={{fontSize:20, fontWeight:700, color:ADM.DANGER, marginBottom:6, display:'flex', alignItems:'center', gap:7}}>
                  <BuIcons.x size={20}/> Motivo del rifiuto
                </div>
                <div style={{fontSize:18.5, color:ADM.MUTED, marginBottom:10, lineHeight:1.5}}>Sarà inviato al locale. Sii specifico così sa cosa correggere.</div>
                ...
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:17.5, color:ADM.MUTED, fontWeight:700, marginBottom:6, textTransform:'uppercase', letterSpacing:'0.05em'}}>Motivi rapidi</div>
                  <div style={{display:'flex', flexWrap:'wrap', gap:6}}>
                    {['Documento illeggibile','Scaduto','Dati non corrispondenti','Tipo sbagliato','Firma mancante','Ente non riconosciuto'].map(t => (
                      <button key={t} onClick={()=>setMotivo(motivo ? motivo+' · '+t : t)} style={{
                        ...
                        borderRadius:99, fontSize:18, cursor:'pointer', fontFamily:'inherit', color:ADM.TEXT,
                      }}>+ {t}</button>
                    ))}
                  </div>
                </div>
