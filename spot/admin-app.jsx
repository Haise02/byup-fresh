    ...
    <div style={{
      ...
      <aside style={{
        ...
        <div style={{padding:'18px 18px 14px', display:'flex', alignItems:'center', gap:11}}>
          ...
          <div style={{minWidth:0}}>
            <div style={{fontSize:16.5, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.02em'}}>byup</div>
            <div style={{fontSize:12, color:ADM.MUTED, fontWeight:600, marginTop:0, letterSpacing:'0.08em', textTransform:'uppercase'}}>Spot</div>
          </div>
        </div>
        ...
        <nav style={{flex:1, overflowY:'auto', padding:'6px 10px 14px'}}>
          {NAV_GROUPS.map((group, gi) => (
            <div key={gi} style={{marginTop: gi === 0 ? 0 : 16}}>
              {group.title && (
                <div style={{fontSize:12, fontWeight:700, color:ADM.MUTED_SOFT, textTransform:'uppercase', letterSpacing:'0.08em', padding:'8px 10px 6px'}}>{group.title}</div>
              )}
              {group.items.map(item => {
                ...
                return (
                  <button key={item.id} onClick={()=>setRoute(item.id)}
                    ...
                    <Icon size={15} color={active ? ADM.PINK : ADM.MUTED}/>
        ...
        <button onClick={()=>setRoute('profilo')}
          ...
          <AdmAvatar name="Marco Rinaldi" size={32}/>
          <div style={{flex:1, minWidth:0}}>
            <div style={{fontSize:14.5, fontWeight:600, color:ADM.TEXT, letterSpacing:'-0.01em'}}>Marco Rinaldi</div>
            <div style={{fontSize:12.5, color:ADM.MUTED, marginTop:1}}>Super Admin</div>
          </div>
          <span style={{color: ADM.MUTED_SOFT}}><BuIcons.chevronRight size={13}/></span>
        </button>
      </aside>
      ...
      <main style={{flex:1, display:'flex', flexDirection:'column', overflow:'hidden'}}>
        <header style={{
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:22, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.025em', lineHeight:1.15}}>{pt.t}</div>
            <div style={{fontSize:14.5, color:ADM.MUTED, marginTop:3, letterSpacing:'-0.005em'}}>{pt.s}</div>
          </div>
        </header>
...
function ProfiloPage() {
  ...
  return (
    <div style={{padding:28, maxWidth: 760, margin:'0 auto', display:'flex', flexDirection:'column', gap:16}}>
      {/* Header card */}
      <AdmCard padding={22}>
        <div style={{display:'flex', alignItems:'center', gap:18}}>
          <AdmAvatar name="Marco Rinaldi" size={64}/>
          <div style={{flex:1}}>
            <div style={{fontSize:22, fontWeight:700, color:ADM.TEXT, letterSpacing:'-0.01em'}}>Marco Rinaldi</div>
            <div style={{fontSize:14.5, color:ADM.MUTED, marginTop:3}}>marco.rinaldi@byup.it · Super Admin</div>
            <div style={{fontSize:13.5, color:ADM.MUTED_SOFT, marginTop:4}}>Account creato il 14 gen 2024 · Ultimo accesso oggi alle 09:42</div>
          </div>
        </div>
      </AdmCard>
      ...
      {/* Password */}
      <AdmCard padding={22}>
        <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT, marginBottom:4}}>Cambia password</div>
        <div style={{fontSize:14, color:ADM.MUTED, marginBottom:16}}>Usa almeno 8 caratteri, con lettere e numeri</div>
      ...
      <AdmCard padding={22}>
        <div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:18, marginBottom: twofa || showQr ? 16 : 0}}>
          <div style={{flex:1}}>
            <div style={{display:'flex', alignItems:'center', gap:8}}>
              <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT}}>Autenticazione a due fattori (2FA)</div>
              {twofa && <AdmBadge color="OK" size="xs">Attiva</AdmBadge>}
            </div>
            <div style={{fontSize:14, color:ADM.MUTED, marginTop:4, lineHeight:1.5}}>
        ...
        {showQr && !twofa && (
          <div style={{padding:18, background:ADM.PANEL_SOFT, borderRadius:10, border:`1px solid ${ADM.BORDER}`, display:'flex', gap:18, alignItems:'flex-start'}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:4}}>1 · Scansiona il QR code</div>
              <div style={{fontSize:14, color:ADM.MUTED, lineHeight:1.5, marginBottom:14}}>
                ...
                <div style={{marginTop:8, padding:'8px 10px', background:'#fff', border:`1px solid ${ADM.BORDER}`, borderRadius:6, fontFamily:'ui-monospace,monospace', fontSize:13.5, color:ADM.TEXT, letterSpacing:'0.06em'}}>JBSW Y3DP EHPK 3PXP</div>
              </div>
              <div style={{fontSize:15, fontWeight:600, color:ADM.TEXT, marginBottom:6}}>2 · Inserisci il codice a 6 cifre</div>
              <div style={{display:'flex', gap:8}}>
                <input maxLength={6} placeholder="123 456" style={{
                  flex:1, padding:'9px 12px', border:`1px solid ${ADM.BORDER}`, borderRadius:7,
                  fontSize:16, fontFamily:'ui-monospace,monospace', letterSpacing:'0.2em', textAlign:'center', outline:'none',
                }}/>
        ...
        {twofa && (
          <div style={{padding:14, background:ADM.OK_SOFT, borderRadius:8, border:`1px solid #BBF7D0`, display:'flex', gap:10, alignItems:'center'}}>
            <BuIcons.check size={16} color={ADM.OK}/>
            <div style={{flex:1, fontSize:14.5, color:'#065F46'}}>2FA configurata con app Authenticator. Conserva i codici di recupero in un posto sicuro.</div>
      ...
      {/* Sessioni attive */}
      <AdmCard padding={22}>
        <div style={{fontSize:16, fontWeight:600, color:ADM.TEXT, marginBottom:14}}>Sessioni attive</div>
        {[
          ...
          <div key={i} style={{display:'flex', alignItems:'center', gap:12, padding:'11px 0', borderBottom: i === a.length-1 ? 'none' : `1px solid ${ADM.BORDER_SOFT}`}}>
            <div style={{width:32, height:32, borderRadius:7, background:ADM.PANEL_SOFT, color:ADM.MUTED, display:'grid', placeItems:'center'}}><BuIcons.user size={14}/></div>
            <div style={{flex:1}}>
              <div style={{fontSize:14.5, fontWeight:600, color:ADM.TEXT}}>{s.device}</div>
              <div style={{fontSize:13.5, color:ADM.MUTED}}>{s.loc} · {s.when}</div>
            </div>
            {s.current ? <AdmBadge color="OK" size="xs">Sessione attuale</AdmBadge> : <button style={{background:'transparent', border:'none', color:ADM.DANGER, fontSize:13.5, fontWeight:600, cursor:'pointer', fontFamily:'inherit'}}>Termina</button>}
          </div>
        ))}
      </AdmCard>
    </div>
...
function ProfField({ label, type='text', value, onChange, hint }) {
  return (
    <div>
      <label style={{fontSize:13.5, color:ADM.MUTED, fontWeight:600, display:'block', marginBottom:6, letterSpacing:'-0.005em'}}>{label}</label>
      <input type={type} value={value} onChange={e=>onChange(e.target.value)} style={{
        width:'100%', padding:'10px 13px',
        border:`1px solid ${ADM.BORDER}`,
        background:'#fff',
        borderRadius:9, fontSize:15.5, fontFamily:'inherit', outline:'none', boxSizing:'border-box',
        color:ADM.TEXT, letterSpacing:'-0.005em',
      }}/>
      {hint && <div style={{fontSize:13.5, color:ADM.DANGER, marginTop:5, fontWeight:500}}>{hint}</div>}
    </div>
  );
}
