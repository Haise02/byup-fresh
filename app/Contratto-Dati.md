...
- `preferences`: `{ allergens{}, diets{} }` (booleani) — allergeni = i **14 del Reg. UE 1169/2011** nell'ordine dell'allegato II: glutine, crostacei, uova, pesce, arachidi, soia, lattosio, **fruttaguscio** (era `noci`, corretto il 28/08/2026: l'identificativo che il prodotto usa davvero è quello del menù, ed è quello che il filtro aggancia), sedano, senape, sesamo, solfiti, lupini, molluschi. L'elenco vive in un posto solo, `byup-app-kit.jsx`, che porta accanto a ciascuna voce il codice del dizionario `allergens` del modello dati (`gluten`, `crustaceans`, `eggs`, `fish`, `peanuts`, `soybeans`, `milk`, `nuts`, `celery`, `mustard`, `sesame`, `sulphites`, `lupin`, `molluscs`): è la corrispondenza che l'interfaccia applicativa userà, e sta scritta perché non vada indovinata. Diete = gli otto regimi del dizionario condiviso `dietary_labels`, con i codici del modello: `vegetariano`, `vegano`, `pescetariano`, `halal`, `kosher`, `senza_glutine`, `astemio`, `proteico` (era `senzaglutine`, corretto il 28/08/2026 perché il codice deve coincidere con quello del catalogo, altrimenti la dichiarazione della persona non incontra l'etichetta del piatto). **Regime giuridico per voce, non per schermata (D-60):** halal, kosher, vegano, vegetariano e pescetariano rivelano una convinzione religiosa o filosofica; senza glutine rivela una condizione di salute; astemio rivela entrambe; proteico non rivela né l'una né l'altra ed è dato comune. Il consenso esplicito dell'articolo 9 è richiesto per le prime sette e non per `proteico`, la cui dichiarazione si applica subito: pretenderlo anche lì sarebbe raccolta in eccesso
...
- `settled` 🧮 (prototipo): `{ lineId: { payerId: importoPagato } }` — saldo **per quota**, non per riga: si tiene traccia di *chi* ha pagato *quanto* su ogni riga, altrimenti una riga divisa in due resta aperta finché non la copre qualcuno per intero e a chi ha già saldato la sua metà la si richiede. Il residuo passa da un'unica funzione, `lineRemaining(order, it, settled, payerId)`: senza `payerId` dà il residuo della riga, con `payerId` quello della sola quota di quel commensale; `tableRemaining` ne fa la somma ed è la cifra esposta ovunque (CTA "paga tutto", popup di conferma, card in home, `byup_table.remaining`). In produzione questa granularità è sintetizzata dal **`balance`** server (fonte unica); qui è simulata client.
...
- `coperto`: la voce di coperto o servizio della sede (P-103): qualificazione (coperto | servizio) e forma (fissa a persona | percentuale sul totale) indipendenti, con importo o aliquota; nel prototipo il registro `byup_coperto` (default coperto fisso 2 €). Si espone prima della conferma e il momento va in `orders.cover_disclosed_at`
...
- **Pagamento** (Architettura-Prototipo §9, [Pagamenti-Divisione.md](Pagamenti-Divisione.md)): `{ tableSessionId, mode (mine|all|selection 🟠), paidLineIds[], quotas[ { lineId, fraction, forPayerId? } ], coperto, tip { type (pct|round), pct?, amount }, total, method, channel:'app' }`
  - **`mode`**: `mine` (i miei) · `all` (tutto il tavolo) · `selection` 🟠 (righe/quote scelte). In `PaymentScreen` la CTA a scorrimento alterna `mine` e `all` (con popup di conferma esplicita); dell'«alla romana» (`split`: 1/N del residuo del tavolo) restano solo due voci di stile irraggiungibili in `SlideToPay` — la modalità è fuori dal ciclo e nessuno la imposta; il pagamento **per selezione di righe** è di fatto già realizzato col "+" sulle card "Il tavolo" della `PaymentScreen`, quindi `selection` è più un'etichetta che un buco di UX. `quotas` copre il **piatto diviso tra più commensali** e l'**offrire** un piatto (`forPayerId` = chi riceve la quota). Nota UX: **in pagamento non nasce nessuna divisione** — il piatto preso col "+" va sul conto per intero, e le quote sono solo quelle fissate all'ordine (`splitWith` → `myShareOf`). Il popup `tableSplits` che divideva i piatti del tavolo in questa fase è stato rimosso il 2026-08-19.
  - Il pagamento **decrementa il `balance`** del tavolo (fonte unica, §G.6). Il server risponde col **saldo residuo** e con le **righe ancora scoperte** → l'app mostra il residuo senza far uscire l'utente dal flusso (§G.6). **Nel prototipo attuale** il post-pagamento va **sempre a Successo** (che elenca chi deve ancora pagare); il **residuo** vive nella card Home «**Da saldare al tavolo**» e «**Salda il resto**» riapre la `PaymentScreen`, che è l'unico punto di pagamento dine-in. La `BalanceScreen` (route `balance`) è stata **rimossa** il 2026-08-19: non era raggiungibile da alcun bottone e la sua selezione multipla generava divisioni in fase di saldo. Il modello a quote parziali è simulato con `settled` (vedi §1.6). Ciò che manca è solo il **backend**: il `balance` come **fonte unica condivisa app+cassa real-time** (qui locale) e il **lock** con `expiresAt` (qui `lockedLineIds` senza scadenza). Tavolo libero **solo a `balance` zero**.
  ...
  - **`channel:'app'`** vale il **coefficiente ridotto** del piano (contro il pieno della cassa, §C); il peso segue **dove si paga**, quindi un **ordine nato in webapp ma recuperato e pagato in app** (§G.7) conta col ridotto.

### 2.2 → Byup Fresh (asporto, §G.4)
- **Anche da webapp** (decisione ribaltata, D-14): lo scan del QR del menu asporto **apre l'ordinazione** anche dal browser; senza pagamento integrato l'ordine si salda **in cassa oppure in app**, recuperandolo col codice mostrato dalla webapp (popup in stile OTP con riconoscimento automatico all'incollaggio; su Android aggancio automatico via Install Referrer — SFA §3.8, §G.7), e va in cucina al pagamento (§G.4). Il payload sotto vale per l'app; da webapp mancano `payment{...}` e `channel:'app'`.
...
- `anagrafica` si modifica e si conferma dall'app (P-122 · D-107): niente password né assistenza; ogni modifica confermata scrive una riga di `audit_events` (`profile_updated`, campo, valore precedente, valore nuovo — D-104); una data di nascita che porta l'età sotto i 14 anni è rifiutata (Codice privacy art. 2-quinquies). Nel prototipo: `localStorage.byup_profilo` e `byup_audit_events`.
- `consent_events` (P-123 · D-03, modello v11): `{ consent_type, action: 'granted'|'revoked', occurred_at, policy_version }` con i nomi del modello — `recommendations` e `analytics` (legittimo interesse, attivi per difetto; l'opposizione è l'interruttore in «I miei dati → Privacy e consensi» e scrive **due** eventi per tocco), `dietary_suggestions` (consenso esplicito distinto, spento per difetto, dall'interruttore dentro «Dieta & allergeni»: acceso, i piatti compatibili con la dieta salgono in cima ai suggerimenti; spento, il motore non legge dieta né allergeni). Nel prototipo: `localStorage.byup_consent_state` (stato per voce) e `byup_consent_data` (log append-only), che Hubble legge in sola lettura per la linguetta Consensi della scheda utente.
...
- **Rate limit sui tentativi di riscatto** (per device/account/IP): un codice di sei cifre è forzabile → tetto tentativi + cooldown. Vedi [Sicurezza-AntiAbuso.md](Sicurezza-AntiAbuso.md). *(D-102, che rivede D-42: tre tentativi liberi, un minuto di blocco, altri tre, cinque minuti, altri tre, e al nono la chiusura del recupero dall'app per quel dispositivo, che toglie solo l'assistenza; il conteggio non decade col tempo — vedi Recupero-Ordine §3.bis.)*
