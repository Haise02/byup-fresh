...
- `settled` 🧮 (prototipo): `{ lineId: importoPagato }` — saldo **a quote parziali** per riga. Il residuo del tavolo è `Σ (prezzo·qty − importoPagato)`, sommato sui `lineRemaining` nel punto d'uso; una riga è chiusa quando l'importo copre il totale. In produzione questa granularità è sintetizzata dal **`balance`** server (fonte unica); qui è simulata client.
