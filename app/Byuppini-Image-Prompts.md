# Byuppini — prompt immagini (nanobanana / Midjourney / Ideogram)

Stile guida per tutto: **3D render kawaii soft**, stessa "materia" della mascotte byup (peluche/feltro morbido, illuminazione da studio, ombre morbide), palette **coral #E32459 · magenta hot #FF3D6E · wine #7A1C3E · lime #CEFF00 · off-white #F6ECE9**, sfondo **trasparente PNG** salvo diversa indicazione. Rendere sempre a risoluzione alta e centrato.

## 1 · Moneta "Byuppino" (valuta)
> 3D render of a soft plush-felt coin, glossy studio lighting, rounded thick edge, front face embossed with a lowercase letter "b" in the byup brand style, coral-to-magenta gradient (#FF3D6E to #B81C47) with a subtle inner ring, tiny sparkle highlight, playful premium mascot-toy aesthetic, transparent background, centered, high detail — 1:1

Variante "pila di monete" per il saldo:
> a small stack of 3 plush-felt coral coins with embossed "b", soft 3D, slight tilt, sparkles, transparent background — 1:1

## 2 · Badge di livello (set progressione, 8 tier)
Un set coerente che scala di preziosità: LIV.1 novizio → LIV.8 icona. Genera come **sprite/serie** o singoli.
> a set of 8 collectible level badges for a food loyalty game, soft 3D plush-felt style, escalating prestige from simple to elaborate: L1 simple coral disc, L2 bronze rim, L3 silver, L4 gold, L5 gold+gems, L6 wine-and-gold crown motif, L7 prismatic, L8 radiant lime-and-coral aura with a little crown; each badge a rounded medallion with the byup "b", consistent lighting, transparent background, arranged in a row — wide
...
## 5 · Mascotte — pose celebrative
Serve la mascotte byup (peluche coral) in pose per momenti di gioco:
> the byup plush mascot (coral felt character) [POSE], soft 3D, studio light, transparent background — 1:1

[POSE]: `jumping with arms up celebrating, confetti around` (level-up) · `holding a giant coin with both hands, proud` (saldo) · `sitting cross-legged meditating with a lime aura` (streak) · `pointing forward like a guide on a path` (roadmap "sei qui").

## 6 · Locali della roadmap (le "tappe" del percorso)
Otto edifici che salgono di prestigio, stesso stile coerente: **illustrazione 3D soft / flat premium**, palette byup (coral/wine/cream con accenti lime), **PNG trasparente**, vista 3/4 leggera, come le tappe di una mappa di gioco. Genera come **set coerente** (stessa luce, stesse proporzioni) o singoli.
> a set of 8 cute isometric-ish restaurant buildings for a mobile game level map, cohesive soft-3D style, byup palette (coral #E32459, wine #7A1C3E, cream, lime #CEFF00 accents), transparent background, escalating prestige: 1) tiny street food KIOSK with striped coral awning, 2) a FOOD TRUCK / snack bar, 3) a cozy TRATTORIA with a little sign, 4) a chic BISTROT with arched windows, 5) an elegant RISTORANTE with columns, 6) a luxury FINE-DINING restaurant, 7) a glamorous venue with a golden sign, 8) a byup FRANCHISE tower/skyscraper with the byup "b" logo on top; consistent lighting, playful, high detail

Oppure singolo (esempio tappa attuale):
> a cozy trattoria building, soft 3D game-asset style, coral awning and warm windows, small "byup" sign, byup palette, transparent background, 3/4 view — 1:1
...
**Stato:** nel prototipo è montato un solo pannello, `assets/road-city.png` (864×1821); cima e fondo vengono specchiati e sfumati via codice per dare continuità. Un secondo terreno (`assets/road-terrain.png`) era stato generato ma non è mai stato montato: rimosso il 2026-07-28.
