...
| Safe-area PWA | var CSS `--byup-sat` (desktop 54px, mobile `max(env(safe-area-inset-top),12px)`); bottom bar `78px` + `env(safe-area-inset-bottom)`; striscia blur sopra la status bar |
...
| In arrivo / orario futuro | `#FFC839` / `#FFD84D` |
| Chiuso / errore / danger | `#FF6B6B` (dark) · `#B42222` (light) |
| Rating star | `#FFC839` |
...
| Rilascio drag | `transform .45–.5s cubic-bezier(.2,1.4,.4,1)` | Molla elastica di ritorno |
...
| Extra | `coin.png`, `icon-*` kawaii (pasta, watermelon, popcorn…), `premium/*.webp` (23 render piatti), `road-city.png`, `avatar-default.png` |
...
5. Danger: `#FF8A8A` su dark, `#B42222` su light.
