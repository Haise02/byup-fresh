...
Come add-on è possibile integrare Fresh con applicazioni di terze parti per il delivery (10€ + IVA) oppure collegare le API con servizi terzi come Zapier, Google, Claude e ChatGPT (22,90€ + IVA). Nel prototipo il **collegamento con le app esterne non è esposto** (4 settembre 2026, per decisione del titolare; ribalta P-125): il modello lo predispone e la documentazione è in raccolta, ma l'elenco degli eventi che il ristoratore può automatizzare e dei dati che escono con ciascuno non è ancora scritto, e vale lo stesso criterio con cui è uscito Google Business Profile — un'integrazione non studiata non si promette, tanto più se genera una credenziale verso un terzo. Rientra quando quel catalogo esisterà.
...
Nel prototipo (P-124): registro, coda e i tre layout a 80 mm vivono in `stampa.jsx`; la configurazione sta in **POS e integrazioni**, in un box con «Cerca stampante», e la sezione Impostazioni → Stampanti non esiste più.

**Che cosa vuol dire «cercare».** Da una pagina web non esiste una scansione della rete locale, e il browser non espone a JavaScript l'elenco delle stampanti che il sistema conosce: nessuna API lo permette. Quindi la ricerca guarda **chi si è presentato al nostro server** — una stampante che lo interroga si annuncia da sé al primo sondaggio, col suo identificativo e il suo modello, e quella la troviamo davvero; compare in elenco appena le si scrive il nostro indirizzo nella sua pagina di configurazione. La stampante di sistema della postazione non si cerca perché non si può elencare: c'è sempre, si aggiunge come «Questa postazione», e la si sceglie nella finestra di stampa. Fingere una scansione sarebbe una promessa che il primo sabato sera si scopre falsa.

**Che cosa si sceglie quando si aggiunge.** Il nome (proposto dal modello, riscrivibile) e l'**uso**: comande, e allora si assegnano le categorie di tutti i menù come in «Collega un dispositivo» di Personale, con quelle già prese da un'altra stampante spente e col nome di chi le tiene; oppure documenti per il cliente, cioè documento commerciale e scontrino di cortesia. «Questa postazione» non può stampare comande: la stampa dal browser vuole una persona che conferma, e una comanda deve poter uscire mentre nessuno guarda uno schermo.
...
La stampa dal browser è vera; il rilevamento, lo stato «in linea» e l'esito della prova sono simulati e lo dicono. La stampa automatica del documento di cortesia per gli ordini da piattaforma è riscritta come documento in coda alla comanda sulla stampante di cucina: dal browser non può avvenire, perché la stampa dal browser vuole una persona.
