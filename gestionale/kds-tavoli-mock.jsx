...
const alleMock = window.alle, MIN_MOCK = window.MIN;

// ─── Dati finti ───────────────────────────────────────────────────────────
const ORA0 = alleMock(21, 10);
...
const TAVOLI = [
  ...
  { id:'t23', n:23, sala:'Sala interna', cop:4, arrivo: ORA0 - 34*MIN_MOCK,
    ...
    uscite:[ {id:'u1', ids:[0], avvio: ORA0-26*MIN_MOCK, pronta: ORA0-18*MIN_MOCK},
             {id:'u2', ids:[1,2], avvio: ORA0-6*MIN_MOCK, pronta:null} ] },
  ...
  { id:'t14', n:14, sala:'Dehors', cop:6, arrivo: ORA0 - 40*MIN_MOCK,
    ...
    uscite:[ {id:'u3', ids:[0], avvio: ORA0-30*MIN_MOCK, pronta: ORA0-13*MIN_MOCK} ] },

  // IL PEGGIORE: sul fuoco da 17 minuti. Uno solo diventa scuro, ed e' questo.
  { id:'t7', n:7, sala:'Sala interna', cop:2, arrivo: ORA0 - 25*MIN_MOCK,
    piatti:[ p('Impepata di cozze',1,'Antipasti'), p('Spaghetti aglio e olio',2,'Primi') ],
    uscite:[ {id:'u4', ids:[0,1], avvio: ORA0-17*MIN_MOCK, pronta:null} ] },
  ...
  { id:'t19', n:19, sala:'Dehors', cop:3, arrivo: ORA0 - 16*MIN_MOCK,
    piatti:[ p('Margherita',1,'Pizze'), p('Diavola',1,'Pizze'), p('Carbonara',1,'Primi'),
             p('Tiramisu',3,'Dolci') ],
    uscite:[ {id:'u5', ids:[0,1,2], avvio: ORA0-4*MIN_MOCK, pronta:null} ] },
  ...
  { id:'t26', n:26, sala:'Sala interna', cop:8, arrivo: ORA0 - 22*MIN_MOCK,
    ...
    uscite:[ {id:'u6', ids:[0], avvio: ORA0-11*MIN_MOCK, pronta:null},
             {id:'u7', ids:[1,2], avvio: ORA0-3*MIN_MOCK, pronta:null} ] },

  // appena arrivato: niente ancora mandato, e va bene cosi'
  { id:'t31', n:31, sala:'Saletta', cop:5, arrivo: ORA0 - 2*MIN_MOCK,
  ...
  { id:'o1', tipo:'asporto', cliente:'Anna Bianchi', ritiro: alleMock(21, 35),
    arrivo: ORA0 - 9*MIN_MOCK,
    piatti:[ p('Fritto all\'Italiana',1,'Antipasti'), p('Amatriciana',2,'Primi') ],
    uscite:[ {id:'w1', ids:[0], avvio: ORA0-5*MIN_MOCK, pronta:null} ] },

  { id:'o2', tipo:'delivery', partner:'glovo', cliente:'Luca Verdi', ritiro: alleMock(21, 22),
    arrivo: ORA0 - 14*MIN_MOCK,
    piatti:[ p('Pollo alla diavola',1,'Secondi'), p('Patate al forno',1,'Contorni') ],
    uscite:[ {id:'w2', ids:[0,1], avvio: ORA0-7*MIN_MOCK, pronta:null} ] },

  // tavolo lungo: tutto mollato, aspetta solo il dolce
  { id:'t12', n:12, sala:'Sala interna', cop:2, arrivo: ORA0 - 62*MIN_MOCK,
    piatti:[ p('Tagliere misto',1,'Antipasti'), p('Amatriciana',2,'Primi'),
             p('Branzino al sale',2,'Secondi'), p('Panna cotta',2,'Dolci') ],
    uscite:[ {id:'u8', ids:[0], avvio: ORA0-55*MIN_MOCK, pronta: ORA0-48*MIN_MOCK},
             {id:'u9', ids:[1], avvio: ORA0-40*MIN_MOCK, pronta: ORA0-33*MIN_MOCK},
             {id:'u10', ids:[2], avvio: ORA0-24*MIN_MOCK, pronta: ORA0-14*MIN_MOCK} ] },
];
