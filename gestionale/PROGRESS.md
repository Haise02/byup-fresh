...
**Ultimo aggiornamento:** 14 settembre 2026, sera (gli omonimi di primo livello: il controllo statico ora gira su tutte e ventotto le pagine). Il registro delle sessioni sta qui sotto, dalla più recente; il dettaglio delle motivazioni è in `DESIGN_DECISIONS.md`, che ha una sezione per ogni batch. Ultimo lavoro **backend**: 30 maggio 2026 — fix regressione `strictNullChecks` su colonne nullable TypeORM + warning pg (vedi §5.4, §5.5). Dal 28 luglio il lavoro è tutto sul **prototipo**: il backend descritto dalle sezioni 2–8 non è stato toccato.

### Registro delle sessioni

**14 set (25) — Gli omonimi di primo livello: il controllo statico ora gira su tutte le pagine.** Chiusa la riserva lasciata aperta da P-220. Tre pagine — la Home dell'app, Cucina e KDS — avevano nomi dichiarati con `const` in testa a due script diversi: **a runtime non è un difetto**, perché Babel abbassa i `const` di testa a `var` (è così che i simboli si vedono da un file all'altro), ma la concatenazione degli script non compila e `lint-pagine.js` su quelle pagine non veniva eseguito. È la classe in cui si era nascosto `AC_TITOLARE`. **Le pagine erano quattro, non tre**: `web/index.html` portava lo stesso difetto su `useState`, `useEffect` e `useRef` fra `venue.jsx` e `menu.jsx`, e non si vedeva perché il lint deduplica per messaggio e la riga era identica a quella della Home. **E gli omonimi non erano solo gli hook**: sulle pagine dell'app collidevano anche i dieci token di palette (`__BYUP_DARK`, `PINK`, `PINK_DARK`, `TEXT`, `MUTED`, `BORDER`, `BG_GRAY`, `BG_PAGE`, `SURF`, `TINT`) fra `map.jsx`, `app.jsx` e `menu.jsx`, il componente `Root` fra `app.jsx` e `menu.jsx`, e su KDS anche `MIN`. La via scelta è l'alias con suffisso per file, la convenzione che `auth.jsx` (`useStateA`) e `venue-variants.jsx` (`useStateV`) già usavano: **dichiarare gli hook una volta sola e togliere le ridichiarazioni non si poteva fare**, perché `build-standalone.js` riavvolge ogni file in una IIFE e negli standalone un nome dichiarato in un file non si vede dagli altri. Rinominate 927 occorrenze in otto file con uno strumento che lavora sulle posizioni dell'AST, così stringhe e commenti non vengono toccati e le proprietà shorthand `{ X }` diventano `{ X: nuovoNome }` — le chiavi degli oggetti, e quindi `Object.assign(window, …)`, restano quelle di prima. **Che sia una pura rinomina è dimostrato**, non solo verificato: applicando gli stessi rename alla versione in HEAD si riottengono i file attuali carattere per carattere, salvo un commento allineato al nome nuovo e le due chiavi `cursor` doppie di `web/menu.jsx`. **Un solo scostamento visibile, ed è una riparazione**: `map.jsx` dichiarava `TINT = '#f6f6f6'` ma quel valore non era mai stato in vigore, perché `menu.jsx` si caricava dopo e sovrascriveva il globale con `#f6f1ea`; ora la mappa usa il proprio, come già faceva nello standalone. Verifica: `lint-pagine.js` chiude con **zero FATAL, zero `no-undef`, zero `no-dupe-keys`** su tutte e ventotto le pagine, e `test-pagine.js` con le stesse quattro segnalazioni note.

**14 set (24) — Quattro identificatori che non esistevano, e la mediazione preventiva fuori dal contratto.** Sedicesimo giro, due voci dalla verifica completa del 14 settembre. **P-220** chiude i simboli mai definiti che il controllo statico ha trovato leggendo ogni pagina con tutti i suoi script: `setPrezzoAyce` in `impostazioni-menu-cucina.jsx` (il campo del prezzo del menù a prezzo per persona andava in errore alla prima cifra: il setter giusto è `setPrezzoMenu`, della stessa coppia), `tavoli` in `sala-app.jsx` (dentro `SalaApp` l'elenco si legge con `getTavoli()`, e senza quello la battuta dell'articolo fuori menù da Sala andava in errore), `onRotateFurniture` non destrutturato in `FloorPlan`, `svEur` che la finestra della fattura usava e che stava in un file che Contabilità non carica — spostata in `sala-vendita-data.jsx`, che entrambe le pagine caricano. Nella stessa passata sono cadute le chiavi doppie negli oggetti di stile (`display` in `hub-ui.jsx`, `tavoli` in `impostazioni-sala-tavoli.jsx`, `border` in `venue-variants.jsx`, `transition` e `cursor` in `menu.jsx` — quest'ultima cancellava il cursore «vietato» del bottone disabilitato), il `before` mai definito nel ripiego della Home dell'app, e l'anteprima generata `app/home-standalone.html` è stata rigenerata su tutti e nove i sorgenti. **Il `rightSlot` che arrivava all'`<input>` era la collisione di `_excluded`**: Babel compila il rest-spread del parametro in `_objectWithoutProperties(props, _excluded)`, e `_excluded` è una `var` globale che l'ultimo script caricato sovrascrive — a runtime valeva `["key"]`. `GlassField` e `LightField` calcolano il rest a mano e la collisione non li tocca più. **P-219** toglie dal contratto del prototipo la mediazione preventiva (l'articolo a sé e l'inciso «e di mediatori indicati da Byup» dell'art. 6): il file è un segnaposto per regola (D-107, P-114) e non deve corrispondere a TC-01 articolo per articolo, ma non può descrivere una procedura che TC-01 0.32 art. 21 esclude e che D-167 nega. Restano ventiquattro articoli, il foro esclusivo è sceso al 24, l'impronta è `b67017ee`. Verifica: `lint-pagine.js` chiude senza `no-undef` e senza `no-dupe-keys`, le ventotto pagine si aprono senza errori nuovi, e i tre gesti (prezzo per persona, articolo fuori menù, rotazione di un arredo) girano puliti in Chrome senza interfaccia.

**13 set (23) — Il contrasto delle board della cucina: chiuso l'ultimo punto aperto.** Un commit sulle palette, uno sui documenti. Riesaminando quello che in Fase 6 avevo lasciato aperto sono emerse due cose che cambiavano il quadro. **Non sono due pagine, sono due modi di guardare**: la Visualizzazione Ristorante e la board a tavoli (`kds-tavoli.jsx`, palette `UI`/`STATI`) e vive dentro `byup Cucina.html` e, da sola, in `byup_KDS.html` — il monitor a parete; la Visualizzazione Pub e il KDS v2 (`cucina-kds2-board.jsx`, palette `K`) e vive nella stessa Cucina e nella sua route autonoma. Quale si veda lo decide il monitor, non chi guarda. **E `byup_KDS.html` non era nella scansione del contrasto**: era nelle verifiche di reflow e di bersagli, non in quella dei colori. Coi numeri veri i testi sotto soglia erano 119 su 294 e non 85, con la board a tavoli a 69 e il KDS v2 a 50 — cioe quella messa peggio era quella SENZA una decisione alle spalle, e sistemarla ripara anche lo schermo appeso. Quattro tinte, ognuna scesa nella propria famiglia: `UI.muto` #8C8587 → #736D6F, «In preparazione» #E8402E → #CC3828, «Pronti» #1DA35C → #177F48 (il peggiore di tutti, 2,91:1, ed e la parola che manda qualcuno a ritirare il piatto), `K.TESTO_OFF` #9E9E9E → #707070 — quest'ultimo e non il #717171 che il vecchio commento proponeva, perche quello si ferma a 4,43 sulla velatura corallo della sorgente selezionata. «In attesa» #C2410C resta: passava gia. Nella board a tavoli `ink` e `testo` erano lo stesso valore e non potevano esserlo — il primo riempie e gli basta il 3:1 di 1.4.11, il secondo scrive il nome dello stato e chiede 4,5 — quindi sono diventati due campi distinti e il riempimento non e cambiato: a schermo la board e quella di prima. Il grigio di quiete del KDS v2 era una decisione esplicita, documentata: rivista perche presa contro uno standard interno mentre il metro e diventato EN 301 549, e perche non copre solo righe da ignorare ma nomi di piatti a 28 e 32 px, nomi di persone e cronometri, su uno schermo guardato da due metri. **Verifica**: 294 testi sulle tre route della cucina e 1024 su tutto il prodotto, **zero sotto soglia**; 42 combinazioni di boot senza errori; e il passo che i numeri non fanno — le due board guardate a pieno carico per vedere se «spento» si legge ancora come spento. Regge, perche il salto non e portato dal solo colore: nel nastro delle sorgenti il bordo e tratteggiato contro pieno, e nelle card il nome del piatto resta quasi nero e in grassetto contro un secondario grigio e piu leggero. Se un giorno non bastasse, la strada e il peso o l'opacita della tessera intera, mai un inchiostro piu chiaro.

**13 set (22) — Accessibilità: tre livelli di ingrandimento, e il gestionale a norma su ridimensionamento e reflow.** Sei fasi, nessun redesign. La leva è l'architettura che c'era già: tutta la UI vive in un `.frame` scalato da `zoom`, quindi ingrandire vuol dire alzare quello e RIDURRE la tela logica — zero px riscritti, e il costo si sposta tutto sul reflow. **Fondamenta**: nascono `byup-fit.js` (JS puro, possiede geometria e scala, sostituisce le undici copie del vecchio `fit()`, pubblica `BYUP_FIT` con tela logica e breakpoint, emette `byup:fit`) e `byup-a11y.jsx` (hook `useA11y`, trappola di focus, anello di focus unico, helper per finestre piene e colonne ferme). Il minimo di reflow passa da 1280 a **320 px logici**, e lo zoom si ferma dove la tela arriverebbe al minimo — su un iPad «Molto grande» vale 2,40× e su un telefono da 390 px 1,22×, cioè tutto quello che quel vetro può dare senza perdere una riga. **La riga che ha cambiato tutto**: `statStretto()` in `pn-device.js` misurava `window.innerWidth`; adesso misura la tela logica, e i ventitré file che usano `STG`/`STMIN`/`STSCROLL` si sono accesi a scala alta senza toccarne uno. Nel farlo è emerso un buco di tempismo — dedurre «pagina senza frame» dall'assenza di `.frame` nel DOM apriva la finestra del primo render di React, in cui ogni layout si sentiva rispondere la misura sbagliata: le pagine senza frame ora lo dichiarano con `data-byup-noframe`. **Chrome**: barra laterale a barretta imposta a `md` (preferenza dell'utente intatta) e cassetto con trappola di focus a `sm`/`xs`; le due colonne di Impostazioni passano da 340 a 136 px logici a `md` e a un livello per volta sotto; `PnModal` a tutto schermo con chiusura a 44; linguette che portano in vista la scheda accesa e diventano un elenco nativo quando il nastro sborda. **Schermate**: la board della cucina misurava `innerWidth` e teneva tre colonne su una tela da 646 (card da 130 px con le intestazioni tranciate); Vendita diretta passa a due viste alternate con interruttore persistente; le finestre di pagamento vanno a tutto schermo; la tabella dei Conti scorre con la data ferma a sinistra; le fasce d'avviso diventano una sola e compatta. **Controllo utente** *(rivisto in serata: la scheda è stata tolta dal prodotto — la scala si sceglie sul Login e da tastiera, il comando del movimento è rinviato; vedi A11Y-AUDIT §10.2)*: nasceva come scheda in Profilo → Dati generali, subito dopo «Lingua» — sono la stessa famiglia di scelta, in che lingua leggi il gestionale e quanto grande, e sta nel Profilo perché Impostazioni è del LOCALE mentre questa è di chi guarda lo schermo — con tre livelli etichettati in italiano e senza percentuali, ognuno disegnato ALLA MISURA CHE PROPONE, così il confronto è l'elenco stesso e non una seconda scheda, `⌘/Ctrl + Alt + +/−` su ogni schermata, «Riduci animazioni» a tre stati che parte da `prefers-reduced-motion`, e una proposta al primo avvio che si chiude per sempre se dici no. Il controllo sta lì e sul Login, dove la scala si alza PRIMA di entrare; nella barra laterale c'è stato e non c'è più — a barretta era un bottone che girava fra i livelli senza dire dove portava, e in 68 px non c'è spazio per spiegarlo. **Bersagli e contrasto**: una regola sola (`min-width`/`min-height` a 24, che vince sullo stile inline perché le proprietà `min-*` limitano la misura usata) porta i bersagli sotto misura da ventiquattro a **due**, entrambi link in linea che il criterio esclude, e le sette azioni primarie sono tutte ≥ 44; nascono i token semantici del testo (`BRAND_TEXT`, `GREEN_TEXT`, `AMBER_TEXT`, `RED_TEXT`), sostituiti in 433 punti ma solo dentro `color:` — il corallo resta il corallo ovunque faccia il riempimento — più 128 grigi scritti a mano ricondotti a `MUTED`, scurito a `#636875`, il primo che passa il 4,5 su tutte le superfici chiare. I CTA di marca vanno a 19/700 perché il testo bianco sul corallo sia conforme senza toccare la tinta (decisione del 13/09). **Verifica**: 13 superfici × 3 modalità × 3 finestre = 117 combinazioni — zero scroll orizzontale di pagina, zero testi tagliati senza recupero, zero elementi fuori dal frame, zero bersagli sotto misura, zero errori JS; test di spaziatura 1.4.12 su 11 pagine senza rotture; percorso critico login → sala → salda conto completato a 250% **solo da tastiera**. Resta scoperta la palette delle due cucine (85 testi sotto soglia): è una decisione chiusa e aspetta una scelta esplicita. Dettaglio criterio per criterio, eccezioni 1.4.10 rivendicate e residui in **`gestionale/A11Y-AUDIT.md`**.

**13 set (21) — «Modifiche prototipo 14»: gli scostamenti del riscontro delle cartelle 12 e 13.** Undici voci in sette commit, da `8f889af` a `4de06a2`. **P-209** (`8f889af`, fatta per prima): la presa d'atto scriveva `action: 'acknowledged'`, che il dominio chiuso non ammette — ora `accepted`, coi tre punti allineati e il commento che spiega perché non va rimesso. **P-211** (`cd5eac7`): il registro delle consegne diventa un insieme di PORZIONI («tavolo|piatto», l'unica chiave che le tre superfici sanno calcolare) invece di una mappa tavolo→istante; il monitor mostra le porzioni della voce, ferma il cronometro solo di quelle portate e toglie il tavolo dalla lista quando sono uscite tutte; la Sala smette di mutare `o.stato` in memoria e legge dal registro, così dopo un ricaricamento non si contraddicono più. **P-210** (`41b3f4c`): il cancello si misura dalla città guardata in entrambi i rami — provato che «concesso + Venezia» ora chiude, cosa prima impossibile da vedere — `distanze()` vuole permesso E città di residenza, e tutte le distanze (hero, slot, carosello, preferiti, mappa, foglio) passano dallo stesso controllo, ordinamento per vicinanza compreso. **P-207** (`2b077c3`): `TavoliStore.nuovoOrdine` — su conto aperto le righe entrano in quello, su conto saldato nasce la sessione 2 con `copertoGiaPagato`, il tavolo resta occupato e il pulsante dice «Nuovo ordine». **P-208, P-216, P-215, P-212** (`ab58911`): quota per territorio a 65 invece di 40, tutti i territori elencati con la dichiarazione di sotto-soglia e quanti ne mancano, la riga delle condizioni accettate spostata nella scheda del Piano attivo, e la schermata della catena che non promette più il menù ereditato — dove ho anche trovato e corretto `AC_TITOLARE`, mai definito, che faceva fallire quella schermata all'apertura. **P-214 e P-217** (`2130a23`): il segnale di ritardo arriva a `barraCucina` e l'intestazione si accende in ambra (provato con la board che corre a 6×, quindi in ~30 secondi veri); cinque frasi smentite tolte, e i due `*-standalone.html` rigenerati con `app/build-standalone.js` e marcati come artefatti, col comando per rifarli. **P-213** (`4de06a2`): il menù diventa un dato della sede (`byup_menu_sedi`), la scelta del terzo passo crea una copia o un menù vuoto, e la schermata del gestionale mostra quello della sede attiva — provato che togliendo un piatto alla sede nuova la prima resta intatta.

**13 set (20) — «Modifiche prototipo 12 e 13»: i buoni pasto come forma di pagamento, e le undici voci del tredicesimo giro.** Dodici voci in nove commit, da `e11ee41` a `6fdebe0`. **P-195** (`e11ee41`): registrare un buono non dipende da nulla — la tessera «Buoni pasto» c'è sempre in Salda conto e in Vendita diretta con la griglia a tre colonne, gli emittenti vengono da `PN_BUONI_EMITTENTI` e non dalle convenzioni, con «Altro» in coda e il campo del nome che compare solo scegliendolo (obbligatorio, e viaggia sull'accettazione), il facciale non si propone per «Altro», le accettazioni si leggono tutte. La linguetta del riepilogo in Contabilità è spenta per decisione e non per assenza di righe. **P-196** (`ee74fcc`): la lista della board a piatti si chiama «Ordini pronti» e la variabile `consegnati` diventa `mandati`; nella board a tavoli il tavolo finito lascia la board con un calco che vola verso il conteggio e le card che scivolano a chiudere il buco, si ripristina con conferma, e il tempo scorre solo con la conferma della consegna accesa — soglia dei tre minuti letta da una costante sola. **P-197** (`db7dc08`): l'interruttore in Impostazioni → Servizio, l'azione «Consegnato» sulla riga pronta della card del tavolo (più il gesto per tutto il tavolo), la coda del cameriere che scrive lo stesso segnale, banco e asporto fuori dalla regola. **P-198** (`1c4baa7`): il più apre una porzione nuova senza stato invece di gonfiare la riga, il commento fossile riscritto, nessuna guardia sul conto aperto. **P-199** (`dcaee64`): la sede di catena fa tre passi e sceglie fra il menù del soggetto e uno proprio. **P-200** (`b4658bb`): il Piano fuori dal catalogo dei documenti in Hubble e dal pacchetto nel gestionale, riga di prevalenza che rimanda all'abbonamento nella scheda del locale e nel modale del contratto. **P-201 e P-205** (`2bcb432`): via il ripiego su A6 e via `marketing_whatsapp` — un gesto, un consenso, provato contando gli eventi scritti. **P-202 e P-204** (`2de2f12`): la schermata informativa prima del prompt con la presa d'atto `acknowledged`, le tre azioni dell'avviso che portano a esiti diversi, la scelta della città, la freccetta della home che apre quella scelta, distanze e filtro per distanza che spariscono senza posizione, e il cancello di densità con Venezia e Palermo sotto soglia. **P-203** (`3f73039`): `pushIrraggiungibileDal` al posto della monetina, `hubDisinstallato` come lettura a trenta giorni, tre stati nella scheda del contatto, e gli invii che continuano a guardare il recapito caduto. **P-206** (`6fdebe0`): la scheda dello spegnimento passa in Analisi Dati → Locali, perde la frase su banche e osservatori, e in Mercato resta la quota per territorio che sotto i venti contributori dichiara di non pubblicarsi. Verifiche: Babel su ogni file toccato e render headless di ogni schermata — finestra dei buoni senza convenzioni, tre tessere in Salda conto, board a tavoli pilotata fino a svuotarsi nei due regimi, consegna dalla card, porzione nuova nel conto, passo del menù della sede, fascicolo contratti senza Piano, consensi contati sul registro, permesso negato → città → cancello, tre stati della disinstallazione, scheda spostata fra le due linguette.

**9 set (20) — «Modifiche prototipo 11»: tre voci, sessantatré punti.** Dodici commit, da `497db85` a `decac3b`. **P-192 · i difetti e le guardie (19).** `ScreenLocali` di Byup Staff leggeva una `sede` che in quella funzione non esiste e la schermata dei locali non si costruiva mai; `sedeOk` ora chiede anche il CAP. **Il modo di consegna non si deduce più** (D-146): lo porta l'ordine e nasce dalla porta da cui il cliente è entrato — la webapp riconosce le tre porte (tavolo, banco, asporto) e al banco si compone senza coperti e senza fascia. Quello che nasce al tavolo resta al tavolo: l'unico travaso è l'ordine del banco che si siede, chiesto al cliente col QR del tavolo e fatto dal personale con «Porta a un tavolo». Le fasce di ritiro guardano due orari: si ordina finché la cucina prende comande, si ritira fino alla chiusura. **L'annullo del conto non saldato** con la conferma per intero e l'età in giorni, che `liberatoOre` portava e nessuno leggeva. `cover_disclosed_at` è il momento in cui la riga del coperto COMPARE, non quello della conferma (nucleo in `panoramica-tokens`, copie guardate in app e webapp). In Hubble chi chiude una pratica di ripristino è l'account collegato; il collegamento di reimpostazione vale un quarto d'ora per i primi due livelli. Cinque testi che promettevano cose non vere (referral, supporto telefonico condizionato al piano, prossima fattura dal listino con «IVA esclusa», motivi della disdetta con «altro», due risposte del supporto). **Le pagine di Impostazioni del ruolo personalizzato governano il filtro**: i ruoli custom vivono in `byup_ruoli_custom`, `PN_UTENTE` porta le pagine concesse e colonna e cancello le rispettano — provato con un ruolo ristretto a Vetrina e Integrazioni. Dalla cucina non si stampa (D-151); Zapier è predisposta e spenta (D-145), con la forma delle piattaforme di consegna. **P-193 · i campi da completare (27).** Il menù dichiara la formula (D-141) e col prezzo per persona coperto e servizio non si applicano (D-143), regola nel nucleo del coperto e nelle due copie guardate; il tipo di menù è il QR da scaricare, tre porte (D-149). Sul piatto: tempo di preparazione, «Decongelato» al posto di «contiene surgelati» (D-152) col bollino nel menù del cliente, quantità dell'ingrediente scomposta in numero e unità, opzioni di variante con sovrapprezzo, predefinita e allergeni; il campo `channels` sparisce. La fascia di prezzo si deduce dai prezzi del menù acceso; le certificazioni hanno sei stati, con scaduta e revocata anche in Hubble. Il tavolo ha il campo attivo separato dallo stato, le sale si riordinano, l'alias sparisce, la stampante dichiara la misura della carta (D-148) e la destinazione di ripiego non esiste (D-147). L'account porta la lingua e non più fuso e valuta; uscire da un locale ha la sua causale e il suo evento a nome di chi esce; i tre pulsanti della sicurezza rispondono. In Hubble: priorità dei ticket con filtro e ordinamento, la comunicazione più vecchia non risolta nel cruscotto, i recapiti soppressi in sola lettura (D-150), i dispositivi con il proprio tipo di contatto — esclusi da consensi e campagne per costruzione, coi monitor che restano nel conteggio delle utenze. Il «disiscriviti» apre il **centro delle preferenze** (`web/preferenze.html`, D-153). Nell'app: carta e Apple Pay soltanto, via il badge dei Byuppini, e la versione del consenso letta dal documento mostrato (0.6 per l'informativa, 1.1 per i termini). **P-194 · la pulizia (17).** Commenti allineati a D-126, D-123 e alla collocazione vera di «Sicurezza e sistemi»; la vista da ristorante tiene le righe divise per tavolo e portata, e a mandarle è la sala. L'area di permesso si chiama `vendita_diretta` e resta distinta dalla chiave di navigazione; la revoca di un dispositivo ha la sua etichetta; la forma del tavolo non si scrive più; un solo tetto per i coperti. Nel profilo i ruoli sono titolare, cassa e cameriere, e la testata mostra quello del locale attivo. Via il pannello di notifiche dell'app che nessuno apriva; le fatture di Byup hanno serie, progressivo e anno. «Cessato» diventa «Disdetto» (il locale che lascia) e «Risolto» (il contratto risolto da Byup). Restano come sono, col commento che dice perché: il badge «Risparmia 15%», i numeri d'esempio dell'IVA, i calcoli della scheda camerieri che servono alle Statistiche del gestionale, la sala non cancellabile, e le piattaforme di consegna che entreranno nelle due schede per canale quando saranno collegate. **Prove**: Babel su 48 file, prova in node del nucleo del coperto con la formula del menù e della versione del consenso, Chrome headless su tredici schermate — Impostazioni (anche con un ruolo personalizzato), Sala, Cucina, KDS, Contabilità, Profilo, Statistiche, Panoramica, Login, Supporto, Onboarding, Hubble e il centro delle preferenze — più le harness di Ticket, cruscotto dell'assistenza, recapiti di invio e webapp: zero errori nuovi.

**7 set (19) — «Modifiche prototipo 10»: gli scostamenti del nono giro.** Una voce, P-191, in un commit (`6630678`), otto punti. **Il difetto bloccante**: `HubPostaComposer` chiamava `hubInterrogaPosta(..., m.filtri)` con una `m` che in quello scope non esiste — è il messaggio di `HubPostaDettaglio` — e ogni apertura di «Scrivi un messaggio» andava in errore, in servizio come in marketing; ora legge `filtri`. Provato aprendo il compositore nelle due corsie: la schermata si costruisce e la riga «Senza il consenso alla profilazione» non compare senza filtri di comportamento, mentre l'interrogazione con un filtro dà `profilato: true` e diciannove esclusi. **Il ban comunica**: la scrittura della comunicazione guardava il tipo (`if (sosp)`) invece del testo, e per il ban restava falsa anche dopo che P-190 aveva scritto il testo apposta; ora la condizione è sul testo, nel flusso e nel seme, e la via di contestazione vale anche per il ban — i due ban del seme la portano e uno nuovo la scrive. **Lo sconto in fattura dal salda conto**: la finestra era invocata senza `sconto` benché il file avesse il suo `adjustDelta`; ora passa `sconto={-adjustDelta}` come in Vendita diretta e ogni riga porta la sua aliquota congelata — provato a 0, 5 e 3,33 euro, righe e riepilogo per aliquota tornano al centesimo. **Il piatto personalizzato del cameriere entra nel carrello**: il pulsante chiudeva e basta e il chiamante non passava `onAdd`; ora la riga esce con nome, prezzo, tipologia scelta e aliquota congelata (provato pilotando la finestra). **Due code**: «Spesso ordinato con» passa da `catalogoVisibile()`, così a un minorenne non propone un alcolico, e il commento di `stat-data.jsx` cita venti consumatori e otto locali invece dei cinque clienti. Trappola ritrovata: le prove che contano i testi devono leggere il DOM SENZA i sorgenti che Babel inietta, o «profilazione» si trova anche quando a schermo non c'è.

**7 set (18) — «Modifiche prototipo 9»: dodici voci, dalla lettura integrale delle sei superfici.** Nove commit, da `6549e79` a `8059a6d`. **P-179**: il campo della carta è un riquadro del prestatore, e ora i due moduli — gestionale e app — lo dichiarano in testa; nessuna schermata cambia. **P-181**: cinque regimi fiscali (ordinario, forfettario, semplificato, agricolo, agriturismo) in una costante sola, `PN_REGIMI`, letta dalle tre tendine. **P-186**: sale e tavoli sono della sede, non del locale: la configurazione è per sede e la testata dice quale. **P-184**: il menù importato non va online da solo — si conferma con una finestra che dice quanti piatti e quante categorie, e la conferma finisce nell'audit. **P-189**: lo sconto si ripartisce sulle righe con l'ultimo che assorbe il centesimo, nome e cognome del titolare in avvio, la delega detta com'è, il «Sicuro?» diviso in due, l'accettazione nel registro condiviso. **P-190**: il ban comunica, gli insight hanno una soglia vera (venti consumatori e otto locali, con `localiDistinti` nel seme), le preferenze alimentari si contano sui consenzienti, i canali del Supporto seguono il piano. **P-185**: nasce l'area di permesso «Dati fiscali», le sezioni di Impostazioni si filtrano sul permesso con un cartello al posto della sezione, e in Hubble le azioni di assistenza e moderazione stanno dietro al permesso di scrittura. **P-182**: il pubblico costruito su filtri di comportamento è profilazione, e chi non l'ha prestata esce anche col canale acceso; il disinstallato esce dalle sole notifiche. **P-180**: l'articolo fuori menù si batte anche in Sala (pulsante accanto alla ricerca, prima tipologia e somministrazione già scelte, stessa tendina di Vendita diretta) e nella finestra «Piatto personalizzato» del cameriere, dove le voci sono righe intere perché sul telefono si sceglie col pollice; la riga nasce con categoria, tipologia e aliquota CONGELATA, entra nel tavolo così com'è, e la stampa legge invece di ricalcolare — il ripiego a dieci resta solo per le righe vecchie. Il seme del mini-menù e degli ordini acquista la tipologia. **P-183**: gli allergeni restano come sono, nessun terzo stato e il commento lo dice; le etichette dietetiche passano a quindici in `PN_ETICHETTE_DIETETICHE`, ognuna con quello che TOGLIE; l'etichetta col sovrapprezzo diventa una versione vera, col suo prezzo e i suoi allergeni, e nel menù dell'app chi filtra il glutine vede la carbonara senza glutine a 17 € con l'etichetta, mentre il piatto che quella versione non ce l'ha sparisce; la versione fa riga a sé e arriva in cucina nel nome e in un campo suo. L'aggiunta a pagamento dichiara gli allergeni che introduce, e nel foglio del piatto lo dice — in rosso se sono fra quelli dichiarati. **P-187**: il regime protettivo dei minori esiste per la prima volta — la data di nascita si scrive, il catalogo che si sfoglia si costruisce senza gli alcolici (segno `alcol` sul piatto, come `hasAlcohol` nel gestionale), la profilazione non si propone né in registrazione né nel cassetto dei consensi, i suggerimenti non si costruiscono sulla storia; il regime è un conto sull'età, quindi cade da sé a diciotto anni. **P-188**: nella prenotazione dell'app gli allergeni sono quattordici chip preselezionati dal profilo e viaggiano coi codici del modello, l'occasione è una fila di chip (la laurea entra anche nel vocabolario del gestionale), la nota torna una nota qualsiasi; e i due dialoghi dell'eliminazione dell'account smettono di promettere che si cancella tutto — i documenti fiscali si conservano per legge, lo storico resta senza il nome. Verifiche: Babel su ogni file toccato, render headless del fuori menù in Sala e della finestra del cameriere, del menù dell'app col filtro del glutine (versioni mostrate, riga con la versione nel carrello) e col regime del minorenne (zero card alcoliche su ventotto, zero suggerimenti; da maggiorenne due su trenta e cinque suggerimenti), della lista delle aggiunte col nuovo pannello degli allergeni e del foglio di prenotazione coi chip.

**7 set (17) — «Modifiche prototipo 8»: le code del settimo giro, e il cambio programmato che non entrava.** Una voce sola, P-178, nata dal riscontro sul codice della cartella 7: un difetto e dieci code. **Il difetto**: la lettura di un lato scartava il cambio programmato con la data raggiunta, e la normalizzazione — che gira dopo — non lo trovava più; il numero restava il vecchio e il cambio moriva nel registro. Ora la lettura PROMUOVE il programmato scaduto e la normalizzazione gira sul dato grezzo, riscrivendo il registro una volta sola; prova in node con data di ieri, di oggi e di domani, sul numero letto e sul contenuto del registro dopo la lettura. **Il numero di oggi torna modificabile**: il campo porta sempre l'aliquota in vigore, il cambio programmato ha il suo riquadro ambra dove si leggono e si cambiano aliquota e data, e l'audit distingue i tre casi — modifica del numero in vigore, programmazione, correzione di un cambio già programmato. **Code di P-175**: `HubStrumento` ha ora lo stato spento, con cursore e colore che lo dicono, e «Aggiungi mittente» lo usa davvero quando non c'è un dominio verificato; la scheda SMS ha «Aggiungi mittente SMS» con la sua finestra, e il controllo sulla sigla alfanumerica promozionale segnala ogni modo di opporsi diverso dal collegamento e dal rimando all'app; il nome vecchio sparisce dall'ultimo testo visibile e dal README. **Code di P-176**: in Cassa la fascia sostituisce i documenti, con una riga che dice che cosa ci sarebbe; la guardia copre tutti e quattro i punti di emissione, Byup Staff compreso; il regime entra nel seme dei locali di Hubble, così il filtro trova qualcuno anche su una rubrica appena caricata, e la derivazione pigra nella scheda fiscale sparisce; l'evento `byup-regime-change` viene ascoltato da Cassa, Sala, Vendita diretta e dalle due schermate di Staff, anche via `storage`. **Coda di P-177**: le due impronte si mostrano anche nella scheda del locale, per tutti i documenti versionati, e il commento vecchio è riscritto. **Il resoconto**: la voce «6 set (16)» è corretta dove il codice non la confermava. Prove: Babel su ogni file toccato; node sul registro con le tre date; Chrome headless su Hubble con il pannello aperto su una voce che ha un cambio programmato e sulla scheda SMS, sulla Cassa col forfettario e sulle due schermate di Byup Staff con le credenziali valide, per vedere la guardia del regime e non quella delle credenziali: zero errori.

**6 set (16) — «Modifiche prototipo 7»: quattro voci su quattro, applicate.** Un commit solo (`e6f36de`): le quattro voci toccano superfici che si leggono insieme. **La data dell'aliquota** (P-174 · D-126): nel pannello di una voce degli articoli, sotto ciascun numero, il campo facoltativo «Da questa data» con «Vuoto: vale da subito» e il minimo a domani. Vuoto, il numero vale subito; compilato, il numero in vigore non si muove e il chip porta la pillola ambra «dal 1 gen → 5%», l'audit dice «ha programmato l'aliquota» con la data per esteso, e nel pannello compare «Annulla il cambio programmato», che chiede conferma. Alla sua data il cambio entra da sé, e il registro se ne accorge (corretto da P-178: la prima stesura scartava il programmato scaduto in lettura, e la normalizzazione, che gira dopo, non lo trovava più — il numero restava il vecchio e il cambio moriva in `localStorage`). Il registro porta la chiave `prossima` per lato; il gestionale non la guarda, perché la riga d'ordine congela il numero in vigore. **Recapiti e domini di invio** (P-175 · D-127): la sezione cambia nome, i domini diventano di sola lettura — escono aggiunta, copia dei record, verifica dei domini e del CNAME, e la scelta del sottodominio — con in testa la riga che dichiara da dove arrivano i valori e il rimando alla console del fornitore, che nel mockup non porta da nessuna parte e lo dice; la firma mancante resta in ambra e dice la conseguenza, senza rimedi a schermo. I mittenti si scrivono da qui su un dominio verificato, portano lo scopo fra cinque, il recapito di risposta, il blocco identificativo in calce e il modo di opporsi, e si disattivano invece di sparire, perché le campagne spedite li nominano; nessun mittente per gli avvisi sulle informative è segnalato. Gli SMS uguale, con l'avviso sulla sigla alfanumerica promozionale. **Il forfettario si ferma** (P-176 · D-128): scegliere «Forfettario» scrive il regime nel registro condiviso `byup_regime_fiscale` e mostra la stessa frase ovunque — sotto il campo in Dati fiscali, in testa alle attivazioni (riga 1 «Non disponibile · regime forfettario», le altre spente), in Cassa, dove dal riscontro di P-178 la fascia SOSTITUISCE chiusure, riepiloghi e documenti invece di aggiungersi sopra, in Sala, in Vendita diretta e — da P-178 — nelle due schermate di Byup Staff, accanto alla guardia delle credenziali, dove spegne il pulsante — e in onboarding il passo si chiude con «Continua senza cassa fiscale». Hubble lo vede: pastiglia nella scheda del locale e proprietà «Regime fiscale» su cui filtrare — il regime sta nel seme dei locali (corretto da P-178: nasceva aprendo la scheda fiscale, quindi su una rubrica appena caricata il filtro non trovava nessuno). Il ramo con aliquota zero e natura N2.2 è scritto e spento: è una funzione a sé (`pnRigaIvaRisolta`) che oggi nessuno chiama, e si collegherà al calcolo quando arriverà la Soluzione Software. **La copia con la sua impronta** (P-177 · D-129): la copia archiviata porta un'impronta propria, distinta da quella del testo, e il piè di pagina le mostra entrambe dicendo a che cosa serve ciascuna; il rimando e le impronte valgono anche per le informative, nella schermata Documenti e — da P-178 — nella scheda del locale, e la scheda del documento dichiara i tre artefatti — markdown sorgente, HTML reso, copia PDF/A — con «Si esibisce la copia». Prove: Babel su ogni file toccato; node sul registro delle voci, sulla guardia del forfettario e sul ramo N2.2; Chrome headless su Hubble (voce modificata e cambio programmato salvato da clic simulati, le due impronte su contratto e informativa, la copia PDF/A in un iframe), sui recapiti di invio e su Dati fiscali col regime forfettario: zero errori. Trappola: un messaggio di commit con i backtick è stato mangiato dalla shell — riscritto da file e rispinto con force-with-lease.

**6 set (15) — Hubble · le voci degli articoli sono nome, descrizione e due numeri; la copia PDF/A alla pubblicazione.** Rilievo del titolare, netto: nel prototipo una voce è il nome che si vuole, la descrizione che si vuole, un'aliquota per il consumo sul posto e una da asporto — si aggiunge, si modifica, si toglie, si divide — e basta; niente fondamenti, modalità o citazioni a schermo, perché stanno nei documenti; se la legge cambia, l'operatore cambia i numeri o divide una voce, e un giorno il pane sta al 4%, un altro è una voce a sé. In Hubble la vista è la lista di card con nome, descrizione e i due chip del numero, il pannello con quattro campi (i numeri con la pillola «oggi X%» quando cambiano) e «Togli la voce…» con il passaggio degli articoli; i trattamenti, la cronologia per lato e la data «da quando» del giro precedente escono, e con loro i semi con le citazioni di legge. Il registro condiviso porta `locale.aliquota` e `asporto.aliquota`; il gestionale (`byupReadTipologieRegistro`, `pnTipologie`) legge i numeri, risolve ancora le forme scritte prima (trattamenti puntati, cambio programmato) e congela sulla riga d'ordine il profilo `voce:modo:aliquota`, così un numero che cambia vale per le righe nuove; la spiegazione sotto il campo dice solo «al banco o al tavolo X% · da asporto Y%». **Documenti**: alla pubblicazione nasce la copia PDF/A della versione (`copia` sulla versione), e «Copia archiviata · PDF/A» apre la resa d'archivio — intestazione con codice, versione e date, il testo depositato reso dal markdown, l'impronta SHA-256 e il riferimento nel piede — al posto del segnaposto di testo; la conferma di pubblicazione lo dichiara. Il markdown resta la sorgente e l'HTML il testo che si accetta: è la prassi (GitHub, Basecamp e Automattic tengono le policy in markdown sotto git; il PDF/A è la resa firmata o d'archivio). Trappola: un taglio di blocco senza intestazione trovata ha portato via la testa di `admin-team.jsx`; ricostruita da HEAD e verificata. Prove: node su seme, forma vecchia e forma nuova del registro; Chrome headless con il pannello aperto e il numero cambiato da un clic simulato, la cronologia e la copia PDF/A in un iframe: zero errori.

**6 set (14) — Hubble · Piattaforma: «Aliquote degli articoli» e «Documenti» ridisegnate.** Richiesta del titolare, con Dribbble come riferimento (esplorato in Chrome headless: le ricerche «tax settings», «version history», «legal documents dashboard», gli shot «Cannabis Taxes» di Dispense, «iGate Product Settings» di Basant Nasr, le cronologie di Never Before Seen, Lyearn, Korina Karamalaki, Dropbox Design, Autentika). **Aliquote**: prima un foglio di calcolo con campi, tendine e aree di testo accesi su ogni riga più la tabella dei trattamenti; ora una card per tipologia, che si legge — numero d'ordine, etichetta, spiegazione, i due «fatti» dell'aliquota (sul posto, da asporto) come chip con fondamento e modalità, la pillola ambra del cambio programmato, «Proposta a tutti» sulla prima — e si modifica in un pannello laterale (`HubPannello`): etichetta, spiegazione, i due trattamenti scelti da tendina con aliquota, modalità e fondamento nel nome, la data «da quando» che compare solo se il trattamento cambia, «Ritira la tipologia…» in fondo con il passaggio degli articoli; le regole stanno in una «i». **La sezione «Trattamenti IVA» è tolta**, come chiesto: i trattamenti restano nel registro e si scelgono dal pannello, «Nuovo trattamento» e «Chiudi vigenza» non esistono più a schermo. Sola lettura per chi non è Super Admin, con la pillola che lo dice. **Documenti**: il catalogo è una lista a righe per destinatario — icona, nome e natura, versione corrente, «Efficace dal» o «In arrivo dal», stato (Corrente, Pubblicata, Bozza) — con tre numeri in testa (documenti, bozze, in arrivo) e la «i» per le regole; il dettaglio ha la cronologia delle versioni come timeline (linea, un punto per versione, la corrente accesa, pillola della versione, badge, date «pubblicata → efficace», che cosa cambia, copia archiviata) e a fianco la scheda fissa: chi lo riceve, versione corrente, prossima, regola, accettazione. Bozza, pacchetto e conferma di pubblicazione restano quelli di prima. Poi, su rilievo del titolare: **le aliquote non stanno ancorate alle parole della tipologia**, perché cambiano nel tempo per legge. Sono una dimensione a sé: ogni chip dell'aliquota apre il proprio pannello con la **cronologia** (com'era, com'è oggi, cosa è programmato) e «Nuova aliquota da una data», che sceglie un trattamento in vigore o ne apre uno nuovo — aliquota, modalità, fondamento, citazione — senza riscrivere quello di oggi; il cambio programmato si annulla dal pannello. Ogni lato del registro porta `profilo`, `dal`, `prossimo` e `storico`; alla data del programmato il registro lo promuove a oggi e sposta il vecchio nello storico; il gestionale, che risolve già `prossimo` per data, non cambia. Il pannello «Modifica» tocca solo etichetta, spiegazione e ritiro. Trappola trovata: una prop chiamata `ref` non arriva al componente (React la riserva) — rinominata `lato`. Provato in Chrome headless con i tre pannelli aperti da clic simulati, su un registro con un cambio passato e uno programmato: zero errori.

**6 set (13) — «Modifiche prototipo 6»: quattro voci su quattro, applicate.** Un commit per voce: `a341733`, `64a0854`, `57e59fd`, `378313c`. **Dati fiscali** (P-170 · D-119): la riga 1 dice che le fatture partono comunque e che con la delega Byup le conserva presso l'Agenzia; la diagnosi «due delegati» non chiede di revocarne una ma apre «Faccio da me», due spunte datate che portano le righe 2 e 3 a «Dichiarata il …» in ambra; i timer spariscono — l'adesione alla conservazione dice «In attesa di Byup» finché Hubble, nel registro delle deleghe, non la segna fatta con data e nome scrivendo la stessa chiave `byup_ade_delega` — e l'accreditamento come esercente è la riga guidata di D-123: che cos'è, «Come ci si accredita» con i passi sul portale, la spunta datata. La notifica dell'atterraggio distingue credenziali e delega. «Dati fatturazione» perde il codice del canale e la sua spunta (via `byup_ade_ricezione` e `PN_CODICE_DESTINATARIO`, anche dall'onboarding): restano PEC e codice destinatario del soggetto, «Dove ricevi le fatture, comprese le nostre». Il codice fiscale di Byup resta il numero finto, dichiarato segnaposto in `byup-costanti.js` alla radice, la costante che leggono Impostazioni e Hubble. **Coperto per regione** (P-171 · D-120): `byupRegoleRegionali` e `byupRegioneDaProvincia` (107 sigle) in `panoramica-tokens`; la sede si salva da Dati anagrafici in `byup_esercente`; la scheda Servizio offre solo le qualificazioni ammesse, a Roma la sola «Servizio» con la riga sulla L.R. 22/2019, il predefinito nel Lazio è «servizio»; se la sede passa in una regione che vieta la qualificazione in uso la voce si sospende (`byup_coperto.sospesa`, letta anche da app e webapp) e la scheda chiede di reimpostarla. **Ripristino dell'accesso** (P-172 · D-121): «Password dimenticata» sul login manda il collegamento al recapito e lo dichiara, con le quarantotto ore; in Personale «Reimposta l'accesso» manda il collegamento alla persona e scrive `access_reset_sent` nel registro delle attività; in Hubble la voce «Ripristini accesso» esce, la pratica pendente è un ticket «ripristino accesso» con «Apri la pratica» che porta alla scheda del locale, tab Account, con la finestra «Non riesce ad accedere» aperta — metodo, prova, nota, esito, chi decide, le quarantotto ore, usato/valido/scaduto e «Riemetti» con l'atto a registro. Trovato per strada: `srvDataOra` non esisteva e la vecchia sezione andava in errore appena si apriva una pratica; ora c'è, e la pratica ripristinata del seme sta su un locale della rubrica. **Buoni pasto** (P-173 · D-124): in Sala e in Vendita diretta la tessera «Buoni pasto» solo con una convenzione; la finestra condivisa `PnBuoniPasto` chiede emittente, titoli fermi a otto («oltre otto buoni per transazione la legge non consente»), facciale proposto dall'ultimo usato, formato (il cartaceo: «valida il buono prima di ritirarlo»), riferimento dell'autorizzazione; se i buoni superano il dovuto lo dice prima e chiede conferma, se non lo coprono il residuo resta aperto come acconto; «La validità del buono la verifica lo strumento dell'emittente». L'accettazione si registra e `payments.method` vale `meal_voucher`; lo storno dei buoni è come il contante. Impostazioni → Integrazioni → «Buoni pasto»: le convenzioni (emittente dall'anagrafe, codice esercente, sconto, rimborso, decadenza), sconto oltre il cinque per cento segnalato. Contabilità → «Buoni pasto»: riepilogo per mese e per emittente con titoli, facciale, sconto, netto atteso, stato, numero e data della fattura; la chiusura di cassa ha la colonna «Buoni pasto», credito fuori dal contante. Registri: `byup_buoni_convenzioni`, `byup_buoni_accettazioni`, `byup_buoni_riepiloghi`. Verifiche: Babel su ogni file; prove in node (regole regionali, sospensione, buoni: otto sì e nove no, eccedenza con conferma, riepilogo con sconto e netto); render headless senza errori di Dati fiscali, registro delle deleghe e scheda del locale, scheda Servizio, Assistenza e ticket, scheda del locale con la pratica, Personale, login, convenzioni e finestra dei buoni, riepilogo, Sala. Le righe di stato con le riserve sono nel registro del prototipo.

**6 set (12) — Hubble: «Messaggi all'app» e l'editor dei form tornavano vuoti.** Il rifacimento di P-156.5 (`9246a40`) aveva tolto tre definizioni lasciandone gli usi: `POSTA_STATI` (la colonna e la pillola di stato dei messaggi), `postaCorsia` (la pillola servizio/marketing, anche nell'audit e nell'esito) e `FRM_CAMPI` (i dieci tipi di campo dell'editor dei form). Aprendo «Messaggi all'app» o un form la pagina andava in ReferenceError e restava vuota. Le tre definizioni sono ripristinate accanto a chi le usa, con `postaStato()` che non esplode su uno stato ignoto; i cache-buster di `hub-marketing.jsx`, `admin-team.jsx` e `admin-data.jsx` salgono perché in locale il browser teneva le copie vecchie. Trovato con un controllo statico: ogni `const`/`function` tolta da quel commit e ancora citata nel codice; provato in Chrome headless montando da sole le quattro pagine di Marketing, il dettaglio e il compositore della Posta, l'editor e il dettaglio dei form: zero errori.

**6 set (11) — a locale chiuso non si ordina (P-169).** La prima riserva del quinto giro diventa una voce: la specifica dei flussi 0.23 (3.3, 11.6) dice che la sessione del tavolo si apre soltanto negli orari di apertura della sede. Il nucleo di `byupCucinaInfo` guadagna `byupLocaleChiusoMessaggio()` — «Oggi il locale è chiuso» oppure «Il locale è chiuso, apre alle HH:MM», con la coda «il menù si consulta, si ordina quando è aperto» — e le tre copie restano identiche per impronta. **App e webapp**: la riga in testa dice «Chiuso · apre alle HH:MM» o «Oggi chiuso» (la riga della cucina compare solo a locale aperto); nella scheda del piatto il pulsante è spento con quella riga, eccezioni della cucina comprese; l'invio dal foglio dell'ordine è respinto con lo stesso testo, prima ancora dei limiti; nella webapp il prompt dei coperti non si apre; nell'app la scansione del QR legge il codice, dice «Il locale è chiuso» con l'ora di apertura, non apre il tavolo e non porta al menù dal tavolo — resta «Sfoglia il menù» per consultarlo. **Sala, Vendita diretta e cameriere**: nessun blocco, non ne avevano e non ne hanno. L'asporto resta come in P-167: a cucina aperta. Prova: `?ora=08:30` prima dell'apertura, o un giorno chiuso del seme (la domenica). Prova in node: domenica a mezzogiorno «Oggi chiuso», martedì alle 8:30 e alle 23:30 «apre alle 09:00», alle 12:30 nessun blocco.

**6 set (10) — «Modifiche prototipo 5»: quattro voci su quattro, applicate.** Un commit per voce, da `478d63e` a `0a6e7d4`. **Cucina** (P-165 · D-115): la sorgente della comanda porta `orders.delivery_mode` — `al_tavolo`, `asporto`, `consegna`, `al_banco` — con `partner` accanto a `consegna`; il convertitore dal monitor v1 legge il campo e non traduce più; il vocabolario `table|takeaway|delivery|order` non esiste più (l'omonimo lasciato da P-161 è chiuso). **Hubble** (P-166 · D-116): la scheda staff mostra solo «Mesi di lavoro»; scontrino medio, mancia media, mediane, «Dati da cameriere» nella scheda del locale e le due tessere del cruscotto escono; i campi restano nel seme. **Ultima comanda** (P-167 · D-117): in Servizio la scheda «Ultima comanda» — uguale alla chiusura, oppure un orario per giorno — e sul piatto «Ordinabile anche a cucina chiusa»; gli orari della Vetrina scrivono `byup_orari` e l'ultima comanda `byup_ultima_comanda`, e un nucleo unico (`byupCucinaInfo`, in `panoramica-tokens.jsx` con copie guardate in `byup-app-kit.jsx` e `web/menu.jsx`) risponde a gestionale, cameriere, app e webapp, mezzanotte compresa. Passata l'ora, app e webapp scrivono «Cucina chiusa dalle HH:MM», non fanno aggiungere i piatti non eccettuati (le bevande dei semi lo sono) e la scheda del piatto lo spiega; le fasce dell'asporto arrivano alla chiusura del locale, non a mezzanotte, e il foglio dice fino a che ora la cucina prende ordini; scoperta e scheda del locale dicono «Aperto · cucina chiusa»; Sala, Vendita diretta e cameriere lasciano partire la comanda con l'avviso. Ora di prova `?ora=HH:MM`. **Difese dei canali cliente** (P-168 · D-118): il tavolo aperto da un cliente col QR (`orders.source_surface` = `webapp_guest` | `byup_app`) porta «dal QR» sotto l'etichetta del ticket — chip del monitor e testata della stampa — nella card e nella tessera della Sala e nella card del cameriere, e sta in cima agli occupati; «da verificare», ambra e discreto, se ha superato un limite o la rete non è della città (`?rete=fuori`). App e webapp passano ogni invio da `byupInvioConsentito` — tre invii in due minuti per tavolo, trenta righe per invio, cinque tavoli in un'ora per dispositivo — e il rifiuto compare nel foglio dell'ordine con l'attesa scritta («Aspetta 40 secondi prima di inviare ancora»); nessun cancello del pagamento. Hubble → Sicurezza e sistemi ha la quarta scheda «Limiti»: i tre parametri, modifica con motivo obbligatorio e traccia in audit («Limiti dei canali»), e i rifiuti del giorno per limite, quante volte e non chi. Registri: `byup_limiti`, `byup_tavoli_qr` (le sessioni si dimenticano dopo sei ore), `byup_invii`, `byup_limiti_rifiuti`; il nucleo ha quattro copie identiche (gestionale, app, webapp, Hubble), verificate per impronta. Prova in node: il quarto invio in due minuti è respinto e il tavolo passa a «da verificare», trentuno righe sono respinte, il sesto tavolo dallo stesso dispositivo è respinto, i rifiuti si contano per limite. Le righe di stato con le riserve sono nel registro del prototipo. Guardie: ogni file toccato compila con Babel (preset react).

**5 set (notte, 9) — i nomi che il prototipo scrive sono quelli del modello (P-161 · D-115).** L'ultima voce della cartella 4, decisa dal titolare punto per punto. **Metodo di pagamento**: `payments.method` vale `cash`, `card_terminal`, `in_app`, `platform` al posto di contanti, carta, byup, piattaforma — nei semi di Conti e Sala, nella scelta del metodo in Sala e nella Vendita diretta che costruisce il pagamento, nelle mappe delle etichette (`CC_METODO`, `esibMetodo`, `methodMeta`, `PAG_META`) e in ogni lettura; le etichette a schermo restano in italiano. **Asporto dalla webapp**: `delivery_mode: 'asporto'` e `status: 'pending_payment'` al posto di `type: 'takeaway'` e `stato: 'da_saldare'`. **Disattivazione** in Byup Staff: nessuna causale, `deactivated_reason` resta nullo, `owner_deactivated` sparisce. **Opposizione al legittimo interesse**: spegnere suggerimenti o analisi d'uso scrive `action: 'opposed'`, riaccendere `'opposition_withdrawn'`; il campo `natura` esce dal registro dei consensi e dallo stato; Hubble mostra l'ultima azione. **Codici dei consensi**: `dietary_preferences` (era A3), `offers_on_preferences` (era A18), `gender` (era GEN), con la sigla del registro dei trattamenti che resta nell'etichetta; Hubble legge le chiavi nuove e i quattro del marketing. **Applicazione collegata**: `client` al posto di `application`. **Eventi d'uso**: `qr_scan` e `menu_view` portano `venue_id` (la sede del menù demo, `v_settembrini`), `app_open` no. **Profilo della cessione**: `cessione_generica_22` al posto di `ordinaria_22`, nel seme del gestionale e in quello di Hubble. Il dominio del contenuto generato non cambia: è il modello che acquista `menu_item_description`. La ricerca dei nomi vecchi come nomi del modello dà zero; restano tre omonimi che non sono quei nomi — `type: 'takeaway'` nel tipo di sorgente del monitor di cucina (`table|takeaway|delivery|order`), `da_saldare` come id del filtro di Conti e del suo parametro in URL, `'GEN'` come mese di un evento — lasciati perché D-115 dice «cambia questo, e solo questo». Prova in node: il registro scrive `recommendations:opposed`, `analytics:opposition_withdrawn`, `dietary_preferences:granted`, `gender:granted`, senza `natura`.

**5 set (sera, 8) — i documenti interni escono, la spiegazione vive nei documenti ufficiali (P-162, secondo tempo).** Diciassette file di comportamento se ne vanno — i contesti di app, webapp e staff, il contratto dei dati, pagamenti e divisione, recupero dell'ordine, sicurezza anti abuso, LIA dei suggerimenti, concept dei Byuppini, specifica del backend dell'app di incasso, riesame degli accessi, i quattro `byup-punto*`/`byup-spec-tecnica*`/`byup-contratto*` della webapp, il riferimento dei valori v7 e il `CLAUDE.md` del gestionale — dopo che il loro contenuto è entrato nel documento tecnico 0.21 e nella specifica dei flussi 0.18 (primo tempo, fatto a parte e verificato: ogni sezione citata dai nuovi rimandi esiste, e i fatti peculiari dei file usciti — la scala di D-102, A.5.18, i Byuppini, il blocco del conto col suo battito, il reinvio come conto nuovo, la webapp guest, `order_claim_lockouts` — si ritrovano là). Restano i file che parlano del mockup: design system, inventari delle icone, decisioni grafiche, handoff, `Architettura-Prototipo.md`, i README di ogni cartella, che ora rimandano alle sezioni del DPT e della SFA invece che ai file spariti; `staff/README.md` nasce per il solo mockup dell'app di incasso, `web/CLAUDE.md` e `hubble/README.md` sono ridotti a com'è fatto il prototipo. I commenti nel codice che citavano quei file (`extras.jsx`, `venue-variants.jsx`, `panoramica-tokens.jsx`, `admin-data.jsx`, `admin-team.jsx`, `web/api.jsx`, `web/menu.jsx`, le due pagine standalone dell'app) puntano ora a «documento tecnico n.n» e «specifica dei flussi n.n». La regola, da qui: niente su come funziona il prodotto si scrive dentro il prototipo; se non è nel DPT, nella SFA o nel modello, non esiste.

**5 set (sera, 7) — «Modifiche prototipo 4»: ventinove voci su trenta, applicate.** Il quarto giro chiude i rilievi medi e bassi della verifica del 3 settembre (P-135…P-164, cartella sul Desktop del titolare, con la regola: si corregge quello che il prototipo **fa** e quello che i documenti dicono, non i dati finti). Trenta commit, uno per voce, da `0c8683c` a `b4ad21f`. **Fiscale.** La **chiusura di giornata** attua D-22 (P-141): all'ora di cambio giornata la cassa aperta si chiude e ne riapre una nuova, con `closure_mode` automatico, nessun autore e il numero dei documenti senza esito scritto sulla chiusura (`byup_cassa_chiusura`); la chiusura **a mano** con documenti in ritrasmissione o in attesa non procede finché non si prende in carico la pendenza, che poi resta segnalata in testa alla Cassa fino all'ultimo esito (demo `?cambio=1`). La **finestra notturna** calcola l'ora con `timeZone: Europe/Rome` in tutte le copie e vale anche in app e webapp (P-148). Le **colonne IVA delle chiusure**, che venivano da un'impronta, escono e al loro posto c'è «Da piattaforma», così la somma delle colonne dà il totale (P-149, P-157); in console fiscale (fase 2) il velo della scheda ferma la propagazione. Il **Terminal id** del POS virtuale non è un punto aperto («Non richiesto», allegato 1 della guida AdE) e la finestra del censimento applica la regola transitoria del punto 3.1 — contratto entro il 31/01/2026 → dal 5 marzo al 20 aprile 2026 — invece di date del 2024 (P-150). Il riepilogo della Verifica fiscale si calcola sui soli documenti nella finestra, con «parziale, dalle HH:MM» (P-136). Il **coperto** si legge in testa al menù di app e webapp se la sede lo ha attivato (P-155), accetta un importo libero e la card non cita più FIPE 2023 (P-146). **Perimetro.** La **seconda sede** dello stesso soggetto è MVP (P-153, D-110 emendata): nasce nel registro `byup_sedi` sotto il ristorante, con il CAP, sopravvive al ricaricamento, il riquadro dice «Regime fiscale: lo stesso del locale» e non parla più di POS; «Sala e tavoli» legge `byup_sale`, scritto dal passo dell'onboarding; un onboarding vergine con partita IVA esistente si ferma e rimanda al profilo. L'**asporto dalla webapp** chiede la fascia di ritiro (quarti d'ora come Vendita diretta), dichiara la scadenza a ritiro più trenta minuti e usa **un codice solo a sei cifre** per banco e recupero, che l'app accetta da `byup_asporto_webapp` (P-154). L'**onboarding** chiede alla ditta individuale i dati di nascita del titolare, che Dati fiscali rilegge da `byup_anagrafica_onboarding`, e «Continua» si accende solo con le condizioni del passo (P-151). **Una fonte sola** per la forma giuridica (`byup_locale_attivo.forma`, P-152), per la valutazione del locale (`byup_valutazione`, con le stelle che seguono il numero, P-157), per lo stato di collegamento delle piattaforme (`byup_delivery_collegamenti`, P-157); la Vetrina chiama «locali collegati» quelli che chiamava sedi (P-158); in Byup Staff decidono solo le appartenenze, via `DISPOSITIVO` (P-145). **Hubble.** Le azioni della scheda utente scrivono davvero in audit e il ban ha un motivo (P-144, P-156.7); quattordici aree, Piattaforma solo al Super Admin, l'elenco degli accessi si esporta in CSV, il registro delle deleghe ha una riga per servizio con i codici fiscali, «Per tipo locale» usa le otto categorie, il glossario dice comanda, il catalogo acquista INF-04/05/06 e MOD-02 e i consensi dello staff valgono contro INF-04 (P-156). **Posta e push diventano «Messaggi all'app»** (D-113): un compositore con la corsia e la casella «anche come notifica sul telefono», l'archivio delle notifiche senza compositore, il consenso della corsia marketing è quello del marketing (almeno un canale acceso), il nodo push del workflow lo controlla (P-156.5). **«Aliquote degli articoli»** al posto dei Dizionari (P-164, D-112): tipologie con etichetta, spiegazione e ordine che valgono subito, trattamenti puntati che cambiano da una data, trattamenti versionati con quattro modalità compresa la cessione, riservato al Super Admin con audit; il gestionale legge lo stesso registro (`byup_tipologie_articolo`) attraverso i nomi di prima, e il seme porta la voce 80 e le esclusioni della quarta tipologia. Il tempo medio di utilizzo esce dalla scheda utente (P-147). **App.** Il marketing è un interruttore che ne apre tre — Email, Messaggi (SMS e WhatsApp), Notifiche — con la profilazione a parte, coi nomi del modello, e «Promo per te» compare solo con un canale acceso (P-163, D-113); la pillola «IA» segue il testo mostrato (P-137); via `autoComplete="one-time-code"` (P-138); il conteggio dei preferiti segue la lista (P-143); il testo sulle prenotazioni dice quello che dicono le condizioni d'uso (P-160). **Comportamenti.** La barra laterale nasconde le voci che il ruolo non può aprire (P-135); «Salva costo» salva il costo (P-142); la rubrica fattura comprende l'estero e si popola alla conferma (P-139); il segnalino delle notifiche si riascolta su tutti gli eventi dei registri e l'assistente manda «sposta il tavolo 4» alla sala, con sette casi di prova al caricamento (P-159); «proprietario» → «titolare» nei tre punti e le testate del contratto rimandano alle cartelle (P-160). **Documenti.** Due commenti su regole ritirate spariscono, la specifica del backend descrive il blocco del conto come stato del conto e il ritiro con le sue due colonne, il manifesto `PROVENIENZA.md` è rigenerato (P-162, punti 1-2-4-5; il punto 3 era già a D-102). **Non fatto, e detto:** P-161 (i tredici nomi) si decide con il titolare e non è stata toccata; la cancellazione in due tempi dei 36 file interni (P-162) non è cominciata, perché prima va fatta l'estrazione nei documenti ufficiali. Le righe di stato delle trenta voci sono nel registro del prototipo. Guardie: `tc01Verifica()` dà `9bf2c708` dichiarata e calcolata; ogni file toccato compila con Babel (preset react).

**5 set — la configurazione completa è ancora onboarding, e la sua rail smette di crescere.** Le due fasce di attivazione — «Collega Stripe per incassare» e «Collega i dati fiscali all'Agenzia» — **escono dalla Configurazione completa**: ieri sera erano state messe in tutti e tre i punti d'arrivo, ma quella pagina non è un arrivo — ci si entra solo dallo step 4 dell'onboarding e da lì si esce nel gestionale. Chiedere Stripe e il fiscale mentre il locale sta ancora mettendo in piedi vetrina e personale è lo stesso muro sulla porta che il 4 settembre (D-105) si era tolto dall'onboarding, rimesso due schermate più in là. Le fasce restano dove si atterra **davvero**: **Panoramica** per chi dall'onboarding rapido va dritto nel gestionale, **Profilo → Piani** per chi conclude la configurazione completa. Chi dal rapido passa al completo non le vede: è ancora onboarding.

Nella rail dello step Personale il difetto era uno solo, e stava a monte: **«Membri e dispositivi» cresceva senza limite** — tre persone più tutti i dispositivi del registro del locale d'esempio, otto righe — e spingeva fuori dalla schermata tutto quello che gli sta sotto, prima la tessera «Scarica Byup Staff» (se ne leggevano il titolo e mezza riga, la mascotte mai) e poi anche «Come creare un membro del team?». Non era un problema della tessera né del link: era l'elenco.

Ora **l'elenco ha un'altezza sua e scorre dentro**, che è il posto giusto dove far scorrere una lista. La misura è quella di **quattro righe e mezza** e non di quattro: la mezza riga tagliata in fondo è l'unico modo che ha un elenco di dire che continua, ora che la barra di scorrimento è sottile e si vede solo mentre si scorre. Sotto tornano, sempre visibili senza scorrere, la tessera di Byup Staff e il link alla guida.

Nella tessera la **mascotte va a destra e si alza**: sta all'estremità opposta del QR, spinta da un margine automatico — così sui riquadri larghi non le resta appiccicata — e i piedi tornano sulla stessa riga del fondo del codice invece di sporgere sotto, dove passa «Inquadra il codice…» e dove finiva addosso alle parole con le scarpe e con l'ombra. La sporgenza imitava il taglio a filo del banner orizzontale da cui la tessera è nata: lì la figura era alta 152 su una fascia larga, qui sotto c'è del testo e non c'è niente da tagliare. Il riquadro prende anche `flex-shrink: 0`: nella colonna della rail si lasciava comprimere e, avendo `overflow: hidden`, si mangiava l'ultima riga di testo senza dirlo.

L'elenco intanto **dimagrisce**: la riga di partenza porta **un monitor e una stampante** invece di quattro dispositivi, e la sincronizzazione col registro aggiunge **solo quello che si collega da lì** — la fotografia di ciò che il registro aveva all'apertura del passo non entra in elenco, mentre una stampante collegata o un monitor approvato dentro lo step compaiono come prima. E la **stampante prende il segno del foglio**: fino a ieri ogni dispositivo aveva l'icona del monitor, e una stampante era disegnata come uno schermo.
