        ...
        <div style={{
          ...
          <div style={{display:'flex', alignItems:'center', gap: 8, marginBottom: 10}}>
            <button onClick={() => setVal(v => Math.max(min, v - 1))} disabled={val <= min} style={{
              ...
              fontSize: 18, fontWeight: 600, color: val <= min ? '#D1D5DB' : '#0F1115',
              display:'inline-flex', alignItems:'center', justifyContent:'center',
              fontFamily:'inherit',
            }}>−</button>
            <div style={{flex: 1, textAlign:'center', fontSize: 22, fontWeight: 700, color:'#0F1115', fontVariantNumeric:'tabular-nums'}}>{val}</div>
            <button onClick={() => setVal(v => Math.min(max, v + 1))} disabled={val >= max} style={{
              ...
              fontSize: 18, fontWeight: 600, color: val >= max ? '#D1D5DB' : '#0F1115',
              display:'inline-flex', alignItems:'center', justifyContent:'center',
              fontFamily:'inherit',
            }}>+</button>
          </div>
          ...
          {overflow > 0 && (
            <Tip text={`${overflow} ospiti aggiuntivi connessi`}>
              <div style={{
                ...
                marginLeft: -overlap,
                display:'inline-flex', alignItems:'center', justifyContent:'center',
                color: '#FFFFFF', fontSize: expanded ? 9.5 : 8, fontWeight: 700,
                cursor:'help',
              }}>+{overflow}</div>
            </Tip>
          )}
        ...
        <div onClick={(e)=>e.stopPropagation()} style={{
          ...
          <div style={{display:'flex', alignItems:'center', gap: 10, marginBottom: 10}}>
            <button onClick={() => onAdjust(Math.max(coperti || 1, posti - 1))} disabled={posti <= (coperti || 1)} style={{
              ...
              fontSize: 18, fontWeight: 600, color: posti <= (coperti || 1) ? '#D1D5DB' : '#0F1115',
              display:'inline-flex', alignItems:'center', justifyContent:'center',
              fontFamily:'inherit',
            }}>−</button>
            <div style={{flex: 1, textAlign:'center'}}>
              <span style={{fontSize: 22, fontWeight: 700, color:'#0F1115', fontVariantNumeric:'tabular-nums'}}>
                {posti}
              </span>
            ...
            <button onClick={() => onAdjust(Math.min(20, posti + 1))} disabled={posti >= 20} style={{
              ...
              fontSize: 18, fontWeight: 600, color: posti >= 20 ? '#D1D5DB' : '#0F1115',
              display:'inline-flex', alignItems:'center', justifyContent:'center',
              fontFamily:'inherit',
            }}>+</button>
          </div>
...
// ─────────────────────────────────────────────────────────
function SalaV3Card({ t, expanded, onToggle, onAdd, onPay, onAddArticle, onConfirmCart, cart, onCartChange, onAdjustCoperti, onAdjustReservationPosti, onLibera, onMove, onEdit, onAssignOther, onNoShow, onUnisci, onModificaCoperti }) {
  ...
  // Menu 3-puntini — disponibili per tutte le card.
  // "Modifica Tavolo" copre sia unisci che split (vedi SalaV3UnisciModal).
  const menuItems = (() => {
    const base = [];
    base.push({ key:'modifica-tavolo', label:'Modifica Tavolo', onClick: () => onUnisci && onUnisci(t) });
    base.push({ key:'sposta',          label:'Sposta tavolo',   onClick: () => onMove && onMove(t) });
    if (t.state === 'occupato' && !occupatoSaldato) {
      base.push({ key:'libera', label:'Libera tavolo', onClick: () => onLibera && onLibera(t), danger: true });
    } else if (t.state === 'prenotato') {
      if (isLate) base.push({ key:'noshow', label:'Segna no-show', onClick: () => onNoShow && onNoShow(t), danger: true });
      else        base.push({ key:'cancella', label:'Cancella prenotazione', onClick: () => {}, danger: true });
    }
    return base;
  })();
  ...
  if (t.state === 'dapulire') {
    ...
    return (
      <div style={{display:'flex', alignItems:'center', gap: 6, flexWrap:'wrap'}}>
        <span style={{fontSize: 11.5, color: pulireColor, fontWeight: 700, flex: 1, textTransform: 'uppercase', letterSpacing: 0.4}}>
        ...
        {t.state === 'prenotato' && t.nextReservation && (
          <>
            ...
            {extraNote && extraNoteMeta && !extraNoteMeta.critical && (
              <div style={{
                fontSize: 12, color: extraNoteMeta.color, fontWeight: 600,
  ...
  return (
    <div style={{
      ...
      <div style={{display:'flex', alignItems:'center', gap: 8, marginTop: 4, paddingTop: 6, borderTop:'1px dashed #FDBA74'}}>
        ...
        <span style={{fontSize: 14, fontWeight: 700, color:'#0F1115'}}>€{total.toFixed(2)}</span>
        <span style={{flex:1}}/>
        <button onClick={(e)=>{e.stopPropagation(); onConfirm();}} style={{
          padding:'8px 14px',
          background:'linear-gradient(180deg, #B0411B 0%, #9A3412 100%)',
          color:'#fff', border:'1px solid rgba(124, 45, 18, 0.45)',
          borderRadius: 7, fontSize: 12, fontWeight: 700, cursor:'pointer',
          fontFamily:'inherit',
          boxShadow:'inset 0 1px 0 rgba(255,255,255,0.22), 0 1px 2px rgba(154,52,18,0.20)',
...
const btnIconStyle = {
  ...
  color:'#9A3412', fontSize: 14, fontWeight: 700,
...
function ContiApertiPanel({ collapsed, onToggle, onSalda }) {
  ...
  // Refs + highlight state for scroll-to-card behavior
  ...
  const scrollToIndex = (idx) => {
    ...
    if (target) {
      ...
      window.setTimeout(() => setActiveHighlight(null), 1500);
    }
  };
      ...
      <div className="pn-scroll" style={{flex: 1, overflow: 'auto', padding: 10}}>
        {conti.map((c, i) => (
          <ContoApertoCard
            ...
            isHighlighted={activeHighlight === i}
          />
        ))}
      </div>
    </aside>
...
// MOD 2: outline rosso ~1.5s quando highlighted (dopo scrollToIndex).
function ContoApertoCard({conto, isOld, onSalda, innerRef, isHighlighted}) {
  const [hover, setHover] = React.useState(false);
    ...
    <div
      ...
      style={{
        ...
        boxShadow: isHighlighted
          ? '0 0 0 3px #DC26261F, 0 0 0 1px #DC2626, 0 1px 0 rgba(15, 17, 21, 0.04)'
          : (hover ? `0 6px 16px ${accent}1F, 0 1px 2px rgba(15,17,21,0.04)` : '0 1px 0 rgba(15, 17, 21, 0.02)'),
      ...
      <div style={{display: 'flex', flexDirection: 'column', gap: 2}}>
        <div style={{fontSize: 14, fontWeight: 600, color: '#0F1115', letterSpacing: '-0.01em', fontVariantNumeric: 'tabular-nums'}}>
          €{conto.daSaldare.toFixed(2)} da incassare
        </div>
        <div style={{fontSize: 11.5, color: '#6B7280', fontWeight: 500, fontVariantNumeric: 'tabular-nums'}}>
