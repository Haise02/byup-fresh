  ...
  return (
    <div
      ...
      <div style={{display: 'flex', alignItems: 'center', gap: 8, marginTop: 2}}>
        ...
        {t.state === 'occupato' && (
          <Tip text={`Seduti da ${formatOpenDuration(t.sittingMin)}`}>
            <span style={{
              ...
              fontSize: 13, color: C.muted, fontWeight: 600, cursor:'help',
              lineHeight: 1,
            }}>
              <ChairIcon size={12}/>
        ...
        {t.state !== 'occupato' && (
          <span style={{fontSize: 13, color: C.muted, fontWeight: 500}}>· {t.posti}p</span>
        )}
      ...
      {!expanded && <SalaCardCompact t={t} alert={alert} urgent={urgent} isLate={isLate} lateMin={lateMin} cta={cta} pulireSev={pulireSev} isFill={isFill} C={C}/>}
...
function SalaCardCompact({ t, alert, urgent, isLate, lateMin, cta, pulireSev, isFill, C = SALA_FLAT_INK }) {
  if (t.state === 'libero') {
    if (!t.nextReservation) {
      return null;
    }
    return (
      <div style={{display:'flex', alignItems:'baseline', gap: 6, fontSize: 13.5, color: C.muted}}>
  ...
  if (t.state === 'prenotato') {
    if (!t.nextReservation) {
      return (
        <div style={{fontSize: 13, fontWeight: 600, color: C.muted}}>Prenotato</div>
      );
    }
    ...
    return (
      <div style={{display:'flex', flexDirection:'column', gap: 2}}>
        ...
        <div style={{display:'flex', alignItems:'baseline', gap: 6}}>
          <span style={{fontSize: 17, fontWeight: 700, color: C.text, flexShrink: 0, fontVariantNumeric:'tabular-nums'}}>
            {t.nextReservation.time}
          </span>
          <span style={{flex: 1, minWidth: 0, fontSize: 13.5, color: C.text, fontWeight: 600,
...
function SalaCardExpanded({ t, alert, cta, note, noteMeta, extraNote, extraNoteMeta, onAddArticle, onConfirmCart, cart, onCartChange, onAdjustCoperti, onAdjustReservationPosti, menuItems, occupatoSaldato, isLate, lateMin, isNoShow, onAssignOther, onNoShow, pulireSev, isFill, C = SALA_FLAT_INK }) {
  return (
    <>
      <div style={{display:'flex', flexDirection:'column', gap: 14}}>
        {t.state === 'libero' && t.nextReservation && (
          <div style={{display:'flex', flexDirection:'column', gap: 4}}>
            <div style={{fontSize: 12.5, fontWeight: 700, color: C.muted, letterSpacing: 0.4, textTransform:'uppercase'}}>
              Prossima prenotazione
            </div>
            <div style={{fontSize: 19, fontWeight: 700, color: C.text, letterSpacing:'-0.01em', lineHeight: 1.2}}>
              {t.nextReservation.time} · {t.nextReservation.name}
            </div>
            <div style={{display:'flex', alignItems:'baseline', gap: 8, fontSize: 14.5, color: C.muted}}>
        ...
        {t.state === 'prenotato' && t.nextReservation && (
          <>
            <div style={{display:'flex', flexDirection:'column', gap: 4}}>
              ...
              <div style={{fontSize: 21, fontWeight: 700, color: C.text, letterSpacing:'-0.01em', lineHeight: 1.2,
                overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontVariantNumeric:'tabular-nums'}}>
                {t.nextReservation.time} · {t.nextReservation.name}
              </div>
              <div style={{fontSize: 14.5, color: C.muted, display:'flex', alignItems:'baseline', gap: 6}}>
        ...
        {t.state === 'occupato' && (
          <>
            ...
            <div style={{
              ...
              {!occupatoSaldato && (
                <div style={{fontSize: 13, color: C.faint, fontWeight: 500, fontVariantNumeric:'tabular-nums'}}>
        ...
        {t.state === 'dapulire' && (
          <div style={{display:'flex', flexDirection:'column', gap: 4}}>
            ...
            {t.nextReservation && (
              <div style={{fontSize: 14.5, color: C.muted}}>
...
function ContiApertiPanel({ collapsed, onToggle, onSalda }) {
  const conti = SALA_CONTI_APERTI;
  const totDaSaldare = conti.reduce((s,c)=>s+c.daSaldare, 0);
  const oldThreshold = (window.SALA_THRESHOLDS || SALA_THRESHOLDS).oldBillHours;
  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        <div style={{position: 'relative', fontSize: 12.5, fontWeight: 700, color: 'rgba(255,255,255,0.62)', letterSpacing: 0.6, textTransform: 'uppercase'}}>
          Da incassare
        </div>
        <div style={{
          position: 'relative',
        ...
        <div style={{position: 'relative', textAlign: 'right', marginTop: 4}}>
          <span style={{fontSize: 12.5, color: 'rgba(255,255,255,0.48)', fontWeight: 500}}>
