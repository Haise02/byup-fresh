...
**In modifica si toglie, non si conta.** Nessun «+»: aggiungere porzioni è un altro ordine, e per quello c'è «Aggiungi articolo». Togliere invece serve, ed è due gesti distinti con due comandi distinti, entrambi a un tocco solo: il **«−» accanto al numero** toglie una porzione — «una delle tre birre non l'hanno presa», la correzione più comune che esista, fatta dove il numero si legge e si vede scendere — e il **cestino** in fondo alla riga porta via la riga intera. Stanno lontani apposta: uno corregge, l'altro fa sparire.
...
**Poi i buoni pasto sono stati sospesi del tutto, lo stesso giorno.** Non per l'interfaccia: perché accettare il buono di un emittente terzo è una transazione che avviene sui *suoi* strumenti — la sua app, il suo terminale, o un POS bancario abilitato a quel circuito dopo che l'emittente lo ha autorizzato. Nessuno dei cinque emittenti espone un'interfaccia applicativa con cui un gestionale in cloud possa accettare un buono, e una pagina servita in https non può nemmeno parlare a una scatola nella rete del locale: è lo stesso muro del contenuto misto che la D-109 ha aggirato per le stampanti. Il gestionale poteva quindi soltanto registrare a mano l'esito di una cosa avvenuta altrove — e nessuna legge obbliga un esercente ad accettarli, quindi non c'era nemmeno un dovere a giustificare quella fatica.

**Si spegne un interruttore solo.** È sparita la card delle convenzioni da Impostazioni → Integrazioni, e con lei tutta la catena: senza convenzione la tessera in cassa non compare, la scheda in Contabilità non compare, il riepilogo non ha righe. Non è stato necessario toccare nient'altro, il che è la prova che quella dipendenza era disegnata bene. La card resta nel file, non montata: è **sospesa, non cancellata**, e si riaccende rimettendo una riga.

**Quello che resta, e perché.** Il modello e il lato fiscale non si toccano: un buono incassato è non riscosso al dieci per cento col riferimento «TICKET», è un credito e non contante nella chiusura di giornata, e `payments.method` vale `meal_voucher`. Quella catena vale identica il giorno in cui il buono lo emetteremo noi — per il documento fiscale un buono è un buono — e smontarla adesso vorrebbe dire rifarla dopo tenendo nel frattempo l'ERD disallineato. La strada del buono Byup è anche l'unica che scioglie il nodo tecnico: un buono nostro è digitale, nasce dentro queste schermate e non ha bisogno di nessun terminale certificato.

## La card della cucina si legge come una comanda — 11 set 2026

Secondo giro sulla board per tavolo (visualizzazione Ristorante), tutto sullo stesso filo: **l'ordine degli stadi è quello del percorso del piatto, non quello dell'urgenza**. Prima veniva per primo ciò che scottava — in preparazione, poi l'attesa, poi i pronti — e la card si leggeva come una classifica. Adesso è *in attesa, in preparazione, pronti*: si legge dall'alto in basso come si legge una comanda, e ci guadagna anche il gesto, perché i piatti che mandi **scendono** verso lo stadio successivo invece di risalire.

**Una sezione aperta per volta, e chiudersi non vuol dire sparire.** Le due sezioni di lavoro erano fisse e non si chiudevano, per non permettere a nessuno di nascondere ciò che sta sul fuoco; il prezzo era che su un tavolo da otto, fra attesa e fuoco, quindici righe lasciavano entrare tre card nello schermo. Ora aprire uno stadio accorpa gli altri, e il rischio di prima non si ripresenta perché **un'intestazione accorpata porta il conteggio**: «IN ATTESA 6» dice quanto lavoro c'è lì dentro senza occupare sei righe. Da aperta invece il numero sparisce — ripeteva una cosa che si vede, e sul fuoco si conta quello che si ha davanti. La card apre di suo il fuoco, che è l'unico stadio coi cronometri che corrono; **agire in una sezione la sceglie**, così l'attesa non si accorpa sotto le mani di chi sta ancora mandando, e quando si svuota la card torna a decidere da sé.

**Ogni stadio porta il suo colore, con due pesi diversi.** L'attesa ha un velo arancio *sfumato* — si annuncia in cima e si ritira scendendo — e il fuoco il velo rosso pieno che aveva già. Non è una svista che siano diversi: l'attesa è la sezione più lunga della card, e una tinta uniforme su otto righe peserebbe quanto il lavoro sul fuoco, che deve restare la cosa più calda. Da due metri, senza leggere una parola, si vede dove finisce quello che aspetta e comincia quello che cuoce.

**Il comando si veste del suo stadio, tranne dove chiude il lavoro.** «Manda» e «Manda tutto» prendono l'arancio dell'attesa: lì il corallo sarebbe la terza cosa rossa della stessa sezione — filo delle tessere, anello della scelta, bottone — e un comando dentro una sezione tinta deve dire a quale sezione appartiene prima ancora di dire che è un comando. Il primario di marca resta su «Tutto pronto», che è l'azione che chiude.

**Una primaria si stringe solo se deve far posto a qualcuno.** Si stringeva a ogni selezione, anche in attesa, dove il ritorno non esiste — non c'è niente prima dell'attesa — quindi lasciava spazio a nessuno: un comando che cambia misura senza che cambi niente intorno dà solo la sensazione che la schermata si muova da sola. Ora la misura la detta la compagnia, e larga quanto la sezione vuol dire **allineata alle tessere**: prima, fra la casella vuota del ritorno e lo stacco che la seguiva, il bottone partiva sedici pixel più a destra del bordo delle righe — un disallineamento piccolo, continuo, ripetuto su ogni card della board.

Una trappola trovata per strada, e vale per tutto il repo: **questi file Babel li compila con `var`**, quindi un oggetto di props costruito sopra alla funzione che ci mette dentro non esplode — ci entra `undefined`, e il bottone fallisce solo al tocco, che è il momento peggiore per accorgersene.
...
**Anche la board Pub fa vedere dove va il piatto.** Nella visualizzazione per PIATTO il gesto è uno solo — si tocca la carta e parte la preparazione — e finiva in due fatti simultanei e slegati: la carta svaniva sotto il dito e da qualche parte, in fondo, spuntava una scheda nella fascia del conto alla rovescia. Chi premeva non vedeva niente andare da qui a lì, e quella fascia è l'unica via per tornare indietro e dura dieci secondi: la si scopriva solo guardando in basso per caso. Ora il calco della carta toccata si stacca, scende fino alla *sua* scheda e ci entra dentro; premendo «Annulla» il movimento si inverte e la carta risale al suo posto in lista, che è quello di prima perché la sua attesa non si è mai fermata.

**Un volo consegna qualcosa, o è una decorazione.** La prima versione era brutta e il motivo non era la curva: la scheda d'arrivo era già lì, a destinazione, dall'istante del tocco — quindi il calco passava davanti a un fatto già avvenuto, e per giunta sbiadendo lungo la strada, che è il modo in cui si legge «sparisce», non «arriva». Tre correzioni, e valgono per qualunque volo: la carta resta PIENA per tutto il viaggio e si spegne solo negli ultimi centocinquanta millisecondi, mentre la scheda — invisibile fino a quel momento — si accende sotto di lei con un soffio di rimbalzo; la carta arriva LARGA QUANTO LA SCHEDA e col suo stesso lato sinistro, perché la silhouette è quello che fa leggere due immagini come la stessa cosa in due momenti (una carta larga il triplo posata su una scheda stretta resta due oggetti, per quanto le si sfumi l'una nell'altra); e in volo la carta porta un'ombra lunga che rientra all'atterraggio, cioè è sollevata sopra il board e non dentro. Le carte sotto aspettano ottanta millisecondi prima di richiudere il buco: con trenta righe che si muovono nello stesso istante, quella che se ne va si perde dentro il movimento di tutte. Il volo si ancora al **lato sinistro** e non al centro: la scheda sta in fondo a sinistra, e una carta larga quanto lo schermo che punta il proprio centro là esce dal bordo della lista e ci si taglia contro per mezzo viaggio. Due trappole che valgono ovunque si animi un elemento appena nato: lo stato di partenza va forzato a calcolare (`void el.offsetWidth`) o il browser fonde partenza e arrivo e la carta arriva senza aver attraversato niente; e i nomi dichiarati in cima a un file — questi script non hanno un modulo attorno — vanno prefissati, o collidono con quelli di un altro file della stessa pagina.
