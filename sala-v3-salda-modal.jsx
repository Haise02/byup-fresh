...
function SalaV3SaldaModal({ open, tavolo, onClose, onConfirm }) {
      ...
      <span style={{flex:1, minWidth: 0, display:'inline-flex', alignItems:'center', gap: 4}}>
        {editingName ? (
          ...
          <span
            ...
            style={{
              fontSize: 13, color:'#0F1115', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap',
              cursor:'default', borderRadius: 3, padding:'1px 3px', marginLeft: -3,
            }}
          >
            {o.nome}{o._added && <span style={{marginLeft: 6, fontSize: 9, color:'#16A34A', fontWeight: 800, letterSpacing: 0.4}}>NUOVO</span>}
          </span>
        )}
      </span>
...
function AddArticleBar({ query, setQuery, open, setOpen, onPick }) {
  ...
  // Flatten SALA_V3_MENU in lista piatto per ricerca
  const allMenu = React.useMemo(() => {
    const m = window.SALA_V3_MENU || {};
    const out = [];
        ...
        {query && (
          <button
            ...
            style={{
              ...
              background:'transparent', border:'none', cursor:'pointer',
              color:'#9CA3AF', fontSize: 14, fontFamily:'inherit',
              display:'inline-flex', alignItems:'center', justifyContent:'center',
            }}>×</button>
        )}
      </div>
          ...
          {matches.length === 0 ? (
            ...
            matches.map(m => (
              <button
                ...
                onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
              >
                <span style={{flex:1, fontSize: 13, color:'#0F1115'}}>{m.nome}</span>
                <span style={{fontSize: 11, color:'#9CA3AF'}}>{m.categoria}</span>
                <span style={{fontSize: 13, fontWeight: 700, color:'#0F1115', fontVariantNumeric:'tabular-nums', minWidth: 50, textAlign:'right'}}>
                  €{m.prezzo.toFixed(2)}
                </span>
              </button>
            ))
